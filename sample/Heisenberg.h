...
{
   public:
    Heisenberg(SiteSet const &sites, Args const &args = Args::global());
...
inline Heisenberg::Heisenberg(SiteSet const &sites, Args const &args)
...
void inline Heisenberg::init_()
{
    ...
    for (int l = 0; l <= N_; ++l) {
        ...
        links.at(l) = Index(QN({"Sz", 0}), 3, QN({"Sz", -2}), 1, QN({"Sz", +2}),
                            1, Out, ts);
    }
    ...
    for (int n = 1; n <= N_; ++n) {
        ...
        W += sites_.op("Id", n) * setElt(row(1)) *
             setElt(col(1));  // ending state
        W += sites_.op("Id", n) * setElt(row(2)) *
             setElt(col(2));  // starting state
