...
namespace itensor
{
    ...
    {
        if (!store())
            Error("tensor storage unallocated");
...
#ifdef DEBUG
        if (!(*this))
            Error("default initialized tensor in randomize");
#endif
    ...
    {
        if (hasQNs(A))
            return doTask(NNZBlocks{}, A.store());
        return 1;
    }
...
#ifdef DEBUG
        if (!T)
            Error("Default initialized tensor in norm(ITensor)");
#endif
    ...
    h5_read(h5::group parent, std::string const &name, ITensor &I)
    {
        ...
        if (g.has_subgroup("storage"))
            ...
            error("Expected ITensor HDF5 data to have group named \"storage\" or \"store\"");
        ...
        if (s_type == "Dense{Float64}")
            ...
            error(tinyformat::format("Reading of ITensor storage type %s not yet supported", s_type));
    ...
    operator*=(ITensor const &R)
    {
        ...
        if (Global::checkArrows())
            detail::checkArrows(L.inds(), R.inds());
    ...
    removeQNs(ITensor T)
    {
        if (not hasQNs(T))
            return T;
        if (T.store())
            doTask(RemoveQNs{inds(T)}, T.store());
        auto nis = inds(T);
        nis.removeQNs();
    ...
    operator/=(ITensor const &R)
    {
        ...
        if (Global::checkArrows())
            detail::checkArrows(L.inds(), R.inds(), true);
          ...
          Real alpha)
    {
        ...
        if (!L.store())
            Error("L not initialized in daxpy");
    ...
    operator+=(ITensor const &R)
    {
        ...
        if (!R)
            Error("Right-hand-side of ITensor += is default constructed");
        if (&L == &R)
            return operator*=(2.);
                  ...
                  IndexSet const &is)
    {
        ...
        if (isReal)
        {
            ...
            for (auto n : range(M.size()))
                store[n] = M.store()[n].real();
    ...
    combiner(IndexSet const &inds, Args const &args)
    {
        ...
        {
            if (inds.empty())
                Error("No indices passed to combiner");
            long rm = 1;
    ...
    randomITensor(QN q, IndexSet const &is)
    {
        ...
        if (nnz(T) == 0)
            Error("Requested QN for random ITensor resulted in zero allowed blocks (QN not satisfiable by any settings of the indices)");
    ...
    div(ITensor const &T)
    {
        ...
        if (!T)
            Error("div(ITensor) not defined for unallocated IQTensor");
