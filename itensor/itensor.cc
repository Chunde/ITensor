...
ITensor::ITensor(IndexSet const &is) : is_(is)
{
    IF_USESCALE(scale_ = LogNum(1.);)
}

ITensor::ITensor(IndexSet iset, storage_ptr &&pdat, LogNum const &scale)
    : is_(std::move(iset)), store_(std::move(pdat))
{
    IF_USESCALE(scale_ = scale;)
}

ITensor::ITensor(std::initializer_list<Index> inds) : is_(inds)
{
    IF_USESCALE(scale_ = LogNum(1.);)
}

ITensor::ITensor(Cplx val)
{
    IF_USESCALE(scale_ = LogNum(1.);)
    if (val.imag() == 0) {
...
ITensor::ITensor(QN q, IndexSet const &is) : is_(std::move(is))
{
    store_ = newITData<QDenseReal>(is, q);
}

Cplx ITensor::eltC() const
{
    if (inds().order() != 0) {
        PrintData(inds());
        Error(tinyformat::format(
            "Wrong number of IndexVals passed to elt/eltC (expected 0, got %d)",
            inds().order()));
    }
...
Cplx ITensor::eltC(std::vector<IndexVal> const &ivs) const
{
    ...
    if (size != size_t(inds().order())) {
        ...
        println("---------------------------------------------");
        Error(
            tinyformat::format("Wrong number of IndexVals passed to elt/eltC "
                               "(expected %d, got %d)",
                               inds().order(), size));
    }
...
void ITensor::set(std::vector<IndexVal> const &ivals, Cplx val)
{
    ...
    if (size != size_t(inds().order())) {
        ...
        println("---------------------------------------------");
        Error(tinyformat::format(
            "Wrong number of IndexVals passed to set (expected %d, got %d)",
            inds().order(), size));
    }
...
void ITensor::set(Cplx val)
{
    if (0 != size_t(inds().order())) {
        Error(tinyformat::format(
            "Wrong number of IndexVals passed to set (expected %d, got 0)",
            inds().order()));
    }
...
void ITensor::set(std::vector<int> const &ints, Cplx val)
{
    auto size = ints.size();
    if (size != size_t(inds().order())) {
        println("---------------------------------------------");
...
ITensor &ITensor::takeReal()
{
    doTask(TakeReal{}, store_);
    return *this;
}

ITensor &ITensor::takeImag()
{
    doTask(TakeImag{}, store_);
    return *this;
}

ITensor &ITensor::makeCplx()
{
    doTask(MakeCplx{}, store_);
    return *this;
}

ITensor &ITensor::randomize(Args const &args)
{
    ...
    if (cplx)
        this->generate(detail::quickranCplx);
    else
        this->generate(detail::quickran);
    return *this;
}
...
long nnz(ITensor const &A) { return doTask(NNZ{}, A.store()); }

ITensor &ITensor::fixBlockDeficient()
{
...
ITensor &ITensor::fill(Cplx z)
{
    if (!store_) {
        if (is_)
            detail::allocReal(*this);
        else
            Error("Can't fill default-constructed tensor");
    }
...
#ifdef USESCALE
void ITensor::scaleTo(scale_type const &newscale)
{
...
void ITensor::scaleTo(Real newscale) { scaleTo(LogNum{newscale}); }
#endif

void ITensor::swap(ITensor &other)
{
    is_.swap(other.is_);
...
bool hasQNs(ITensor const &T) { return hasQNs(inds(T)); }

long order(ITensor const &T) { return order(inds(T)); }

IndexSet const &inds(ITensor const &A) { return A.inds(); }

long minDim(ITensor const &T) { return minDim(inds(T)); }

long maxDim(ITensor const &T) { return maxDim(inds(T)); }

std::vector<IndexSet> inds(std::vector<ITensor> const &A)
{
    auto is = std::vector<IndexSet>(A.size());
    for (auto i : range(A.size())) is[i] = inds(A[i]);
    return is;
}
...
ITensor &ITensor::permute(IndexSet const &iset)
{
    auto &A = *this;
    auto Ais = A.inds();
    auto r = Ais.order();
    ...
    if (size_t(r) != size_t(iset.order())) {
        ...
        println("---------------------------------------------");
        Error(tinyformat::format(
            "Wrong number of Indexes passed to permute (expected %d, got %d)",
            r, iset.order()));
    }
...
ITensor &ITensor::replaceInds(IndexSet const &is1, IndexSet const &is2)
{
#ifdef DEBUG
    if (itensor::order(is1) != itensor::order(is2))
        Error("In replaceInds, must replace with equal number of Indices");
#endif
    auto &T = *this;
...
ITensor &ITensor::swapInds(IndexSet const &is1, IndexSet const &is2)
{
#ifdef DEBUG
    if (itensor::order(is1) != itensor::order(is2))
        Error("In swapInds, must swap equal numbers of Indices");
#endif
    auto &T = *this;
...
void ITensor::write(std::ostream &s) const
{
...
void ITensor::read(std::istream &s)
{
    itensor::read(s, is_);
    LogNum scale;
    ...
    } else if (type == StorageType::DenseReal) {
        store_ = readType<DenseReal>(s);
    } else if (type == StorageType::DenseCplx) {
        store_ = readType<DenseCplx>(s);
    } else if (type == StorageType::Combiner) {
        store_ = readType<Combiner>(s);
    } else if (type == StorageType::DiagReal) {
        store_ = readType<Diag<Real>>(s);
    } else if (type == StorageType::DiagCplx) {
        store_ = readType<Diag<Cplx>>(s);
    } else if (type == StorageType::QDenseReal) {
        store_ = readType<QDense<Real>>(s);
    } else if (type == StorageType::QDenseCplx) {
        store_ = readType<QDense<Cplx>>(s);
    } else if (type == StorageType::QDiagReal) {
        store_ = readType<QDiag<Real>>(s);
    } else if (type == StorageType::QDiagCplx) {
        store_ = readType<QDiag<Cplx>>(s);
    } else if (type == StorageType::QCombiner) {
        store_ = readType<QCombiner>(s);
    } else if (type == StorageType::ScalarReal) {
        store_ = readType<ScalarReal>(s);
    } else if (type == StorageType::ScalarCplx) {
        store_ = readType<ScalarCplx>(s);
    } else {
...
void h5_write(h5::group parent, std::string const &name, ITensor const &T)
{
...
void h5_read(h5::group parent, std::string const &name, ITensor &I)
{
    ...
    if (type != "ITensor")
        Error("Group does not contain ITensor data in HDF5 file");

    auto is = h5_read<IndexSet>(g, "inds");

    std::string store_name;
    if (g.has_subgroup("storage"))
        store_name = "storage";
    else if (g.has_subgroup("store"))
        store_name = "store";
    else
        error(
            "Expected ITensor HDF5 data to have group named \"storage\" or "
            "\"store\"");
    ...
    if (s_type == "Dense{Float64}")
        ...
        store = h5_readStore<DenseCplx>(g, store_name);
    else if (s_type == "BlockSparse{Float64}")
        store = h5_readStore<QDenseReal>(g, store_name);
    else if (s_type == "BlockSparse{ComplexF64}")
        store = h5_readStore<QDenseCplx>(g, store_name);
    else
        error(tinyformat::format(
            "Reading of ITensor storage type %s not yet supported", s_type));
...
void allocReal(ITensor &T)
{
    if (hasQNs(T))
        Error("Can't allocate quantum ITensor with undefined divergence");
    T.store() = newITData<DenseReal>(dim(inds(T)), 0);
}

void allocReal(ITensor &T, IntArray const &ints)
{
    if (not hasQNs(T)) {
        T.store() = newITData<DenseReal>(dim(inds(T)), 0);
    } else {
...
void checkArrows(IndexSet const &is1, IndexSet const &is2,
...
void checkSameDiv(ITensor const &T1, ITensor const &T2)
{
    if (hasQNs(T1) && hasQNs(T2)) {
        if (div(T1) != div(T2)) {
            Error(tinyformat::format(
                "div(T1)=%s must equal div(T2)=%s when adding T1+T2", div(T1),
                div(T2)));
        }
    }
}
...
ITensor &ITensor::operator*=(ITensor const &R)
{
    auto &L = *this;
    ...
    if (hqL && !hqR)
        ...
        Rdense = removeQNs(Rdense);

    auto C =
        doTask(Contract{L.inds(), Rdense.inds()}, L.store(), Rdense.store());
...
bool isDense(ITensor const &T) { return doTask(IsDense{}, T.store()); }
...
ITensor &ITensor::operator*=(Real r)
{
...
ITensor &ITensor::operator*=(Cplx z)
{
...
ITensor &ITensor::operator/=(ITensor const &R)
{
    ...
    auto C = doTask(NCProd{L.inds(), R.inds()}, L.store(), R.store());
...
Index directSumITensors(Index const &i, Index const &j, ITensor &D1,
                        ITensor &D2, Args const &args = Args::global())
{
                                        ...
                                        Args const &args)
{
    if (order(I) != order(J))
        Error(
            "In directSum(ITensor, ITensor, ...), must sum equal number of "
            "indices");
    auto AD = A;
    auto BD = B;
...
void daxpy(ITensor &L, ITensor const &R, Real alpha)
{
    if (L.order() != R.order())
        Error("ITensor::operator+=: different number of indices");
...
ITensor &ITensor::operator+=(ITensor const &R)
{
    ...
    if (!L || !L.store()) {
        return (L = R);
    }  // special case when this (L) is not initialized
...
detail::IndexValIter iterInds(ITensor const &T) { return iterInds(inds(T)); }
...
Arrow dir(ITensor const &T, Index const &i) { return dir(inds(T), i); }
...
bool isComplex(ITensor const &T) { return doTask(CheckComplex{}, T.store()); }

bool isReal(ITensor const &T) { return not isComplex(T); }

ITensor random(ITensor T, const Args &args)
{
    T.randomize(args);
    return T;
}
...
Real sumels(ITensor const &t)
{
    auto z = sumelsC(t);
    if (z.imag() != 0)
        Error("ITensor has non-zero imaginary part, use sumelsC");
    return z.real();
}
...
ostream &operator<<(ostream &s, ITensor const &t)
{
...
std::tuple<ITensor, Index> combiner(IndexSet const &inds, Args const &args)
{
    ...
    if (not hasQNs(inds)) {
        ...
        for (auto &I : inds) newind.nextIndex(std::move(I));
        return std::tuple<ITensor, Index>(ITensor(newind.build(), Combiner()),
                                          cind);
    } else if (hasQNs(inds)) {
        if (inds.empty()) Error("No indices passed to combiner");
        ...
        return std::tuple<ITensor, Index>(ITensor(newind.build(), std::move(C)),
                                          c);
    }
...
struct IsCombiner {
    ...
    bool operator()(Combiner const &d) { return true; }
};
...
ITensor randomITensor(IndexSet const &inds) { return random(ITensor(inds)); }
ITensor randomITensorC(IndexSet const &inds)
{
    return random(ITensor(inds), {"Complex=", true});
}

ITensor randomITensor(QN q, IndexSet const &is)
{
#ifdef DEBUG
    if (not hasQNs(is))
        Error(
            "Cannot use randomITensor(QN,...) to create non-QN-conserving "
            "ITensor");
#endif
    ITensor T;
    ...
    if (nnz(T) == 0)
        Error(
            "Requested QN for random ITensor resulted in zero allowed blocks "
            "(QN not satisfiable by any settings of the indices)");
    T.generate(detail::quickran);
    return T;
}

ITensor randomITensorC(QN q, IndexSet const &is)
{
#ifdef DEBUG
    if (not hasQNs(is))
        Error(
...
{
    if (not hasQNs(T))
        Error("div(ITensor) not defined for non QN conserving ITensor");
    if (!T) Error("div(ITensor) not defined for unallocated IQTensor");
    return doTask(CalcDiv{inds(T)}, T.store());
}

QN flux(ITensor const &T) { return div(T); }

IndexSet moveToFront(IndexSet const &isf, IndexSet const &is)
{
    auto rf = isf.order();
    auto r = is.order();
    ...
    if (rf >= r) {
        ...
        println("---------------------------------------------");
        Error(tinyformat::format(
            "Wrong number of indices passed to permute (expected < %d, got %d)",
            r, rf));
    }
...
IndexSet moveToBack(IndexSet const &isb, IndexSet const &is)
{
    auto rb = isb.order();
    auto r = is.order();
...
long rank(ITensor const &T)
{
    Global::warnDeprecated(
        "rank(ITensor) is deprecated in favor of order(ITensor)");
    return order(T);
}

void randomize(ITensor &T, Args const &args)
{
    Global::warnDeprecated(
...
{
    Global::warnDeprecated(
        "matrixTensor(Matrix,Index,Index) is deprecated in favor of "
        "matrixITensor(Matrix,Index,Index)");
...
{
    Global::warnDeprecated(
        "matrixTensor(CMatrix,Index,Index) is deprecated in favor of "
        "matrixITensor(CMatrix,Index,Index)");
    return matrixITensor(M, i1, i2);
}
