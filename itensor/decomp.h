...
//
Spectrum svd(ITensor const &AA, ITensor &U, ITensor &D, ITensor &V,
...
std::tuple<ITensor, ITensor, ITensor> svd(ITensor const &AA, Index const &i1,
                                          IndsArgs &&...indsargs);
...
// connecting A and B.
//
Spectrum factor(ITensor const &T, ITensor &A, ITensor &B,
                Args args = Args::global());
...
template <typename... IndsArgs>
std::tuple<ITensor, ITensor> factor(ITensor const &T, Index const &i1,
                                    IndsArgs &&...indsargs);
...
class NoOp
{
   public:
    ...
    ITensor deltaRho(ITensor const &rho, ITensor const &combine,
                     Direction dir) const
    {
...
Spectrum denmatDecomp(ITensor const &AA, ITensor &A, ITensor &B, Direction dir,
                      BigMatrixT const &PH, Args args = Args::global());
...
template <class BigMatrixT>
std::tuple<ITensor, ITensor> denmatDecomp(ITensor const &T, IndexSet const &Ais,
                                          Direction dir, BigMatrixT const &PH,
                                          Args args = Args::global())
{
    ITensor A(Ais), B;
...
//
Spectrum diagHermitian(ITensor const &M, ITensor &U, ITensor &D,
                       Args args = Args::global());

// TODO: allow specifying IndexSets, instead
// of assuming inds(T) = unionInds(inds,prime(inds))
std::tuple<ITensor, ITensor> diagHermitian(ITensor const &T,
                                           Args const &args = Args::global());

Spectrum diagPosSemiDef(ITensor const &M, ITensor &U, ITensor &D,
                        Args args = Args::global());

std::tuple<ITensor, ITensor> diagPosSemiDef(ITensor const &T,
                                            Args const &args = Args::global());

ITensor expHermitian(ITensor const &T, Cplx t = 1.);
...
//
//
void eigen(ITensor const &T, ITensor &V, ITensor &D,
           Args args = Args::global());

std::tuple<ITensor, ITensor> eigen(ITensor const &T,
                                   Args const &args = Args::global());
...
void qr(ITensor const &AA, ITensor &Q, ITensor &R, Args args = Args::global());
...
// Version that takes variable number of indices
template <typename... IndsArgs>
std::tuple<ITensor, ITensor> qr(ITensor const &T, Index const &i1,
                                IndsArgs &&...indsargs);
...
Spectrum svdOrd2(ITensor const &A, Index const &ui, Index const &vi, ITensor &U,
                 ITensor &D, ITensor &V, Args args = Args::global());

void qrOrd2(ITensor const &A, Index const &Qi, Index const &Ri, ITensor &Q,
            ITensor &R, Args args = Args::global());

Spectrum diag_hermitian(ITensor rho, ITensor &U, ITensor &D, Args const &args);
                      ...
                      BigMatrixT const &PH, Args args)
{
    ...
    if (args.getBool("TraceReIm", false)) rho = realPart(rho);
...
struct GetBlocks {
    ...
    GetBlocks(IndexSet const &is_, Index const &i1_, Index const &i2_) : is(is_)
    {
...
template <typename T>
std::vector<Ord2Block<T>> doTask(GetBlocks<T> const &G, QDense<T> const &d);
...
struct EigQN {
    ...
    bool operator<(EigQN const &o) const { return eig < o.eig; }
