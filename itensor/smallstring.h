...
struct SmallString {
   ...
   public:
    ...
    void set(size_t i, const char c)
    {
        CHECK_IND(i)
        name_[i] = c;
        return;
    ...
    explicit operator const int64_t() const
    {
        return reinterpret_cast<const int64_t &>(name_[0]);
    }

   private:
    void check_ind(size_t j) const
    {
        if (j >= size())
...
bool inline operator==(SmallString const &t1, SmallString const &t2)
{
...
bool inline operator==(SmallString const &t1, std::string s2)
{
    return operator==(t1, SmallString(s2));
}
bool inline operator==(std::string s1, SmallString const &t2)
{
...
bool inline operator==(SmallString const &t1, const char *s2)
{
    return operator==(t1, SmallString(s2));
}
bool inline operator==(const char *s1, SmallString const &t2)
{
    return operator==(SmallString(s1), t2);
}
bool inline operator!=(SmallString const &t1, const char *s2)
...
inline SmallString::SmallString() { name_.fill('\0'); }

inline SmallString::SmallString(const char *name)
{
    name_.fill('\0');
    auto len = std::min(std::strlen(name), size());
#ifdef DEBUG
    if (std::strlen(name) > size()) {
        std::cout << "Warning: SmallString name will be truncated to " << size()
                  << " chars" << std::endl;
    }
#endif
    for (size_t j = 0; j < len; ++j) {
        ...
        if (name[j] == ',')
            throw std::runtime_error(
                "SmallString cannot contain character ','");
#endif
        name_[j] = name[j];
    }
    assert(name_[size()] == '\0');
