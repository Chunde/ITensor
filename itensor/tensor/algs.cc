...
int hermitianDiag(int N, Real* Udata, Real* ddata)
{
    LAPACK_INT N_ = N;
    LAPACK_INT info = 0;
...
int hermitianDiag(int N, Cplx* Udata, Real* ddata)
{
    LAPACK_INT N_ = N;
    return zheev_wrapper(N_, Udata, ddata);
}

int QR(int M, int N, int Rrows, Real* Qdata, Real* Rdata)
{
...
int QR(int M, int N, int Rrows, Cplx* Qdata, Cplx* Rdata)
{
    LAPACK_INT M_ = M, N_ = N, Rrows_ = Rrows;
    LAPACK_INT info = 0;
...
int SVD_gesdd(int M, int N, Cplx* Adata, Cplx* Udata, Real* Ddata, Cplx* Vdata)
{
...
int SVD_gesdd(int M, int N, Real* Adata, Real* Udata, Real* Ddata, Real* Vdata)
{
    LAPACK_INT M_ = M, N_ = N;
    LAPACK_INT info = 0;
    char S = 'S';
...
int SVD_gesvd(int M, int N, Cplx* Adata, Cplx* Udata, Real* Ddata, Cplx* Vdata)
{
    LAPACK_INT M_ = M, N_ = N;
    LAPACK_INT info = 0;
    char S = 'S';
...
int SVD_gesvd(int M, int N, Real* Adata, Real* Udata, Real* Ddata, Real* Vdata)
{
    LAPACK_INT M_ = M, N_ = N;
    LAPACK_INT info = 0;
    char S = 'S';
...
//         throw std::runtime_error("diagHermitian: Input Matrix must be
//         square");
//         }
...
//         and real and imag part same size");
//         }
//
// #ifdef DEBUG
//     if(N < 1) throw std::runtime_error("diagHermitian: 0 dimensional
//     matrix"); if(!(nrows(Ure) == N && ncols(Ure) == N))
//         throw std::runtime_error("diagHermitian: Ure should have same dims as
//         M");
//     if(!(nrows(Uim) == N && ncols(Uim) == N))
//         throw std::runtime_error("diagHermitian: Uim should have same dims as
//         M");
//     if(d.size() != N)
//         throw std::runtime_error("diagHermitian: d size should be linear size
//         of M");
//     if(!isContiguous(Ure))
...
void diagGeneralRef(MatRefc<value_type> const& M, MatrixRef const& Rr,
                    MatrixRef const& Ri, MatrixRef const& Lr,
                    MatrixRef const& Li, VectorRef const& dr,
                    VectorRef const& di)
{
    auto N = ncols(M);
    ...
    struct Diag {
        LAPACK_INT static call(LAPACK_INT N, Real const* Mdata, Real* Ldata,
                               Real* Rdata, Real* drdata, Real* didata)
        {
            auto cl = (Ldata == nullptr) ? 'N' : 'V';
            return dgeev_wrapper(cl, 'V', N, Mdata, drdata, didata, Ldata,
                                 Rdata);
        }
        LAPACK_INT static call(LAPACK_INT N, Cplx const* Mdata, Cplx* Ldata,
                               Cplx* Rdata, Real* drdata, Real* didata)
        {
            auto d = std::vector<Cplx>(N);
            auto cl = (Ldata == nullptr) ? 'N' : 'V';
            auto info =
                zgeev_wrapper(cl, 'V', N, Mdata, d.data(), Ldata, Rdata);
    ...
    auto info =
        Diag::call(N, M.data(), L.data(), R.data(), dr.data(), di.data());
    if (info != 0) {
        // println("M = \n",M);
    ...
    struct Unpack {
        void static call(VectorRef di, MatrixRef Vr, MatrixRef Vi, MatrixRefc V)
        {
        ...
        void static call(VectorRef di, MatrixRef Vr, MatrixRef Vi,
                         CMatrixRefc V)
        {
            auto N = di.size();
    ...
    if (L) {
        ...
        //     printfln("z %d = %.4E",n,z);
        //     if(std::abs(z) <= 1E-16) Error("Ill conditioned or non-invertible
        //     matrix"); z = 1./z; column(Lr,n) &=
        //     column(Lr,n)*z.real()+column(Li,n)*z.imag(); column(Li,n) &=
        //     column(Lr,n)*z.imag()-column(Li,n)*z.real();
        //     }
    }
}
template void diagGeneralRef(MatRefc<Real> const& M, MatrixRef const& Rr,
                             MatrixRef const& Ri, MatrixRef const& Lr,
                             MatrixRef const& Li, VectorRef const& dr,
                             VectorRef const& di);
template void diagGeneralRef(MatRefc<Cplx> const& M, MatrixRef const& Rr,
                             MatrixRef const& Ri, MatrixRef const& Lr,
                             MatrixRef const& Li, VectorRef const& dr,
                             VectorRef const& di);
...
template <typename V>
void orthog(MatRef<V> M, size_t numpass)
{
...
void checksvd(MatrixRefc const& A, MatrixRefc const& U, VectorRefc const& D,
              MatrixRefc const& V)
{
    Matrix Ach(U);
    ...
    Ach = Ach * transpose(V);
    Ach -= A;
    printfln("relative error with sqrt in low level svd is %.5E",
             norm(Ach) / norm(A));
}

tuple<bool, size_t>  // == (done, start)
checkSVDDone(VectorRefc const& D, Real thresh)
{
    auto N = D.size();
    if (N <= 1 || thresh <= 0) {
    ...
    if (start >= (N - 1)) return make_tuple(true, start);

    return make_tuple(false, start);
}

template <typename T>
void SVDRefImpl(MatRefc<T> const& M, MatRef<T> const& U, VectorRef const& D,
                MatRef<T> const& V, const Args& args)
{
    auto Mr = nrows(M);
...
template <typename T>
void SVDRef(MatRefc<T> const& M, MatRef<T> const& U, VectorRef const& D,
            MatRef<T> const& V, const Args& args)
{
    ...
    if (Mr > Mc) {
        ...
        } else if (svdMethod == "automatic" or svdMethod == "gesdd" or
                   svdMethod == "gesvd") {
...
template void SVDRef(MatRefc<Real> const&, MatRef<Real> const&,
                     VectorRef const&, MatRef<Real> const&, const Args&);
template void SVDRef(MatRefc<Cplx> const&, MatRef<Cplx> const&,
                     VectorRef const&, MatRef<Cplx> const&, const Args&);
...
//         throw std::runtime_error("SVD (ref version), Mim must have same dims
//         as Mre");
...
{
int expPade(MatRef<Real> const& F, int N, int ideg)
{
    LAPACK_INT info = 0;
    ...
    //
    auto ns = dlange_wrapper(
        'I', N, N,
        F.data());  // infinite norm of the matrix to be exponentiated
#ifdef DEBUG
    ...
    for (int k = 1; k <= ideg; ++k) {
        coef[k] = coef[k - 1] *
                  ((double)(ideg + 1 - k) / (double)(k * (2 * ideg + 1 - k)));
    }
...
int expPade(MatRef<Cplx> const& F, int N, int ideg)
{
    LAPACK_INT info = 0;
