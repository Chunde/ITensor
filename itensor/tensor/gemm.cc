...
struct dgemmTask {
    ...
    size_t Apart = 0ul, Bpart = 0ul, Cpart = 0ul;
    Real alpha = 0., beta = 0.;

    dgemmTask(size_t Ap, size_t Bp, size_t Cp, Real a, Real b)
        : Apart(Ap), Bpart(Bp), Cpart(Cp), alpha(a), beta(b)
    {
        if (b != 0.0) copyFromC = true;
    }

    dgemmTask(size_t Ap, size_t Bp, size_t Cp, Real a)
        : Apart(Ap), Bpart(Bp), Cpart(Cp), alpha(a), beta(1.0)
    {
    }

    dgemmTask(size_t Cp) : copyToC(true), Cpart(Cp) {}
};

void cplxToRealBuf(SAFE_PTR_OF(const Real) C, size_t imagPart,
                   SAFE_PTR_OF(Real) Rbegin, SAFE_PTR_OF(Real) Rend)
{
    C += imagPart;
...
void realBufToCplx(SAFE_PTR_OF(Real) Rbegin, SAFE_PTR_OF(Real) Rend,
                   SAFE_PTR_OF(Real) C, size_t imagPart)
{
    C += imagPart;
    for (auto r = Rbegin; r != Rend; ++r, C += 2) {
        *C = *r;
...
void gemm_emulator(MatRefc<VA> A, MatRefc<VB> B, MatRef<Cplx> C, Real alpha,
                   Real beta, std::array<const dgemmTask, NTask> const& tasks)
{
    ...
    for (auto t : tasks) {
        if (t.copyToC) {
            ...
            if (isReal(A)) {
                gemm_wrapper(isTransposed(A), isTransposed(B), nrows(A),
                             ncols(B), ncols(A), t.alpha,
                             SAFE_PTR_GET(Ard, A.size()),
                             SAFE_PTR_GET(bb, B.size()), t.beta,
                             SAFE_PTR_GET(cb, C.size()));
            } else if (isReal(B)) {
                gemm_wrapper(isTransposed(A), isTransposed(B), nrows(A),
                             ncols(B), ncols(A), t.alpha,
                             SAFE_PTR_GET(ab, A.size()),
                             SAFE_PTR_GET(Brd, B.size()), t.beta,
                             SAFE_PTR_GET(cb, C.size()));
...
void gemm_impl(MatRefc<Cplx> A, MatRefc<Cplx> B, MatRef<Cplx> C, Real alpha,
               Real beta)
{
#ifdef ITENSOR_USE_ZGEMM
    gemm_wrapper(isTransposed(A), isTransposed(B), nrows(A), ncols(B), ncols(A),
                 alpha, A.data(), B.data(), beta, C.data());
#else  // emulate zgemm by calling dgemm four times
    std::array<const dgemmTask, 6> tasks = {
        {dgemmTask(0, 0, 0, +alpha, beta), dgemmTask(1, 1, 0, -alpha),
         dgemmTask(0), dgemmTask(1, 0, 1, +alpha, beta),
         dgemmTask(0, 1, 1, +alpha), dgemmTask(1)}};
    gemm_emulator(A, B, C, alpha, beta, tasks);
#endif
}

void gemm_impl(MatRefc<Real> A, MatRefc<Cplx> B, MatRef<Cplx> C, Real alpha,
               Real beta)
{
    std::array<const dgemmTask, 4> tasks = {
        {dgemmTask(0, 0, 0, +alpha, beta), dgemmTask(0),
         dgemmTask(0, 1, 1, +alpha, beta), dgemmTask(1)}};
    gemm_emulator(A, B, C, alpha, beta, tasks);
}

void gemm_impl(MatRefc<Cplx> A, MatRefc<Real> B, MatRef<Cplx> C, Real alpha,
               Real beta)
{
    std::array<const dgemmTask, 4> tasks = {
        {dgemmTask(0, 0, 0, +alpha, beta), dgemmTask(0),
         dgemmTask(1, 0, 1, +alpha, beta), dgemmTask(1)}};
    gemm_emulator(A, B, C, alpha, beta, tasks);
}

void gemm_impl(MatRefc<Real> A, MatRefc<Real> B, MatRef<Real> C, Real alpha,
               Real beta)
{
    // call dgemm directly
    gemm_wrapper(isTransposed(A), isTransposed(B), nrows(A), ncols(B), ncols(A),
          ...
          Real alpha, Real beta)
{
    ...
    if (!(isContiguous(A) && isContiguous(B) && isContiguous(C)))
        throw std::runtime_error(
            "multiplication of non-contiguous MatrixRefs not currently "
            "supported");
#endif
