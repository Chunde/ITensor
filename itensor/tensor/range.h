...
class RangeT : public RangeType
{
   ...
   public:
    ...
    explicit RangeT(index_type i0, Inds&&... rest)
    {
    ...
    explicit RangeT(storage_type&& store) : store_(std::move(store)) {}

    explicit RangeT(std::vector<index_type> const& inds) { init(inds); }

    template <size_t N>
    explicit RangeT(std::array<index_type, N> const& inds)
    {
        init(inds);
    }
    ...
    size_t constexpr start() const { return start_; }

    size_type extent(size_type i) const
    {
        return static_cast<size_type>(store_[i].ind);
    }

    size_type stride(size_type i) const { return store_[i].str; }

    index_type const& index(size_type i) const { return store_[i].ind; }

    index_type& index(size_type i) { return store_[i].ind; }

    size_type order() const { return store_.size(); }

    // Deprecated
    size_type r() const { return this->order(); }

    value_type const& operator[](size_type i) const { return store_[i]; }

    value_type& operator[](size_type i) { return store_[i]; }

    value_type const& at(size_type i) const { return store_.at(i); }

    value_type& at(size_type i) { return store_.at(i); }

    size_type size() const { return store_.size(); }

    value_type const& front() const { return store_.front(); }

    value_type const& back() const { return store_.back(); }

    void clear() { return store_.clear(); }

    bool empty() const { return store_.empty(); }

    value_type* data() { return store_.data(); }

    value_type const* data() const { return store_.data(); }

    storage_type const& store() const { return store_; }

    void swap(RangeT& other) { store_.swap(other.store_); }

    template <typename Container>
    void init(Container const& c);

    void resize(size_type nsize) { store_.resize(nsize); }

    iterator begin() const { return iterator(*this); }

    iterator end() const { return iterator::makeEnd(*this); }
...
template <typename range_type, typename Indexable,
          typename storage_type = typename range_type::storage_type>
auto initImpl(stdx::choice<2>, Indexable&& v, storage_type& store_)
...
template <typename range_type, typename Iterable,
          typename storage_type = typename range_type::storage_type>
auto initImpl(stdx::choice<1>, Iterable&& v, storage_type& store_)
...
template <typename Container>
void RangeT<index_type, start>::init(Container const& c)
{
    detail::initImpl<RangeT>(stdx::select_overload{}, c, store_);
}

template <typename index_type, size_t start>
auto order(RangeT<index_type, start> const& R) -> decltype(R.size())
{
    return R.size();
}
...
// template<typename index_type, size_t start>
// auto
// order(RangeT<index_type,start> const& R) -> decltype(R.size()) { return
// R.size(); }

template <typename index_type, size_t start>
std::ostream& operator<<(std::ostream& s, RangeT<index_type, start> const& r)
{
    s << "exts: ";
...
void write(std::ostream& s, RangeT<I, S> const& r)
{
    itensor::write(s, r.store());
}

template <typename I, size_t S>
void read(std::istream& s, RangeT<I, S>& r)
{
...
class RangeBuilderT
{
   ...
   public:
    RangeBuilderT(size_type size) : store_(size) {}
    ...
    void resize(size_type newsize) { store_.resize(newsize); }

    size_type const& extent(size_type j) const
    {
        return static_cast<size_type>(store_[j].ind);
    }

    size_type stride(size_type j) const { return store_[j].str; }

    storage_type& store() { return store_; }

    storage_type const& store() const { return store_; }

    void nextIndex(index_type const& ind)
    {
        store_[n_].ind = ind;
        ++n_;
    }

    void nextIndex(index_type&& ind)
    {
        store_[n_].ind = std::move(ind);
        ++n_;
    }

    void nextIndStr(index_type const& ind, size_type str)
    {
        store_[n_].ind = ind;
        store_[n_].str = str;
    ...
    void setIndex(size_type j, index_type const& i) { store_[j].ind = i; }

    void setStride(size_type j, size_type s)
    {
        store_[j].str = s;
        auto_compute_strides_ = false;
    }

    void setIndStr(size_type j, index_type const& ind, size_type str)
    {
        store_[j].ind = ind;
        store_[j].str = str;
        auto_compute_strides_ = false;
    ...
    void sortByIndex()
    {
        auto comp = [](value_type const& i1, value_type const& i2) {
...
auto offsetImpl(stdx::choice<1>, Range_ const& r, Iterable const& inds)
...
template <typename Range_, typename Iterable>
auto offsetImpl(stdx::choice<2>, Range_ const& r, Iterable const& inds)
    // Constrain this template to only work for inds that have .operator[] and
    // .size()
    -> stdx::if_compiles_return<decltype(r.extent(0)), decltype(inds[0]),
                                decltype(inds.size())>
...
template <typename Range_, typename Iterable>
auto offsetImpl(stdx::choice<3>, Range_ const& r,
                Iterable const& I) -> decltype(I.offset())
{
...
auto offset(Range_ const& r, Iterable const& inds) -> typename Range_::size_type
{
    return detail::offsetImpl(stdx::select_overload{}, r, inds);
}

template <typename Range_, class = stdx::require<isRange<Range_>>,
          typename... Inds>
auto offset(Range_ const& r, size_t i1, Inds... inds) -> decltype(r.stride(0))
{
#ifdef DEBUG
    if (1 + sizeof...(inds) != order(r))
        throw std::runtime_error(tinyformat::format(
            "Wrong number of indices passed to TenRef (expected %d got %d)",
            order(r), 1 + sizeof...(inds)));
#endif
...
template <typename I, size_t S>
auto dim(RangeT<I, S> const& R) -> decltype(R.extent(0))
{
...
template <typename I, size_t S>
Range normalRange(RangeT<I, S> const& R)
{
...
//
template <typename I, size_t S>
bool isContiguous(RangeT<I, S> const& R)
{
    using size_type = decltype(R.size());
    size_type max_offset = 0, dim = 1;
    for (decltype(R.order()) n = 0; n < R.order(); ++n) {
