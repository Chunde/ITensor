...
template <typename T>
bool inline constexpr isReal()
{
    return std::is_same<stdx::decay_t<T>, Real>::value;
}

template <typename T>
bool inline constexpr isCplx()
{
    return std::is_same<stdx::decay_t<T>, Cplx>::value;
}
...
class DataRange
{
   ...
   public:
    ...
    DataRange(pointer pdat, size_t size) : pdata_(pdat), size_(size) {}

    DataRange(pointer pdat, size_t offset, size_t size)
        : pdata_(pdat + offset), size_(size - offset)
    {
    }

    explicit operator bool() const { return static_cast<bool>(pdata_); }
    ...
    reference operator[](size_t n)
    {
        ...
        if (n >= size_) {
            ...
            if (size_ == 0)
                ...
                std::cout << "max offset (size-1) = " << (size_ - 1ul)
                          << std::endl;
    ...
    pointer data() const { return pdata_; }

    size_t size() const { return size_; }

    DataRange operator+(size_t off) const
    {
        auto res = DataRange{};
    ...
    DataRange<value_type> cast_away_const() const
    {
...
template <typename T>
std::ostream& operator<<(std::ostream& s, DataRange<T> const& d)
{
    s << "DataRange" << "\n";
    s << "Size: " << d.size() << "\n";
    for (auto i : range(d.size())) s << i << " " << d[i] << "\n";
    return s;
}

template <typename T>
DataRange<T> makeDataRange(T* p, size_t size)
{
    return DataRange<T>(p, size);
}

template <typename T>
DataRange<const T> makeDataRange(T const* p, size_t size)
{
    return DataRange<const T>(p, size);
}

template <typename T>
DataRange<T> makeDataRange(T* p, size_t offset, size_t size)
{
    return DataRange<T>(p, offset, size);
}

template <typename T>
DataRange<const T> makeDataRange(T const* p, size_t offset, size_t size)
{
    return DataRange<const T>(p, offset, size);
}

template <typename T>
DataRange<T> sliceData(DataRange<T> d, size_t begin, size_t end)
{
    auto size = end - begin;
#ifdef DEBUG
...
std::ostream& operator<<(std::ostream& s, InfArray<T, N> const& v)
{
    if (v.empty()) return s;
    decltype(v.size()) j = 0;
    for (; 1 + j < v.size(); ++j) {
