...
class RangeIter
{
   ...
   public:
    RangeIter() : prange_(nullptr), off_(0) {}

    RangeIter(range_type const& R)
        : prange_(&R), off_(0), ind_(R.order(), R.start())
    {
    }

    RangeIter const& operator*() const { return *this; }

    size_type const& operator[](size_type n) const { return ind_[n]; }

    size_type size() const { return ind_.size(); }

    offset_type offset() const { return off_; }

    ind_type index() const { return ind_; }
    ...
    bool operator==(RangeIter const& o) const
    {
        ...
        if (prange_ != o.prange_)
            Error(
                "Can't compare RangeIter created from different range objects");
#endif
        return off_ == o.off_;
    }

    bool operator!=(RangeIter const& o) const { return !operator==(o); }

    const_ind_iterator begin() const { return ind_.begin(); }

    const_ind_iterator end() const { return ind_.end(); }
    ...
    void increment()
    {
...
#ifdef DEBUG
        if (range().order() == 0)
            Error("Can't increment RangeIter made from order 0 range");
#endif
        auto r = range().order();
        ind_[0] += 1;
        off_ += range().stride(0);
        if (rextent(ind_[0] - range().start()) == range().extent(0)) {
            for (decltype(r) n = 1; n < r; ++n) {
                ...
                ind_[n] += 1;
                off_ += range().stride(n);
                if (rextent(ind_[n] - range().start()) < range().extent(n))
                    return;
            }
   ...
   public:
    ...
    RangeIter static makeEnd(range_type const& R)
    {
        RangeIter end;
...
template <typename R>
std::ostream& operator<<(std::ostream& s, RangeIter<R> const& it)
{
