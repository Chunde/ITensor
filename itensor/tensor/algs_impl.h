...
void copyNegElts(Iter m, MatRef<V> const& U)
{
...
void copyNegElts(Iter mre, Iter mim, std::vector<Cplx>& Mc)
{
    for (auto& z : Mc) {
        realRef(z) = -(*mre);
        imagRef(z) = -(*mim);
...
inline Real conjIfCplx(Real n) { return n; }

inline Cplx conjIfCplx(Cplx n) { return std::conj(n); }
...
template <class MatM, class MatU, class Vecd, class>
void diagHermitian(MatM&& M, MatU&& U, Vecd&& d)
{
    using Mval = typename stdx::decay_t<MatM>::value_type;
    using Uval = typename stdx::decay_t<MatU>::value_type;
    static_assert((isReal<Mval>() && isReal<Uval>()) ||
                      (isCplx<Mval>() && isCplx<Uval>()),
    ...
    if (isContiguous(M))
        detail::copyNegElts(M.data(), makeRef(U));
    else
        detail::copyNegElts(M.cbegin(), makeRef(U));
...
template <typename V>
void diagGeneralRef(MatRefc<V> const& M, MatrixRef const& Rr,
...
void eigen(MatM&& M, MatV&& Vr, MatV&& Vi, Vecd&& dr, Vecd&& di)
{
    auto N = ncols(M);
    resize(Vr, N, N);
    resize(Vi, N, N);
    ...
    auto Lr = MatrixRef{};
    auto Li = MatrixRef{};
    diagGeneralRef(makeRef(M), makeRef(Vr), makeRef(Vi), Lr, Li, makeRef(dr),
                   makeRef(di));
}

template <class MatM, class MatV, class Vecd, class>
void eigDecomp(MatM&& M, MatV&& Lr, MatV&& Li, Vecd&& dr, Vecd&& di, MatV&& Rr,
               MatV&& Ri)
{
    auto N = ncols(M);
    resize(Lr, N, N);
    ...
    resize(dr, N);
    resize(di, N);
    diagGeneralRef(makeRef(M), makeRef(Rr), makeRef(Ri), makeRef(Lr),
                   makeRef(Li), makeRef(dr), makeRef(di));
}
...
void SVD(MatM&& M, MatU&& U, VecD&& D, MatV&& V, const Args& args)
{
...
template <class MatM, class ScalarT, class>
Mat<common_type<val_type<MatM>, ScalarT>> expHermitian(MatM&& M, ScalarT t)
{
    // using Mval = typename stdx::decay_t<MatM>::value_type;
...
template <class MatM, class ScalarT, class>
Mat<common_type<val_type<MatM>, ScalarT>> expMatrix(MatM&& M, ScalarT t,
                                                    int ideg)
{
    ...
    if (info != 0) throw std::runtime_error("exp failed");

    return tM;
}

template <class MatA, class MatQ, class MatR>
void QR(MatA&& A, MatQ&& Q, MatR&& R, const Args& args)
{
    ...
    static_assert((isReal<Aval>() && isReal<Qval>()) ||
                      (isCplx<Aval>() && isCplx<Qval>()),
    ...
    if (N > M) {
        println(
            "Warning: QR for ncol > nrow may require pivoting to be "
            "numerically stable.");
        resize(Q, M, N);
    } else {
        Rrows = complete ? M : N;
...
#ifdef DEBUG
    if (!isContiguous(Q)) throw std::runtime_error("QR: Q must be contiguous");
    if (!isContiguous(R)) throw std::runtime_error("QR: R must be contiguous");
#endif
    for (int i = 0; i < M; ++i) {
        for (int j = 0; j < N; ++j) Q(i, j) = A(i, j);
        for (unsigned int j = N; j < ncols(Q); ++j) Q(i, j) = 0;
    }
...
template <typename T>
void SVDRefLAPACK(MatRefc<T> const& M, MatRef<T> const& U, VectorRef const& D,
                  MatRef<T> const& V, const Args& args)
{
    int Mr = nrows(M), Mc = ncols(M);
    ...
    // we have to pass M**T stored in column-major. Copy of inpput matrix has to
    // be done in any case, since input matrix is destroyed in ?gesdd
    if (isTransposed(M)) {
    ...
    if (svdMethod == "automatic") {
        info =
            detail::SVD_gesdd(Mr, Mc, cpA.data(), U.data(), D.data(), V.data());

        // if gesdd failed, try gesvd; need to restore cpA data since gesdd
        // destroyed it
        if (info != 0) {
            if (isTransposed(M)) {
                for (unsigned int i = 0; i < cpA.size(); i++, pA++)
            ...
            info = detail::SVD_gesvd(Mr, Mc, cpA.data(), U.data(), D.data(),
                                     V.data());
        }
        ...
        info =
            detail::SVD_gesvd(Mr, Mc, cpA.data(), U.data(), D.data(), V.data());
    ...
    for (unsigned int i = 0; i < vt.size(); i++, pV++)
        *pV = detail::conjIfCplx(vt[(i % Mc) * l + i / Mc]);
}
