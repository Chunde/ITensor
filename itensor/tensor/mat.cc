...
void apply(Iter1 it1, Iter1 end1, Iter2 it2, Func const& f)
{
...
void apply(MatRef<V> const& M, Iter it, Func const& f)
{
    for (auto& el : M) {
        f(el, *it);
        ++it;
...
void operator&=(MatrixRef const& a, MatrixRefc const& b)
{
    ...
    if (a.range() == b.range() && isContiguous(b)) {
        ...
        auto pae =
            MAKE_SAFE_PTR_OFFSET(a.data(), dim(a.range()), a.store().size());
...
template <typename V>
void multReal(MatRef<V> const& M, Real fac)
{
    if (isContiguous(M)) {
#ifdef DEBUG
        if (M.size() > std::numeric_limits<unsigned long>::max())
            throw std::runtime_error(
                "MatrixRef overflow of size beyond long unsigned int range");
#endif
        auto d = realData(M);
...
void multCplx(CMatrixRef const& M, Cplx fac)
{
    for (auto& el : M) el *= fac;
}

void operator*=(MatrixRef const& M, Real fac) { multReal(M, fac); }
void operator*=(CMatrixRef const& M, Real fac) { multReal(M, fac); }

void operator*=(CMatrixRef const& M, Cplx fac) { multCplx(M, fac); }

template <typename V>
void divReal(MatRef<V> const& M, Real fac)
{
    if (isContiguous(M)) {
        auto d = realData(M);
...
void operator/=(MatrixRef const& M, Real fac) { divReal(M, fac); }
void operator/=(CMatrixRef const& M, Real fac) { divReal(M, fac); }

template <typename MatT1, typename MatT2>
void call_daxpy(MatT1& A, MatT2 const& B, Real alpha_)
{
    ...
    if (Ad.size() != Bd.size())
        throw std::runtime_error(
            "mismatched sizes in MatrixRef/Matrix call_daxpy");
    if (Ad.size() > std::numeric_limits<unsigned long>::max())
        throw std::runtime_error(
            "overflow of size beyond long unsigned int range");
#endif
    daxpy_wrapper(Ad.size(), alpha, Bd.data(), inc, Ad.data(), inc);
}

template <typename V>
void add(MatRef<V> const& A, MatRefc<V> const& B)
{
#ifdef DEBUG
...
void operator+=(MatrixRef const& A, MatrixRefc const& B) { add(A, B); }
void operator+=(MatrixRef const& A, Matrix&& B) { add(A, makeRef(B)); }
void operator+=(CMatrixRef const& A, CMatrixRefc const& B) { add(A, B); }
void operator+=(CMatrixRef const& A, CMatrix&& B) { add(A, makeRef(B)); }

template <typename V>
void subtract(MatRef<V> const& A, MatRefc<V> const& B)
{
#ifdef DEBUG
    if (!(ncols(A) == ncols(B) && nrows(A) == nrows(B)))
        throw std::runtime_error("MatrixRef +=: mismatched sizes");
...
void operator-=(MatrixRef const& A, MatrixRefc const& B) { subtract(A, B); }
void operator-=(MatrixRef const& A, Matrix&& B) { subtract(A, makeRef(B)); }
void operator-=(CMatrixRef const& A, CMatrixRefc const& B) { subtract(A, B); }
void operator-=(CMatrixRef const& A, CMatrix&& B) { subtract(A, makeRef(B)); }

template <typename V>
void printMatrix(std::ostream& s, MatRefc<V> const& M)
{
...
void multAdd(MatrixRefc A, MatrixRefc B, MatrixRef C) { gemm(A, B, C, 1., 1.); }

template <typename V>
void call_gemv(MatRefc<V> const& M, VecRefc<V> const& x, VecRef<V>& y,
               Real alpha, Real beta, bool fromleft)
{
#ifdef DEBUG
    if (!isContiguous(M))
        throw std::runtime_error(
            "multiplication of non-contiguous matrixref by vector not "
            "currently supported");
#endif
    auto trans = fromleft;
    LAPACK_INT m = nrows(M), n = ncols(M);
    if (isTransposed(M)) {
        trans = !fromleft;
        m = ncols(M);
        n = nrows(M);
    }
    gemv_wrapper(trans, alpha, beta, m, n, M.data(), x.data(), stride(x),
                 y.data(), stride(y));
}
template void call_gemv(MatRefc<Real> const&, VecRefc<Real> const&,
                        VecRef<Real>&, Real, Real, bool);
template void call_gemv(MatRefc<Cplx> const&, VecRefc<Cplx> const&,
                        VecRef<Cplx>&, Real, Real, bool);

template <typename V>
void mult(MatRefc<V> M, VecRefc<V> x, VecRef<V> y, bool fromleft)
{
#ifdef DEBUG
    ...
    if (fromleft ? ncols(M) != y.size() : nrows(M) != y.size())
        throw std::runtime_error(
            "matrix vector mult: wrong size for result (y) vec");
#endif
...
Vec<common_type<VM, Vx>> mult(MatRefc<VM> M, VecRefc<Vx> x, bool fromleft)
{
...
template <typename V>
void multAdd(MatRefc<V> M, VecRefc<V> x, VecRef<V> y, bool fromleft)
{
#ifdef DEBUG
    if (fromleft ? nrows(M) != x.size() : ncols(M) != x.size())
...
template <typename V>
void multSub(MatRefc<V> M, VecRefc<V> x, VecRef<V> y, bool fromleft)
{
#ifdef DEBUG
    if (fromleft ? nrows(M) != x.size() : ncols(M) != x.size())
