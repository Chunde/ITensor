...
struct Permutation {
   ...
   public:
    ...
    void setFromTo(size_type from, size_type to) { store_[from] = to; }

    long dest(size_type j) const { return store_[j]; }

    long& operator[](size_type j) { return store_[j]; }

    long operator[](size_type j) const { return store_[j]; }

    storage const& store() const { return store_; }
...
inline Permutation::Permutation(size_type size) : store_(size)
{
    for (size_type n = 0; n < size; ++n) store_[n] = n;
}

Permutation inline inverse(Permutation const& P)
{
...
bool inline isTrivial(Permutation const& P)
{
    for (decltype(P.size()) n = 0; n < P.size(); ++n) {
...
void permute(Permutation const& P, Container const& from, Container& to)
{
...
void calcPerm(Set1 const& s1, Set2 const& s2, Permutation& P)
{
...
template <typename Set1, typename Set2>
Permutation calcPerm(Set1 const& s1, Set2 const& s2)
{
...
inline std::ostream& operator<<(std::ostream& s, Permutation const& P)
{
