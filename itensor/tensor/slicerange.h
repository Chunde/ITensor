...
template <typename Range_, typename Perm_, typename RetType = Range>
RetType permuteRange(Range_&& R, Perm_ const& P)
{
...
template <typename RetType, typename Range_, typename Perm_>
RetType permuteRangeTo(Range_&& R, Perm_ const& P)
{
    return permuteRange<Range_, Perm_, RetType>(std::forward<Range_>(R), P);
}

template <typename Range_, typename Perm_>
Range permuteExtents(Range_&& R, Perm_ const& P)
{
...
Range inline groupIndsRange(Range const& R, size_t istart, size_t iend)
{
