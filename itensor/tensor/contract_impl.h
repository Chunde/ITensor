...
class PtrInd
{
   ...
   public:
    PtrInd(size_type size) : ptrs_(size, nullptr) {}

    size_type size() const { return ptrs_.size(); }

    void set(size_type n, pointer_type p) { ptrs_[n] = p; }

    value_type const& operator[](size_type n) const { return *(ptrs_[n]); }
};
...
void ncprod_impl(TenRefc<R, VA> A, Labels const& ai, TenRefc<R, VB> B,
                 Labels const& bi, TenRef<R, common_type<VA, VB>> C,
                 Labels const& ci)
{
    auto rA = order(A), rB = order(B), rC = order(C);
