...
//
// TODO: replace unordered_map with a simpler container (small_map? or jump
...
void printv(const vector<T>& t, const F& f)
{
    print("{ ");
    for (const auto& i : t) {
        f(i);
...
void printv(const std::array<T, size>& t)
{
    print("{ ");
    for (const auto& i : t) print(i, " ");
    println("}");
...
template <typename T>
void printv(const autovector<T>& t)
{
    print("{ ");
...
struct CProps {
    Labels ai, bi, ci;
    int nactiveA = 0, nactiveB = 0, nactiveC = 0;

   private:
    Labels AtoB_, AtoC_, BtoC_;
    bool permuteA_ = false, permuteB_ = false, permuteC_ = false;

   public:
    using Dimension = size_t;
    Dimension dleft = 1, dmid = 1, dright = 1;
    int ncont = 0, Acstart, Bcstart, Austart, Bustart;
    Permutation PA, PB, PC;
    bool ctrans = false;
    Range newArange, newBrange, newCrange;

    CProps(Labels const& ai_, Labels const& bi_, Labels const& ci_)
        : ai(ai_),
          bi(bi_),
          ci(ci_),
          Acstart(ai_.size()),
    ...
    bool contractedA(int i) const { return AtoC_[i] < 0; }
    bool contractedB(int i) const { return BtoC_[i] < 0; }
    int AtoB(int i) const { return AtoB_[i]; }
    int AtoC(int i) const { return AtoC_[i]; }
    int BtoC(int i) const { return BtoC_[i]; }
    bool permuteA() const { return permuteA_; }
    bool permuteB() const { return permuteB_; }
    bool permuteC() const { return permuteC_; }
    bool Atrans() const { return contractedA(0); }
    bool Btrans() const { return !contractedB(0); }
    bool Ctrans() const { return ctrans; }

   private:
    bool checkACsameord() const
    {
   ...
   public:
    ...
    void compute(TenRefc<R, V1> A, TenRefc<R, V2> B,
                 TenRefc<R, common_type<V1, V2>> C)
    {
        ...
        int ra = ai.size(), rb = bi.size(), rc = ci.size();
        ...
        if (permuteC_ && !(permuteA_ && permuteB_)) {
            ...
            if (extra_pABcost < pCcost) {
                ...
                // if(Global::debug1()) printfln("Permuting %s %s (%d) instead
                // of permuting C
                // (%d)",permuteA_?"":"A",permuteB_?"":"B",extra_pABcost,pCcost);
                permuteA_ = true;
                permuteB_ = true;
                permuteC_ = false;
            }
        ...
        // if(Global::debug1()) printfln("At line %d: permute A,B,C =
        // %s,%s,%s",__LINE__,permuteA_,permuteB_,permuteC_);
        ...
        if (permuteC_) {
            ...
            if (!permuteA_) {
                for (decltype(ra) i = 0; i < ra; ++i)
                    if (!contractedA(i)) Rb.nextIndex(A.extent(i));
            } else {
                for (decltype(ra) i = 0; i < ra; ++i)
                    if (!contractedA(i)) Rb.nextIndex(newArange.extent(i));
            }
            if (!permuteB_) {
                for (decltype(rb) j = 0; j < rb; ++j)
                    if (!contractedB(j)) Rb.nextIndex(B.extent(j));
            } else {
                for (decltype(rb) j = 0; j < rb; ++j)
                    if (!contractedB(j)) Rb.nextIndex(newBrange.extent(j));
            }
            newCrange = Rb.build();
        }
    }
    ...
    void computePerms()
    {
        ...
        int na = ai.size(), nb = bi.size(), nc = ci.size();
    ...
    void computeNactive()
    {
        ...
        // An active index appears as one of the first two indices in each of
        // the two tensor in which it appears.  More specifically: the first
        // index of a tensor is active if its pair is also a first index, or if
        // its pair is a second index and that tensor's first index is active.

        // indval is the number of times an index appears among the first two of
        // each tensor Good indices have indval == 2, bad ones have indval == 1
        ...
        for (int elim = 1; elim <= 3;
             ++elim)  // bad guys at position 0 kill the index at 1
        {
...
struct ABoffC {
    ...
    ABoffC(MatrixRefc& mA_, MatrixRefc& mB_, MatrixRef& mC_, int offC_)
        : mA(mA_), mB(mB_), mC(mC_), offC(offC_)
    {
    }

    void execute() const { multAdd(mA, mB, mC); }
};
...
class CABqueue
{
   ...
   public:
    ...
    void addtask(MatrixRefc& mA, MatrixRefc& mB, MatrixRef& mC, int offC)
    {
    ...
    void run(int numthread)
    {
        ...
        for (auto& t : subtask) {
            ...
            std::move(st_tasks.begin(), st_tasks.end(),
                      std::inserter(tt, tt.end()));
            ss = (ss + 1) % numthread;
        }
        ...
        for (size_t i = 0; i < futs.size(); ++i) {
            ...
            futs[i] = std::async(std::launch::async, [&tt]() {
                for (const auto& task : tt) task.execute();
            });
        }
...
void contract(CProps const& p, TenRefc<range_t, VA> A, TenRefc<range_t, VB> B,
              TenRef<range_t, common_type<VA, VB>> C, Real alpha = 1.,
              Real beta = 0.)
{
    ...
    if (p.permuteA()) {
        ...
        auto tref =
            makeTenRef(SAFE_PTR_GET(aptr, Apsize), Apsize, &p.newArange);
        tref &= permute(A, p.PA);
    ...
    if (p.permuteB()) {
        ...
        auto tref =
            makeTenRef(SAFE_PTR_GET(bptr, Bpsize), Bpsize, &p.newBrange);
        tref &= permute(B, p.PB);
...
#ifdef DEBUG
        if (isTrivial(p.PC))
            Error("Calling permute in contract with a trivial permutation");
#endif
        C &= permute(newC, p.PC);
    }
}
...
void contractScalar(T1 a, TenRefc<R, T2> B, Labels const& bi,
...
void contract(TenRefc<RangeT, VA> A, Labels const& ai, TenRefc<RangeT, VB> B,
              Labels const& bi, TenRef<RangeT, common_type<VA, VB>> C,
              Labels const& ci, Real alpha, Real beta)
{
    if (ai.empty()) {
...
template void contract(TenRefc<Range, Real>, Labels const&,
                       TenRefc<Range, Real>, Labels const&, TenRef<Range, Real>,
                       Labels const&, Real, Real);
template void contract(TenRefc<Range, Cplx>, Labels const&,
                       TenRefc<Range, Real>, Labels const&, TenRef<Range, Cplx>,
                       Labels const&, Real, Real);
template void contract(TenRefc<Range, Real>, Labels const&,
                       TenRefc<Range, Cplx>, Labels const&, TenRef<Range, Cplx>,
                       Labels const&, Real, Real);
template void contract(TenRefc<Range, Cplx>, Labels const&,
                       TenRefc<Range, Cplx>, Labels const&, TenRef<Range, Cplx>,
                       Labels const&, Real, Real);
template void contract(TenRefc<IndexSet, Real>, Labels const&,
                       TenRefc<IndexSet, Real>, Labels const&,
                       TenRef<IndexSet, Real>, Labels const&, Real, Real);
template void contract(TenRefc<IndexSet, Cplx>, Labels const&,
                       TenRefc<IndexSet, Real>, Labels const&,
                       TenRef<IndexSet, Cplx>, Labels const&, Real, Real);
template void contract(TenRefc<IndexSet, Real>, Labels const&,
...
struct MultInfo {
    bool tA = false, tB = false, Bfirst = false;
    MultInfo() {}
};

MultInfo static computeMultInfo(Labels const& ai, Labels const& bi,
                                Labels const& ci)
{
    MultInfo I;
    if (ai[1] == ci[1]) {
...
void contractloop(TenRefc<RangeT> A, Labels const& ai, TenRefc<RangeT> B,
                  Labels const& bi, TenRef<RangeT> C, Labels const& ci,
                  Args const& args)
{
    ...
    CProps p(ai, bi, ci);
    p.computeNactive();
    // printfln("nactive A, B, C are %d %d
    // %d",p.nactiveA,p.nactiveB,p.nactiveC);
    ...
    long ra = ai.size(), rb = bi.size(), rc = ci.size();
    ...
    detail::GCounter couA(ra), couB(rb);
    // Keep couA.i[0] and couA.i[1] fixed at 0
    couA.setRange(0, 0, 0);
    couA.setRange(1, 0, 0);
    // Let couA.i[j] (j >= 2) range from 0 to A.extent(j)-1
    for (int j = 2; j < ra; ++j) couA.setRange(j, 0, A.extent(j) - 1);

    // Similarly for couB
    couB.setRange(0, 0, 0);
    couB.setRange(1, 0, 0);
    for (int j = 2; j < rb; ++j) couB.setRange(j, 0, B.extent(j) - 1);

    Labels aind(ra, 0), bind(rb, 0), cind(rc, 0);
    ...
    for (; couA.notDone(); ++couA) {
        ...
        for (; couB.notDone(); ++couB) {
            for (int ib = 2; ib < rb; ++ib) {
                bind[ib] = couB[ib];
                if (p.BtoC(ib) != -1)
                    if (!p.contractedB(ib)) cind[p.BtoC(ib)] = couB[ib];
            }
...
template void contractloop(TenRefc<Range> A, Labels const& ai, TenRefc<Range> B,
                           Labels const& bi, TenRef<Range> C, Labels const& ci,
                           Args const& args);
template void contractloop(TenRefc<IndexSet> A, Labels const& ai,
...
template <typename RangeT>
void contractDiagFull(VectorRefc A, Labels const& ai, TenRefc<RangeT> B,
                      Labels const& bi, VectorRef C, Labels const& ci)
{
    Error("contractDiagFull not yet implemented");
}
template void contractDiagFull(VectorRefc A, Labels const& ai, TenRefc<Range> B,
                               Labels const& bi, VectorRef C, Labels const& ci);
template void contractDiagFull(VectorRefc A, Labels const& ai,
                               TenRefc<IndexSet> B, Labels const& bi,
