...
//
LAPACK_REAL
dnrm2_wrapper(LAPACK_INT N, const LAPACK_REAL* X, LAPACK_INT incx)
{
#ifdef ITENSOR_USE_CBLAS
                  ...
                  LAPACK_REAL const* B, LAPACK_REAL beta, LAPACK_REAL* C)
{
    ...
    cblas_dgemm(CblasColMajor, at, bt, m, n, k, alpha, A, lda, B, ldb, beta, C,
                m);
#else
    ...
    F77NAME(dgemm)
    (&at, &bt, &m, &n, &k, &alpha, pA, &lda, pB, &ldb, &beta, C, &m);
#endif
}
...
//
void gemm_wrapper(bool transa, bool transb, LAPACK_INT m, LAPACK_INT n,
                  LAPACK_INT k, Cplx alpha, const Cplx* A, const Cplx* B,
                  Cplx beta, Cplx* C)
{
    LAPACK_INT lda = m, ldb = k;
    ...
    //     {
    //     throw std::runtime_error("Complex alpha, beta not supported in zgemm
    //     for PLATFORM=openblas");
    //     }
    ...
    cblas_zgemm(CblasColMajor, at, bt, m, n, k, palpha, pA, lda, pB, ldb, pbeta,
                pC, m);
    ...
    cblas_zgemm(CblasColMajor, at, bt, m, n, k, palpha, (void*)A, lda, (void*)B,
                ldb, pbeta, (void*)C, m);
    ...
    F77NAME(zgemm)
    (&at, &bt, &m, &n, &k, palpha, pA, &lda, pB, &ldb, pbeta, pC, &m);
#endif
#endif
}
                  ...
                  LAPACK_INT incx, LAPACK_REAL* y, LAPACK_INT incy)
{
    ...
    char Tr = trans ? 'T' : 'N';
    F77NAME(dgemv)
    (&Tr, &m, &n, &alpha, const_cast<LAPACK_REAL*>(A), &m,
     const_cast<LAPACK_REAL*>(x), &incx, &beta, y, &incy);
#endif
}
                  ...
                  LAPACK_INT incy)
{
    ...
    cblas_zgemv(CblasColMajor, Tr, m, n, palpha, pA, m, px, incx, pbeta, py,
                incy);
    ...
    cblas_zgemv(CblasColMajor, Tr, m, n, palpha, (void*)A, m, (void*)x, incx,
                pbeta, (void*)y, incy);
#else
    char Tr = trans ? 'T' : 'N';
    auto ncA = const_cast<Cplx*>(A);
                   ...
                   LAPACK_INT& info)   // error info
{
    ...
    work.resize(lwork + 2);
    F77NAME(dsyev)
    (&jobz, &uplo, &n, A, &lda, eigs, work.data(), &lwork, &info, 1, 1);
#else
    ...
    LAPACK_INT* info)
{
    ...
    F77NAME(zgesdd)
    (jobz, m, n, pA, m, s, pU, m, pVt, &l, work.data(), &lwork, rwork.data(),
     iwork.data(), info, jobz_len);
#else
    F77NAME(zgesdd)
    (jobz, m, n, pA, m, s, pU, m, pVt, &l, work.data(), &lwork, rwork.data(),
     iwork.data(), info);
    ...
    LAPACK_INT* info)
{
    ...
    LAPACK_INT jobz_len = 1;
    F77NAME(dgesdd)
    (jobz, m, n, A, m, s, u, m, vt, &l, work.data(), &lwork, iwork.data(), info,
     jobz_len);
#else
    F77NAME(dgesdd)
    (jobz, m, n, A, m, s, u, m, vt, &l, work.data(), &lwork, iwork.data(),
     info);
#endif
}
    ...
    LAPACK_INT* info)
{
    ...
    LAPACK_INT jobz_len = 1;
    F77NAME(zgesvd)
    (jobz, jobz, m, n, pA, m, s, pU, m, pVt, &l, work.data(), &lwork,
     rwork.data(), info, jobz_len);
#else
    F77NAME(zgesvd)
    (jobz, jobz, m, n, pA, m, s, pU, m, pVt, &l, work.data(), &lwork,
     rwork.data(), info);
    ...
    LAPACK_INT* info)
{
    ...
    LAPACK_INT jobz_len = 1;
    F77NAME(dgesvd)
    (jobz, jobz, m, n, A, m, s, u, m, vt, &l, work.data(), &lwork, info,
     jobz_len);
#else
    F77NAME(dgesvd)
    (jobz, jobz, m, n, A, m, s, u, m, vt, &l, work.data(), &lwork, info);
    ...
    LAPACK_INT* info)     // error info
{
    ...
    LAPACK_INT lwork = std::max(one, 4 * std::max(*n, *m));
    work.resize(lwork + 2);
    static_assert(sizeof(LAPACK_COMPLEX) == sizeof(Cplx),
                  "LAPACK_COMPLEX and itensor::Cplx have different size");
    auto pA = reinterpret_cast<LAPACK_COMPLEX*>(A);
              ...
              LAPACK_REAL* d)  // eigenvalues on return
{
    ...
    F77NAME(zheev)
    (&jobz, &uplo, &N, pA, &N, d, work.data(), &lwork, rwork.data(), &info,
     jobz_len, uplo_len);
#else
    F77NAME(zheev)
    (&jobz, &uplo, &N, pA, &N, d, work.data(), &lwork, rwork.data(), &info);
                   ...
                   LAPACK_INT* info)  // error info
{
    ...
    std::vector<LAPACK_REAL> work;
    LAPACK_INT itype = 1;
    LAPACK_INT lwork = std::max(one, 3 * (*n) - 1);  // std::max(1,
                                                     // 1+6*N+2*N*N);
    work.resize(lwork);
#ifdef PLATFORM_acml
    LAPACK_INT jobz_len = 1;
    LAPACK_INT uplo_len = 1;
    F77NAME(dsygv)
    (&itype, jobz, uplo, n, A, n, B, n, d, work.data(), &lwork, info, jobz_len,
     uplo_len);
#else
    F77NAME(dsygv)
    (&itype, jobz, uplo, n, A, n, B, n, d, work.data(), &lwork, info);
#endif
}
              ...
              LAPACK_REAL* vr)       // right eigenvectors on return
{
    ...
    F77NAME(dgeev)
    (&jobvl, &jobvr, &n, cpA.data(), &n, dr, di, vl, &nevecl, vr, &nevecr,
     &wquery, &lwork, &info, 1, 1);
...
#ifdef PLATFORM_acml
    F77NAME(zgeev)
    (&jobvl, &jobvr, &n, cpA.data(), &n, pd, pvl, &nevecl, pvr, &nevecr,
