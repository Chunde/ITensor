...
template <class MatM, class MatU, class Vecd,
          class = stdx::require<hasMatRange<MatM>, hasMatRange<MatU>,
                                hasVecRange<Vecd>>>
...
template <class MatM, class MatV, class Vecd,
          class = stdx::require<hasMatRange<MatM>, hasMatRange<MatV>,
                                hasVecRange<Vecd>>>
...
template <typename V>
void orthog(MatRef<V> M, size_t numpass = 2);

template <typename Mat_, class = stdx::require<hasMatRange<Mat_>>>
void orthog(Mat_&& M, size_t numpass = 2)
{
    orthog(makeRef(M), numpass);
}
...
template <class MatM, class MatU, class VecD, class MatV,
          class = stdx::require<hasMatRange<MatM>, hasMatRange<MatU>,
                                hasVecRange<VecD>, hasMatRange<MatV>>>
void SVD(MatM&& M, MatU&& U, VecD&& D, MatV&& V,
...
void QR(MatA&& A, MatQ&& Q, MatR&& R, const Args& args = Args::global());

//
// Hermitian Matrix exponentiate
// by diagHermitian
//
template <class MatM, class ScalarT, class = stdx::require<hasMatRange<MatM>>>
Mat<common_type<val_type<MatM>, ScalarT>> expHermitian(MatM&& M, ScalarT t);

template <class MatM, class = stdx::require<hasMatRange<MatM>>>
Mat<val_type<MatM>> expHermitian(MatM&& M)
{
    return expHermitian(M, 1.);
}
...
template <class MatM, class = stdx::require<hasMatRange<MatM>>>
Mat<val_type<MatM>> expMatrix(MatM&& M)
{
    return expMatrix(M, 1.);
}
