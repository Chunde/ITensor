...
auto rangeBegin(range_type const& r) -> decltype(r.begin())
{
    return r.begin();
}

template <typename range_type>
auto rangeEnd(range_type const& r) -> decltype(r.end())
{
    return r.end();
}
...
class TenIter
{
   ...
   public:
    ...
    TenIter(storage_type d, range_type const& r) : d_(d), it_(rangeBegin(r)) {}

    reference operator*() { return d_[it_.offset()]; }
    ...
    range_type const& range() const { return it_.range(); }

    bool notDone() const { return it_.notDone(); }

    bool operator!=(TenIter const& other) const { return it_ != other.it_; }
   ...
   public:
    ...
    TenIter static makeEnd(range_type const& r)
    {
        TenIter end;
        end.it_ = rangeEnd(r);
        return end;
