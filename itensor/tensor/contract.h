...
void contract(TenRefc<RangeT, VA> A, Labels const& ai, TenRefc<RangeT, VB> B,
              ...
              Labels const& ci, Real alpha = 1., Real beta = 0.);

template <typename R, typename VA, typename VB>
void contract(Ten<R, VA> const& A, Labels const& ai, Ten<R, VB> const& B,
              Labels const& bi, Ten<R, common_type<VA, VB>>& C,
              Labels const& ci, Real alpha = 1., Real beta = 0.);

template <typename range_type>
void contractloop(TenRefc<range_type> A, Labels const& ai,
                  TenRefc<range_type> B, Labels const& bi, TenRef<range_type> C,
                  Labels const& ci, Args const& args = Args::global());

template <typename range_type>
void contractloop(Ten<range_type> const& A, Labels const& ai,
...
void contractDiagFull(DiagElsA const& A, Labels const& ai,
...
template <typename DiagElsA, typename RangeT, typename VB, typename VC>
void contractDiagPartial(DiagElsA const& A, Labels const& ai,
                         TenRefc<RangeT, VB> const& B, Labels const& bi,
...
void ncprod(TA&& A, Labels const& ai, TB&& B, Labels const& bi, TC&& C,
            Labels const& ci);

template <typename Inds, typename Func>
long computeLabels(Inds const& Lis, long rL, Inds const& Ris, long rR,
                   Labels& Lind, Labels& Rind, Func const& checkCont);

template <typename Inds>
long computeLabels(Inds const& Lis, long rL, Inds const& Ris, long rR,
                   Labels& Lind, Labels& Rind);

template <typename T>
long find_index(std::vector<T> const& v, T const& t)
{
...
long find_index(VarArray<T, MaxSize> const& v, T const& t)
{
    for (decltype(v.size()) i = 0; i < v.size(); ++i)
        if (v[i] == t) return i;
    return -1;
...
template <typename T, size_t MaxSize>
long find_index(InfArray<T, MaxSize> const& v, T const& t)
{
    for (decltype(v.size()) i = 0; i < v.size(); ++i)
        if (v[i] == t) return i;
    return -1;
...
void contractloop(Ten<range_type> const& A, Labels const& ai,
                  Ten<range_type> const& B, Labels const& bi,
                  Ten<range_type>& C, Labels const& ci, Args const& args)
{
...
template <typename DiagElsA, typename RangeT, typename VB, typename VC>
void contractDiagPartial(DiagElsA const& A, Labels const& al,
                         ...
                         IntArray astarts)
{
    ...
    for (; GC.notDone(); ++GC) {
        ...
        for (auto J : range(A)) {
            pc[cstart + J * c_cstride + coffset] +=
                A(J) * pb[bstart + J * b_cstride + boffset];
        }
    }
}
...
// making C diagonal
template <typename DiagElsA, typename RangeT, typename VB, typename VC>
void contractDiagFull(DiagElsA const& A, Labels const& al,
                      TenRefc<RangeT, VB> const& B, Labels const& bl,
                      VecRef<VC> const& C, Labels const& cl, IntArray astarts)
{
    if (astarts.empty()) astarts.assign(al.size(), 0);
    auto bstart = 0ul;
