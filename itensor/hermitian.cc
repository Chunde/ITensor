...
template <typename T>
Spectrum diagHImpl(ITensor H, ITensor &U, ITensor &D, Args args)
{
    ...
    if (not hasQNs(H)) {
        ...
        if (do_truncate) {
            ...
            tie(truncerr, docut_lower, docut_upper, ndegen) = truncate(
                DD, maxdim, mindim, cutoff, absoluteCutoff, doRelCutoff, args);
            m = DD.size();
            reduceCols(UU, m);
        }
        ...
        D = ITensor({prime(newmid, pdiff), newmid},
                    DiagReal{DD.begin(), DD.end()}, H.scale());

        if (not H.scale().isTooBigForReal()) {
            DD *= H.scale().real0();
        } else {
            println(
                "diag_hermitian: scale too big for Real, omitting from "
                "returned "
                "spectrum.");
        }
    ...
    } else  // With QNs
    {
        ...
        if (div(H) != Zero) {
            Print(H);
            Error(
                "diagHermitian currently only defined for block-diagonal "
                "IQTensors");
        }
#endif
        ...
        auto blocks =
            doTask(GetBlocks<T>{H.inds(), ai, prime(ai, pdiff)}, H.store());
        auto Nblock = blocks.size();

        size_t totaldsize = 0, totalUsize = 0;
...
Spectrum diag_hermitian(ITensor H, ITensor &U, ITensor &D, Args const &args)
{
    if (isComplex(H)) {
