...
IndexSet &IndexSet::swapTags(TagSet const &ts1, TagSet const &ts2,
                             VarArgs &&...vargs)
{
    TagSet tempTags;
    if (primeLevel(ts1) < 0) {
        if (primeLevel(ts2) >= 0)
            Error(
                "Error in swapTags: cannot swap integer tag with non-integer "
                "tag");
...
template <typename... VarArgs>
IndexSet &IndexSet::swapPrime(int pl1, int pl2, VarArgs &&...vargs)
{
    ...
    for (auto &I : *this) {
        if (primeLevel(I) == tempPrime) {
            ...
            throw ITError(
                "swapPrime fails if an index has prime level tempPrime");
        }
    }
#endif
                ...
                bool sortResult)
{
    ...
    for (decltype(Lis.order()) j = 0; j < Lis.order(); ++j) {
        ...
        {
            if (LremoveQNs)
                newind.nextIndStr(removeQNs(Lis[j]), Lind[j]);
            else
    ...
    for (decltype(Ris.order()) j = 0; j < Ris.order(); ++j) {
        ...
        {
            if (RremoveQNs)
                newind.nextIndStr(removeQNs(Ris[j]), Rind[j]);
            else
                            ...
                            IndexSet &Nis)
{
    ...
    for (auto i : range(Lis.order())) {
        if (Lind[i] < 0) {
            if (Rind[0] >= 0)
                newind.nextIndex(Ris[0]);
            else
                newind.nextIndex(Ris[1]);
        } else {
