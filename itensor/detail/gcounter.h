...
class GCounter  // General Counter
{
   public:
    ...
    GCounter(long nind)
        : first(0, nind - 1, 0), last(0, nind - 1, 0), i(0, nind - 1, 0), ind(0)
    {
    }

    // for a GCounter that has indices i[1] through i[8], and starts its counts
    // at 0, firstind = 1, lastind = 8, firstval = 0
    GCounter(long firstind, long lastind, long firstval = 0)
    ...
    void setRange(long j, long s, long e)
    {
        first.ref(j) = s;
        last.ref(j) = e;
        i.ref(j) = s;
    ...
    {
        ++ind;
        auto mi = first.mini(), ma = first.maxi();
        ++i[mi];
        if (i[mi] > last[mi]) {
    ...
    long operator[](long j) const { return i[j]; }

    bool notDone() { return i[first.mini()] >= first[first.mini()]; }
};
