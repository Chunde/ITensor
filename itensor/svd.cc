...
namespace itensor
{
            ...
            Args args)
    {
        ...
        else
        {
            ...
            if (dim(uI) == 0)
                throw ResultIsZero("dim(uI) == 0");
            if (dim(vI) == 0)
                throw ResultIsZero("dim(vI) == 0");
            ...
            if (compute_qn)
                stdx::sort(alleigqn, std::greater<EigQN>{});
