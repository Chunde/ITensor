...
template <typename T>
Spectrum svdImpl(ITensor const &A, Index const &uI, Index const &vI, ITensor &U,
                 ITensor &D, ITensor &V, Args args)
{
    ...
    if (litagset == ritagset) {
        Error(
            "In SVD, must specify different tags for the new left and right "
            "indices (with Args 'LeftTags' and 'RightTags')");
    }
    ...
    if (not hasQNs(A)) {
        ...
        long m = DD.size();
        if (do_truncate) {
            tie(truncerr, docut_lower, docut_upper, ndegen) =
                truncate(probs, maxdim, mindim, cutoff, absoluteCutoff,
                         doRelCutoff, args);
            m = probs.size();
            resize(DD, m);
            reduceCols(UU, m);
        ...
        D = ITensor({uL, vL}, Diag<Real>{DD.begin(), DD.end()},
                    A.scale() * signfix);
        ...
        for (auto b : range(Nblock)) {
            ...
            if (do_truncate) {
                ...
                while (ndegen > 0 && this_m < d.size() && total_m < m &&
...
#ifdef DEBUG
        if (Liq.empty())
            throw std::runtime_error("New Index of U after SVD is empty");
        if (Riq.empty())
            throw std::runtime_error("New Index of V after SVD is empty");
#endif
        ...
        for (auto b : range(Nblock)) {
            ...
            auto Vref = makeMatRef(pV.data(), pV.size(), vI.blocksize0(B.i2),
                                   R.blocksize0(n));
                 ...
                 ITensor &D, ITensor &V, Args args)
{
    ...
    if (not args.defined("Truncate")) args.add("Truncate", do_truncate);
