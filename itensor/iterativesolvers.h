...
namespace itensor
{
             ...
             Args const &args)
    {
        ...
        for (auto ii : range(actual_maxiter + 1))
        {
            ...
            bool converged = (qnorm < errgoal_ && std::abs(lambda - last_lambda) < errgoal_) || qnorm < std::max(Approx0, errgoal_ * 1E-3);
        ...
        if (debug_level_ >= 2 && t + 1 < nget)
            printfln("Max iter. reached, computing remaining %d evecs", nget - t - 1);
    ...
    namespace gmres_details
    {
        ...
        void inline applyPlaneRotation(Real &dx, Real &dy, Real const &cs, Real const &sn)
        {
        ...
        void inline applyPlaneRotation(Cplx &dx, Cplx &dy, Cplx const &cs, Cplx const &sn)
        {
            ...
            Args const &args)
    {
        ...
        if (nget == 0)
            Error("No initial vectors passed to arnoldi.");
