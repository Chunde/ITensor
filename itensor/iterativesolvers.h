...
Real davidson(BigMatrixT const &A, ITensor &phi,
...
//
template <class BigMatrixT>
std::vector<Real> davidson(BigMatrixT const &A, std::vector<ITensor> &phi,
                           Args const &args = Args::global());
...
void gmres(BigMatrixT const &A, BigVectorT const &b, BigVectorT &x,
           Args const &args = Args::global());
...
void applyExp(BigMatrixT const &A, ITensor &phi, ElT t,
              Args const &args = Args::global());
...
template <class BigMatrixT>
Real davidson(BigMatrixT const &A, ITensor &phi, Args const &args)
{
                           ...
                           Args const &args)
{
    ...
    if (debug_level_ > 2) printfln("Initial Davidson energy = %.10f", initEn);
    ...
    for (auto ii : range(actual_maxiter + 1)) {
        ...
        bool converged =
            (qnorm < errgoal_ && std::abs(lambda - last_lambda) < errgoal_) ||
            qnorm < std::max(Approx0, errgoal_ * 1E-3);

        last_lambda = lambda;

        if ((qnorm < 1E-20) || (converged && ii >= miniter_) ||
            (ii == actual_maxiter)) {
            if (t < (nget - 1) && ii < actual_maxiter) {
                ...
                if (debug_level_ >=
                    3)  // Explain why breaking out of Davidson loop early
                {
                    if ((qnorm < errgoal_ &&
                         std::fabs(lambda - last_lambda) < errgoal_))
                        printfln(
                            "Exiting Davidson because errgoal=%.0E reached",
                            errgoal_);
                    else if (ii < miniter_ ||
                             qnorm < std::max(Approx0, errgoal_ * 1.0e-3))
                        printfln(
                            "Exiting Davidson because small residual=%.0E "
                            "obtained",
                            qnorm);
                    else if (ii == actual_maxiter)
                        println(
                            "Exiting Davidson because ii == actual_maxiter");
                }
        ...
        while (pass <= Npass) {
            ...
            if (qnrm < 1E-10) {
                ...
                if (ni >= maxsize) {
                    ...
                    if (debug_level_ >= 3)
                        println(
                            "Breaking out of Davidson: max Hilbert space size "
                            "reached");
                    goto done;
                }
                ...
                if (tot_pass > Npass * 3) {
                    ...
                    if (debug_level_ >= 3)
                        println(
                            "Breaking out of Davidson: orthog step too big");
                    goto done;
                }
            }
            q *= 1. / qnrm;
        ...
        if (debug_level_ >= 3)
            println("Done with orthog step, tot_pass=", tot_pass);
    ...
    if (debug_level_ >= 2 && t + 1 < nget)
        printfln("Max iter. reached, computing remaining %d evecs",
                 nget - t - 1);
...
void update(BigVectorT &x, int const k, Matrix const &h, std::vector<T> &s,
            std::vector<BigVectorT> const &v)
{
    std::vector<T> y(s);
    ...
    for (int j = 0; j <= k; j++) x += y[j] * v[j];
}

template <typename T>
void generatePlaneRotation(T const &dx, T const &dy, T &cs, T &sn)
{
    if (dy == 0.0) {
        cs = 1.0;
        sn = 0.0;
...
void inline applyPlaneRotation(Real &dx, Real &dy, Real const &cs,
                               Real const &sn)
{
...
void inline applyPlaneRotation(Cplx &dx, Cplx &dy, Cplx const &cs,
                               Cplx const &sn)
{
...
void dot(BigVectorT const &A, BigVectorT const &B, Real &res)
{
    res = elt(dag(A) * B);
}

template <typename BigVectorT>
void dot(BigVectorT const &A, BigVectorT const &B, Cplx &res)
{
    res = eltC(dag(A) * B);
}
...
void gmresImpl(BigMatrixT const &A, BigVectorT const &b, BigVectorT &x,
               BigVectorT &Ax, Args const &args)
{
    auto n = A.size();
    ...
    while (j <= max_iter) {
        ...
        for (i = 0; i < m && j <= max_iter; i++, j++) {
            ...
            if (debug_level_ > 0) println("norm(w) = ", normw);
            ...
            for (k = 0; k < i; ++k)
                gmres_details::applyPlaneRotation(H(k, i), H(k + 1, i), cs[k],
                                                  sn[k]);

            gmres_details::generatePlaneRotation(H(i, i), H(i + 1, i), cs[i],
                                                 sn[i]);
            gmres_details::applyPlaneRotation(H(i, i), H(i + 1, i), cs[i],
                                              sn[i]);
           ...
           Args const &args)
{
    ...
    if (isComplex(b) || isComplex(Ax)) {
        if (debug_level_ > 0) println("Calling complex version of gmresImpl()");
        gmresImpl<Cplx>(A, b, x, Ax, args);
    } else {
        if (debug_level_ > 0) println("Calling real version of gmresImpl()");
        gmresImpl<Real>(A, b, x, Ax, args);
    }
}

int inline findEig(Vector const &vr, Vector const &vi, std::string whichEig)
{
    int n = -1;
    Real foundval = NAN;
    for (size_t i = 0; i < vr.size(); i++) {
        if (whichEig == "LargestMagnitude") {
            ...
            error(
                "Unsupported eigenvalue target, currently only support: "
                "LargestMagnitude, SmallestReal");
        }
    }
    return n;
}
...
template <class BigMatrixT>
std::vector<Complex> arnoldi(const BigMatrixT &A, std::vector<ITensor> &phi,
                             Args const &args)
{
    ...
    for (size_t j = 0; j < nget; ++j) {
        ...
        if (nrm == 0.0) Error("norm of 0 in arnoldi");
        phi[j] *= 1.0 / nrm;
    }
    ...
    if (phi.size() > size_t(maxsize))
        Error(
            "arnoldi: requested more eigenvectors (phi.size()) than size of "
            "matrix (A.size())");
    ...
    if (dim(phi.front().inds()) != size_t(maxsize)) {
        Error(
            "arnoldi: size of initial vector should match linear matrix size");
    }
    ...
    for (size_t w = 0; w < nget; ++w) {
        for (int r = 0; r <= maxrestart_; ++r) {
            ...
            for (int it = 0; it <= actual_maxiter; ++it) {
                ...
                for (size_t irows = 0; irows < Hnrows; irows++)
                    for (size_t icols = 0; icols < Hncols; icols++)
                        Href(irows, icols) =
                            Complex(HrefR(irows, icols), HrefI(irows, icols));

                eigen(Href, YR, YI, D, DI);
                n = findEig(
                    D, DI,
                    whicheig_);  // continue to target the largest eig
                ...
                // Estimate error || (A-l_j*I)*p_j || = h_{j+1,j}*[last entry of
                // Y_j] See
                // http://web.eecs.utk.edu/~dongarra/etemplates/node216.html
                    ...
                    for (size_t j = 0; j <= w; ++j) {
                        if (fabs(eigs[j].real()) > 1E-6) {
                            if (fabs(eigs[j].imag()) > Approx0)
                                printf(" (%.10f,%.10f)", eigs[j].real(),
                                       eigs[j].imag());
                            else
                                printf(" %.10f", eigs[j].real());
                        } else {
                            if (fabs(eigs[j].imag()) > Approx0)
                                printf(" (%.5E,%.5E)", eigs[j].real(),
                                       eigs[j].imag());
                            else
            ...
            //     Cout << Format("<V[%d]|V[%d]> = %.5E") % i % j %
            //     BraKet(V.at(i),V.at(j)) << Endl;
            // Cout << Endl;
...
template <class BigMatrixT>
Complex arnoldi(const BigMatrixT &A, ITensor &vec,
                Args const &args = Args::global())
{
...
void assembleLanczosVectors(std::vector<ITensor> const &lanczos_vectors,
                            VecT const &linear_comb, double norm, ITensor &phi)
{
...
template <typename BigMatrixT, typename ElT>
void applyExp(BigMatrixT const &H, ITensor &phi, ElT tau, Args const &args)
{
    ...
    for (int iter = 0; iter < max_iter; ++iter) {
        ...
        if (iter > 0) {
            ...
            double phi2 =
                std::abs(nrm * tmat_ext_exp(tmat_size + 1, 0) * avnorm);
            double error;
            if (phi1 > 10 * phi2)
                error = phi2;
            else if (phi1 > phi2)
...
void applyExp(BigMatrixT const &H, ITensor &phi, int tau, Args const &args)
{
