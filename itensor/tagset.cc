...
namespace itensor
{
    ...
    {
        if (primeLevel(t1) != primeLevel(t2))
            return false;
        if (size(t1) != size(t2))
            return false;
        ...
        addTag(Tag const &t)
    {
        ...
        {
            if (primelevel_ > 0)
                throw std::runtime_error("Cannot have more than one integer tag in a TagSet");
            primelevel_ = plev;
        }
        else
        {
            if (size_ == MAX_TAGS)
                error("Too many tags already, cannot add more. If you want more, consider raising MAX_TAGS.");
