...
void checkCompatible(QNum const &qva, QNum const &qvb)
{
#ifdef DEBUG
...
QN::qn_t QN::val(QNName const &name) const
{
    for (auto &v : qvs_) {
...
QN::qn_t QN::mod(QNName const &name) const
{
    for (auto &v : qvs_) {
...
bool QN::hasName(QNName const &name) const
{
    for (auto &v : qvs_) {
...
void checkCompatible(QN const &qa, QN const &qb)
{
#ifdef DEBUG
...
//
template <typename Func>
void combineQN(QN &qa, QN const &qb, Func &&operation)
{
    auto &avs = qa.store();
    for (auto &bv : qb.store()) {
...
{
    auto addOp = [](QNum &qva, QNum const &qvb) { qva += qvb; };
...
void h5_write(h5::group parent, string const &name, QN const &q)
{
...
void h5_read(h5::group parent, string const &name, QN &q)
{
