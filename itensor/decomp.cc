...
template <typename V>
MatRefc<V> doTask(ToMatRefc<V> const &T, Dense<V> const &d)
{
...
template <typename V>
MatRefc<V> toMatRefc(ITensor const &T, Index const &i1, Index const &i2)
{
    if (i1 == T.inds().front()) {
...
template MatRefc<Real> toMatRefc(ITensor const &T, Index const &i1,
                                 Index const &i2);
template MatRefc<Cplx> toMatRefc(ITensor const &T, Index const &i1,
                                 Index const &i2);

/////////////

template <typename T>
vector<Ord2Block<T>> doTask(GetBlocks<T> const &G, QDense<T> const &d)
{
    if (G.is.order() != 2)
        Error("doTask(GetBlocks,QDenseReal) only supports 2-index tensors");
    ...
    for (auto const &dio : d.offsets) {
        ...
        R.i1 = dio.block[0];
        R.i2 = dio.block[1];
        R.M = makeMatRef(d.data() + dio.offset, d.size() - dio.offset, nrow,
                         ncol);
    }
    if (G.transpose) {
        for (auto &R : res) {
...
template vector<Ord2Block<Real>> doTask(GetBlocks<Real> const &G,
                                        QDense<Real> const &d);
template vector<Ord2Block<Cplx>> doTask(GetBlocks<Cplx> const &G,
                                        QDense<Cplx> const &d);

///////////////

Spectrum svd(ITensor const &AA, ITensor &U, ITensor &D, ITensor &V, Args args)
{
    ...
    if (args.defined("UseOrigM")) {
        if (args.defined("UseOrigDim")) {
            Global::warnDeprecated(
                "Args UseOrigM and UseOrigDim are both defined. UseOrigM is "
                "deprecated in favor of UseOrigDim, UseOrigDim will be used.");
        } else {
            Global::warnDeprecated(
                "Arg UseOrigM is deprecated in favor of UseOrigDim.");
    ...
    if (!U && !V)
        Error(
            "U and V default-initialized in svd, must indicate at least one "
            "index "
            "on U or V");
#endif
    ...
    if (noise > 0) Error("Noise term not implemented for svd");
...
std::tuple<ITensor, ITensor, ITensor> svd(ITensor const &AA,
                                          IndexSet const &Uis,
                                          IndexSet const &Vis, Args args)
{
    if (!hasSameInds(inds(AA), IndexSet(Uis, Vis)))
        Error(
            "In svd, U indices and V indices must match the indices of the "
            "input "
            "ITensor");
...
std::tuple<ITensor, ITensor> polar(ITensor const &T, IndexSet const &Uis,
                                   Args const &args)
{
    ...
    if (primeLevel(ts) >= 0)
        Error(
            "In polar, specify a prime level increment with the Prime "
            "argument");
    if (ts == "" && plinc == -1)
        Error(
            "In polar, must either increment prime level or add tags to new "
            "indices");
...
std::tuple<Real, Real, Real, int> truncate(Vector &P, long maxdim, long mindim,
                                           Real cutoff, bool absoluteCutoff,
                                           bool doRelCutoff, Args const &args)
{
    ...
    if (respectDegenerate && (ndegen_below + ndegen_above > 1)) {
        if (maxdim < m + ndegen_above)  // If the maximum dimension falls within
...
void showEigs(Vector const &P, Real truncerr, LogNum const &scale, Args args)
{
    if (args.defined("Minm")) {
        if (args.defined("MinDim")) {
            Global::warnDeprecated(
    ...
    println();
    printfln("mindim = %d, maxdim = %d, cutoff = %.2E, truncate = %s", mindim,
             maxdim, cutoff, do_truncate);
    printfln("Kept m=%d states, trunc. err. = %.3E", P.size(), truncerr);
    printfln("doRelCutoff = %s, absoluteCutoff = %s", doRelCutoff,
             absoluteCutoff);
    IF_USESCALE(printfln("Scale is = %sexp(%.2f)", scale.sign() > 0 ? "" : "-",
                         scale.logNum());)
...
Real absSqrt(Real x) { return std::sqrt(std::fabs(x)); }

Spectrum factor(ITensor const &T, ITensor &A, ITensor &B, Args args)
{
...
std::tuple<ITensor, ITensor> factor(ITensor const &T, IndexSet const &Ais,
                                    IndexSet const &Bis, Args const &args)
{
    if (!hasSameInds(inds(T), IndexSet(Ais, Bis)))
        Error(
            "In factor, A indices and B indices must match the indices of the "
            "input ITensor");
...
void eigDecompImpl(ITensor T, ITensor &L, ITensor &R, ITensor &D,
                   Args const &args)
{
    auto itagset = getTagSet(args, "Tags", "Link");
    ...
    if (not hasQNs(T)) {
...
#ifdef DEBUG
                if (ri == Rr.end() || ii == Ri.end())
                    Error("out of range iterator");
#endif
                store[n] = Cplx(*ri, *ii);
            }
        ...
        if (norm(Di) > 1E-16 * norm(Dr)) {
            ...
            // real eigenvectors
            D = ITensor({prime(newmid), newmid}, DiagReal{move(Dr.storage())},
                        T.scale());
        }
        ...
        if (full) {
            ...
            if (norm(Li) > 1E-16 * norm(Lr)) {
                ...
                auto ri = Lr.begin();
                auto ii = Li.begin();
                for (decltype(Lr.size()) n = 0; n < Lr.size();
                     ++ri, ++ii, ++n) {
#ifdef DEBUG
                    if (ri == Lr.end() || ii == Li.end())
                        Error("out of range iterator");
#endif
                    store[n] = Cplx(*ri, *ii);
                }
...
Spectrum denmatDecomp(ITensor const &AA, ITensor &A, ITensor &B, Direction dir,
                      Args const &args)
{
    return denmatDecomp(AA, A, B, dir, NoOp(), args);
}

std::tuple<ITensor, ITensor> denmatDecomp(ITensor const &T, IndexSet const &Ais,
                                          IndexSet const &Bis, Direction dir,
                                          Args const &args)
{
    if (!hasSameInds(inds(T), IndexSet(Ais, Bis)))
        Error(
            "In svd, A indices and B indices must match the indices of the "
            "input "
            "ITensor");
...
Spectrum diagPosSemiDef(ITensor const &M, ITensor &U, ITensor &D, Args args)
{
    ...
    for (auto &i : M.inds())
        if (noPrime(i) == noPrime(k)) kps.push_back(i.primeLevel());
    if (kps.size() <= 1ul || kps.size() % 2 != 0ul) {
        Error(
            "Input tensor to diagHermitian should have pairs of indices with "
            "equally spaced prime levels");
    }
    auto nk = kps.size();
    ...
    for (auto &i : M.inds())
        for (auto &j : M.inds()) {
            if (noPrime(i) == noPrime(j) &&
                i.primeLevel() + pdiff == j.primeLevel()) {
                inds.push_back(i);
            }
        }
    ...
    try {
        ...
        println(
            "Diagonalize expects opposite arrow directions for primed and "
            "unprimed "
            "indices.");
        throw e;
    }
...
std::tuple<ITensor, ITensor> diagPosSemiDef(ITensor const &T, Args const &args)
{
    ITensor U, D;
...
Spectrum diagHermitian(ITensor const &M, ITensor &U, ITensor &D, Args args)
{
    args.add("Truncate", false);
    return diagPosSemiDef(M, U, D, args);
}

std::tuple<ITensor, ITensor> diagHermitian(ITensor const &T, Args const &args)
{
    ITensor U, D;
...
void eigen(ITensor const &T, ITensor &V, ITensor &D, Args args)
{
    if (!args.defined("Tags")) args.add("Tags", "Link");
    ...
    if (inds(Tc)(1) != prime(cind)) Tc = permute(Tc, {prime(cind), cind});
...
std::tuple<ITensor, ITensor> eigen(ITensor const &T, Args const &args)
{
    ITensor V, D;
    eigen(T, V, D, args);
...
void eigDecomp(ITensor const &T, ITensor &R, ITensor &D, ITensor &Rinv,
               Args const &args)
{
    auto colinds = std::vector<Index>{};
    for (auto &I : T.inds()) {
...
struct Exp {
    ...
    T operator()(Real x) const { return exp(tt * x); }
};

ITensor expHermitian(ITensor const &T, Cplx t)
{
    ITensor U, d;
    diagHermitian(T, U, d);
...
void qr(ITensor const &AA, ITensor &Q, ITensor &R, Args args)
{
#ifdef DEBUG
    if (!Q && !R)
        Error(
            "Q and R default-initialized in qr, must indicate at least one "
            "index "
            "on Q or R");
#endif
...
std::tuple<ITensor, ITensor> qr(ITensor const &AA, IndexSet const &Qis,
                                IndexSet const &Ris, Args args)
{
    if (!hasSameInds(inds(AA), IndexSet(Qis, Ris)))
        Error(
            "In QR, Q indices and R indices must match the indices of the "
            "input "
            "ITensor");
...
template <typename T>
void qrImpl(ITensor const &A, Index const &qI, Index const &rI, ITensor &Q,
            ITensor &R, Args args)
{
    ...
    if (not hasQNs(A)) {
        ...
        Liq.reserve(Nblock);
        if (uppertriangular) indLiq.reserve(Nblock);
        ...
        std::set<int> zerob;
        for (int i = 0; i < nblock(qI); i++) zerob.emplace(i);
        ...
        for (auto b : range(Nblock)) {
            int n = b;
            if (uppertriangular) n = std::get<2>(indLiq[b]);
...
void qrOrd2(ITensor const &A, Index const &qI, Index const &rI, ITensor &Q,
            ITensor &R, Args args)
{
    if (A.order() != 2) {
