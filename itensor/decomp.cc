...
namespace itensor
{
          ...
          Args const &args)
    {
        ...
        if (primeLevel(ts) >= 0)
            Error("In polar, specify a prime level increment with the Prime argument");
        if (ts == "" && plinc == -1)
            Error("In polar, must either increment prime level or add tags to new indices");
                   ...
                   Args args)
    {
        ...
        for (auto &i : M.inds())
            if (noPrime(i) == noPrime(k))
                kps.push_back(i.primeLevel());
           ...
           Args args)
    {
        ...
        else
        {
            ...
            if (Nblock == 0)
                throw ResultIsZero("IQTensor has no blocks");
