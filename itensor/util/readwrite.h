...
namespace itensor
{
    ...
    bool inline fileExists(const std::string &fname)
    {
    ...
    T read(std::istream &s, CtrArgs &&...args)
    {
    ...
    void inline write(std::ostream &s, const std::string &str)
    {
        auto size = str.size();
    ...
    void inline read(std::istream &s, std::string &str)
    {
    ...
    void inline read(std::istream &s, Cplx &z)
    {
    ...
    void inline write(std::ostream &s, const Cplx &z)
    {
    ...
    template <typename T, size_t N, bool isPod = std::is_standard_layout<T>::value && std::is_trivial<T>::value>
    struct ReadIAData
    {
    ...
    void inline write(std::ostream &s, BlOf const &blof)
    {
        itensor::write(s, blof.block);
        itensor::write(s, blof.offset);
    }

    void inline read(std::istream &s, BlOf &blof)
    {
    ...
    T readFromFile(const std::string &fname, InitArgs &&...iargs)
    {
        std::ifstream s(fname.c_str(), std::ios::binary);
        if (!s.good())
            throw ITError("Couldn't open file \"" + fname + "\" for reading");
    ...
    std::string inline mkTempDir(const std::string &pfix,
