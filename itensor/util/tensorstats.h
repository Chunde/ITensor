...
namespace itensor
{
    ...
    void inline doHistogram1(Real bucket_size = 0.5)
    {
        Real max_value = 10.;
    ...
    void inline makeBenchmark(std::string name)
    {
        ...
        for (auto &t : global_tstats())
        {
            ...
            for (int j : range1(Nind))
            {
                ...
                if (!found)
                    found = printSizes(f, j, t.Alabs, t.Adims, t.Ar);
                if (!found)
                    printSizes(f, j, t.Blabs, t.Bdims, t.Br);
            }
            print(f, ";\n");
        }
        f.close();
