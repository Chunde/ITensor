...
namespace itensor
{
    ...
    public:
        ...
        size_t constexpr static arr_size() { return ArrSize; }
            ...
            if (size_ <= ArrSize)
            {
                ...
                for (; p != e; ++p)
                    if (p == it)
                        break;
                if (p == e)
                    throw std::runtime_error("erase: element not found");
                for (; (p + 1) != e; ++p)
                {
                    *p = *(p + 1);
                }
    ...
    private:
        ...
        check_ind(size_t i) const
        {
            ...
            if (i >= size_)
                throw std::runtime_error("index out of range in InfArray");
        }
        void
        check_empty() const
        {
            if (size_ == 0)
                Error("InfArray is empty");
        }
    };
