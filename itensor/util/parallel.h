...
namespace itensor
{
    ...
    template <typename T>
    T sum(Environment const &env, T &obj);

    template <typename T>
    T allSum(Environment const &env, T &obj);
    ...
    private:
        ...
        {
            if (env_ == nullptr)
                throw std::runtime_error("MailBox object is default initialized.");
        }
    ...
    void inline parallelDebugWait(Environment const &env)
    {
        char hostname[256];
    ...
    broadcast(Environment const &env, T &obj)
    {
        ...
        if (env.rank() == root)
            itensor::write(datastream, obj);
        env.broadcast(datastream);
        if (env.rank() != root)
            itensor::read(datastream, obj);
    }
    ...
    double inline sum(Environment const &env, double r)
    {
        if (env.nnodes() == 1)
            return r;
        double res = 0;
