...
namespace itensor
{
    ...
    public:
        ...
        {
            if (pt)
                throw std::runtime_error("SafePtr: single-arg constructor only accepts nullptr");
        }
        ...
        {
            if (!p_)
                throw std::runtime_error("SafePtr: pointer is null");
        }
        ...
        {
            if (!p_)
                throw std::runtime_error("SafePtr operator[]: dereferencing null pointer");
            auto os = offset_ + ind;
            if (os >= offset_end_)
            {
    ...
    template <typename T>
    SafePtr<T>
    operator+(size_t inc, SafePtr<T> p)
    {
        p += inc;
        return p;
    }
    ...
    template <typename T>
    SafePtr<T>
    operator+(SafePtr<T> p, size_t inc)
    {
        p += inc;
        return p;
    }
