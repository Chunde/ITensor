...
namespace itensor
{
    ...
    public:
        ...
        { // Uses the bootstrap procedure to estimate the second cumulant error
            ...
            if (n < 2)
                return 0.0;
            if (firstc.dat.size() != n)
                error("Stats instances don't have the same size data sets.");
            for (int sample = 1; sample <= nresamples; ++sample)
            {
                Real avg2 = 0.0;
                Real avg = 0.0;
                ...
                { // Sample with replacement (replacement means 'which' can take the same value more than once)
                    ...
                    avg2 += dat.at(which);
                    if (!correlated)
                        which = int(Global::random() * n); // use <H^2> and <H> from the same METTS
