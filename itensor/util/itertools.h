...
namespace triqs::utility
{
  ...
  public:
    ...
    template <typename U>
    bool operator==(U const &other) const { return self().equal(other); }
    template <typename U>
    bool operator!=(U const &other) const { return (!operator==(other)); }
  ...
  struct zip_iter : iterator_facade<zip_iter<It...>, std::tuple<typename std::iterator_traits<It>::value_type...>>
  {
    ...
    template <typename... U>
    void nop(U &&...u) {} // do nothing...
    template <size_t... Is>
    void increment_all(std::index_sequence<Is...>) { nop(++std::get<Is>(its)...); }
  ...
  namespace details
  {
    ...
    template <typename It>
    sentinel_t<It> make_sentinel(It &&it) { return {it}; }
  } // namespace details
  ...
  struct prod_iter : iterator_facade<prod_iter<It...>, std::tuple<typename std::iterator_traits<It>::value_type...>>
  {
    ...
    template <typename U>
    bool equal(details::sentinel_t<U> const &s) const { return (s.it == std::get<sizeof...(It) - 1>(its)); }
  ...
  namespace details
  {
    ...
    struct zipped
    {
      ...
      template <typename F, size_t... Is>
      auto tuple_map(F &&f, std::index_sequence<Is...>) const
      {
      ...
      iterator begin()
      {
        return tuple_map([](auto &&x)
                         { return std::begin(x); }, seq_t{});
      }
      const_iterator cbegin() const
      {
        return tuple_map([](auto &&x)
                         { return std::cbegin(x); }, seq_t{});
      }
      const_iterator begin() const { return cbegin(); }
      ...
      {
        return tuple_map([](auto &&x)
                         { return std::end(x); }, seq_t{});
      }
      const_iterator cend() const
      {
        return tuple_map([](auto &&x)
                         { return std::cend(x); }, seq_t{});
      }
      const_iterator end() const { return cend(); }
    };
    ...
    struct multiplied
    {
      ...
      template <size_t... Is>
      auto _cbegin(std::index_sequence<Is...>) const
      {
   ...
   */
  template <typename T>
  auto enumerate(T &&range) { return details::enumerated<T>{std::forward<T>(range)}; }
   ...
   */
  template <typename... T>
  auto zip(T &&...ranges) { return details::zipped<T...>{std::forward<T>(ranges)...}; }
  template <typename... T, typename L>
  auto zip_with(T &&...ranges, L &&lambda)
  {
    return transform(zip(std::forward<T>(ranges)...), [lambda](std::tuple<T...> t)
   ...
   */
  template <typename... T>
  auto product(T &&...ranges) { return details::multiplied<T...>{std::forward<T>(ranges)...}; }
  ...
  namespace details
  {
    ...
    auto make_product_impl(std::array<T, N> &arr, std::index_sequence<Is...>)
    {
  ...
  auto make_product(std::array<T, N> &arr) { return details::make_product_impl(arr, std::make_index_sequence<N>{}); }

  /**
   * Create a std::vector from a range
   */
  template <typename R>
  auto make_vector_from_range(R &&r)
  {
