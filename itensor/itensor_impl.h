...
void allocReal(ITensor &T, IntArray const &inds);
...
ITensor::ITensor(Index const &i1, Inds const &...inds) : is_(i1, inds...)
{
    IF_USESCALE(scale_ = LogNum(1.);)
}

template <typename... Inds>
ITensor::ITensor(QN q, Index const &i1, Inds const &...inds)
{
    *this = ITensor(q, IndexSet(i1, inds...));
}

template <class DataType>
ITensor::ITensor(IndexSet iset, DataType &&dat, LogNum const &scale)
    : is_(std::move(iset)),
      store_(newITData<stdx::decay_t<DataType>>(std::move(dat)))
{
    IF_USESCALE(scale_ = scale;)
    static_assert(
        std::is_rvalue_reference<decltype(std::forward<DataType>(dat))>::value,
        "Error: cannot pass lvalues to ITensor(...,DataType&& dat,...) "
        "constructor");
}

template <typename IV, typename... IVs>
auto ITensor::eltC(IV const &iv1, IVs &&...ivs) const
    -> stdx::if_compiles_return<Cplx, decltype(iv1.index), decltype(iv1.val)>
{
    constexpr size_t size = sizeof...(ivs) + 1;
    auto vals = std::array<IndexVal, size>{
        {static_cast<IndexVal>(iv1), static_cast<IndexVal>(ivs)...}};
...
auto ITensor::eltC(std::vector<Int> const &ints) const
    -> stdx::enable_if_t<std::is_integral<Int>::value, Cplx>
{
    ...
    if (size != size_t(inds().order())) {
        ...
        Error(tinyformat::format(
            "Wrong number of ints passed to elt/eltC (expected %d, got %d)",
            inds().order(), size));
    }
...
auto ITensor::eltC(Int iv1, Ints... ivs) const
...
Real ITensor::elt(IVals &&...ivs) const
{
    if (itensor::isComplex(*this))
        Error(
            "Cannot call .elt(...) on an ITensor with complex storage. Please "
            "use .eltC(...) instead");
...
template <typename... IVals>
Real ITensor::real(IVals &&...ivs) const
{
    return elt(std::forward<IVals>(ivs)...);
}

template <typename... IVals>
Cplx ITensor::cplx(IVals &&...ivs) const
{
...
template <typename IndexValT, typename Iter, typename IV>
auto getVals(Iter it, Cplx &z, IV const &iv)
...
void getVals(Iter it, Cplx &z, Cplx const &w)
{
    z = w;
}

template <typename IndexValT, typename Iter, typename IV, typename... Rest>
auto getVals(Iter it, Cplx &z, IV const &iv, Rest &&...rest)
    -> stdx::if_compiles_return<void, decltype(iv.index), decltype(iv.val)>
{
...
bool getVals(Iter it, Cplx &z, Cplx w, Rest &&...rest)
{
...
template <typename IntT, typename Iter, typename Arg>
auto getInts(Iter it, Cplx &z, Arg const &iv)
...
template <typename IntT, typename Iter, typename Arg>
auto getInts(Iter it, Cplx &z, Arg const &w)
    -> stdx::enable_if_t<std::is_convertible<Arg, Cplx>::value, void>
{
    z = w;
}

template <typename IntT, typename Iter, typename Int, typename... Rest>
auto getInts(Iter it, Cplx &z, Int const &w, Rest &&...rest)
...
template <typename IntT, typename Iter, typename Int, typename... Rest>
auto getInts(Iter it, Cplx &z, Int w, Rest &&...rest)
...
void checkEltFluxInts(ITensor const &A, Ints const &ints)
{
    if (hasQNs(A)) {
        QN elt_flux;
        auto indsA = inds(A);
        ...
        if (elt_flux != flux(A)) {
            ...
            Error(
                "In .set, cannot set element with flux different from ITensor "
                "flux");
        }
    }
    return;
...
auto ITensor::set(IV const &iv1, VArgs &&...vargs)
    -> stdx::if_compiles_return<void, decltype(iv1.index), decltype(iv1.val)>
{
    ...
    Cplx z;
    detail::getVals<IndexVal>(vals.begin(), z, iv1,
...
auto ITensor::set(Int iv1, VArgs &&...vargs)
    -> stdx::enable_if_t<std::is_integral<Int>::value, void>
{
    static constexpr auto size = 1 + (sizeof...(vargs) - 1);
    auto ints = IntArray(size, 0);
    Cplx z;
    detail::getInts<Int>(ints.begin(), z, iv1,
                         std::forward<VArgs &&>(vargs)...);
    if (size != size_t(inds().order())) {
        println("---------------------------------------------");
        println("Tensor indices = \n", inds(), "\n");
        ...
        println("---------------------------------------------");
        Error(tinyformat::format(
            "Wrong number of ints passed to set (expected %d, got %d)",
            inds().order(), size));
    }
    // TODO: if !store_ and !is_real, call allocCplx instead
    // and move this line after check for is_real
...
ITensor &ITensor::generate(Func &&f)
{
    if (not this->store()) {
        ...
        if (std::is_same<RetType, Real>::value) {
            ...
            Error(
                "generate: generator function must return Real or Cplx scalar "
                "value");
        }
    }
    fixBlockDeficient();
...
template <typename Func>
const ITensor &ITensor::visit(Func &&f) const
{
    doTask(VisitIT<decltype(f)>{std::forward<Func>(f), LogNum{scale().real0()}},
           store_);
    return *this;
}
...
template <typename... IVals>
ITensor IndexValsToITensor(IndexVal const &iv1, IVals const &...rest)
{
...
template <typename... IVals>
ITensor setElt(Real el, IndexVal const &iv1, IVals const &...rest)
{
...
template <typename... IVals>
ITensor setElt(Cplx el, IndexVal const &iv1, IVals const &...rest)
{
    auto D = detail::IndexValsToITensor(iv1, rest...);
    D.set(iv1, rest..., el);
    return D;
...
template <typename... IVals>
ITensor setElt(IndexVal const &iv1, IVals const &...rest)
{
...
//
template <>
inline Real eltT(ITensor A)
{
    return A.elt();
}

template <>
inline Complex eltT(ITensor A)
{
    return A.eltC();
}
...
ITensor::storage_ptr readType(std::istream &s, CtrArgs &&...args)
{
...
template <typename T>
ITensor::storage_ptr h5_readStore(h5::group g, std::string const &name)
{
...
inline const char *typeNameOf(Write const &) { return "Write"; }

template <typename D>
auto doTask(Write &W, D const &d)
...
struct H5Write {
    ...
    H5Write(h5::group &parent_, std::string const &name_)
        : parent(parent_), name(name_)
    {
    }
};
inline const char *typeNameOf(H5Write const &) { return "H5Write"; }

template <typename D>
auto doTask(H5Write &W, D const &d)
    -> stdx::if_compiles_return<void, decltype(itensor::h5_write(W.parent,
                                                                 W.name, d))>
{
...
{
    if (block_ind.size() != size_t(T.order()))
        Error("Mismatched number of indices and ITensor order");
    if (not T.store()) {
        QN q;
...
ITensor diagTensor(Container const &C, Index const &i1, Inds &&...inds)
{
    Global::warnDeprecated(
        "diagTensor(Container,Index,...) is deprecated in favor of "
        "diagITensor(Container,Index,...)");
...
{
    Global::warnDeprecated(
        "randomTensor(Index,...) is deprecated in favor of "
        "randomITensor(Index,...)");
...
{
    Global::warnDeprecated(
        "randomTensorC(Index,...) is deprecated in favor of "
        "randomITensorC(Index,...)");
...
{
    Global::warnDeprecated(
        "randomTensor(QN,Index,...) is deprecated in favor of "
        "randomITensor(QN,Index,...)");
...
{
    Global::warnDeprecated(
        "randomTensorC(QN,Index,...) is deprecated in favor of "
        "randomITensorC(QN,Index,...)");
    return randomITensorC(q, i1, std::forward<Inds>(inds)...);
}

template <typename... Inds>
ITensor reindex(ITensor const &cT, Index o1, Index n1, Inds... indxs)
{
    Error(
        "Error: reindex(ITensor,Index,Index,...) is deprecated in favor of "
        "replaceInds(ITensor,Index,Index,...)");
    return ITensor();
}
