...
namespace itensor
{
                   ...
                   IndexSet const &ismatch)
    {
        auto ilocs = indexPositions(*this, ismatch);
        for (auto i : ilocs)
            parent::index(i).removeTags(tsremove);
#ifdef DEBUG
        checkIndexSet(*this);
#endif
        return *this;
                    ...
                    IndexSet const &ismatch)
    {
        auto ilocs = indexPositions(*this, ismatch);
        for (auto i : ilocs)
            parent::index(i).replaceTags(tsold, tsnew);
#ifdef DEBUG
        checkIndexSet(*this);
#endif
        return *this;
    ...
    {
        for (auto &J : *this)
            if ((J.primeLevel() == plold) && hasTags(J, tsmatch))
                J.setPrime(plnew);
#ifdef DEBUG
        checkIndexSet(*this);
#endif
    ...
    {
        if (order(is) > 1)
            Error("In findIndex: more than one Index found, consider using findInds instead");
        else if (order(is) == 1)
            return is(1);
        return Index();
    }
