...
class TooBigForReal : public ITError
{
   public:
    ...
    TooBigForReal(const std::string &message) : Parent(message) {}
};
...
class TooSmallForReal : public ITError
{
   public:
    ...
    TooSmallForReal(const std::string &message) : Parent(message) {}
};

bool inline equal(Real x, Real y, Real eps = 1E-12)
{
    auto ax = std::fabs(x);
    auto ay = std::fabs(y);
...
class LogNum
{
   ...
   public:
    ...
    LogNum() : lognum_(NAN), sign_(1) {}

    explicit LogNum(Real r)
    {
        if (r == 0) {
            sign_ = 0;
            lognum_ = 0;
    ...
    LogNum(Real lognum, int sign) : lognum_(lognum), sign_(sign)
    {
#ifdef DEBUG
    ...
    bool isRealZero() const { return (sign_ == 0 || lognum_ < -maxlogdouble); }
    ...
    bool isTooBigForReal() const { return (lognum_ > maxlogdouble); }

    bool isTooSmallForReal() const { return (lognum_ < -maxlogdouble); }

    bool friend inline isnan(const LogNum &L) { return std::isnan(L.lognum_); }
    ...
    {
        try {
            *this = LogNum(this->real() + other.real());
    ...
    bool operator==(const LogNum &other) const
    {
    ...
    bool approxEquals(const LogNum &other) const
    {
        return (sign_ == other.sign_) &&
               (std::fabs(lognum_ - other.lognum_) < LogNum_Accuracy);
    }
    ...
    LogNum &operator/=(Real other) { return *this /= LogNum(other); }

    LogNum operator-() const
    {
        LogNum res(*this);
        res.sign_ *= -1;
        return res;
    ...
    LogNum operator/(const LogNum &other) const
    {
        LogNum res(*this);
        res /= other;
        return res;
    ...
    bool operator>=(const LogNum &other) const { return !(operator<(other)); }
    ...
    bool magnitudeLessThan(const LogNum &other) const
    {
...
LogNum inline operator*(LogNum L, Real r)
{
    L *= r;
    return L;
}
LogNum inline operator*(Real r, LogNum L)
{
    L *= r;
    return L;
}

LogNum inline sqrt(LogNum L)
{
    if (L.sign() < 0) Error("Negative LogNum in sqrt");
    return L.pow(0.5);
}

inline std::ostream &operator<<(std::ostream &s, LogNum const &N)
{
    s << "LogNum(" << N.logNum() << ",";
    if (N.sign() == 0)
        s << "0)";
