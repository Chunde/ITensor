...
namespace itensor
{
    ...
    operator>(Index const &i1, Index const &i2)
    {
        ...
        {
            if (id(i1) == id(i2))
                return primeLevel(i1) > primeLevel(i2);
            return id(i1) > id(i2);
        }
...
#ifdef DEBUG
        if (!index)
            Error("IndexVal initialized with default initialized Index");
    ...
    public:
        ...
        explicit IQIndexDat(storage const &ind_qn)
            : iq_(ind_qn)
        {
        }

        explicit IQIndexDat(storage &&ind_qn)
            : iq_(std::move(ind_qn))
        {
        }
...
#define IQINDEX_CHECK_NULL \
    if (pd == 0)           \
        throw ITError("IQIndex storage unallocated");
#else
#define IQINDEX_CHECK_NULL
#endif
...
#ifdef DEBUG
            if (dir(i) != dir(j))
                Error("In directSum(Index, Index), input indices must have same arrow direction");
#endif
...
#ifdef DEBUG
            if (siq.empty())
                Error("siq is empty in plussers");
#endif
            ...
            QN const &Q)
    {
        ...
        if (not hasQNs(I))
            Error("Index does not contain any QN blocks");
        println("I = ", I);
        println("Q = ", Q);
    ...
    {
        if (!bool(*this))
            Error("Index::write: Index is default initialized");
