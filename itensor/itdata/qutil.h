...
class IndexDim
{
   ...
   public:
    IndexDim(IndexSet const &is, Indexable const &ind) : is_(is), ind_(ind) {}

    size_t size() const { return is_.order(); }

    // size_t
    // operator[](size_t j) const { return (is_[j])[dim(ind_[j]]); }
    size_t operator[](size_t j) const { return (is_[j]).blocksize0(ind_[j]); }
};

template <typename Indexable>
auto make_indexdim(IndexSet const &is,
                   Indexable const &ind) -> IndexDim<Indexable>
{
    return IndexDim<Indexable>(is, ind);
}

template <typename T>
int getBlockLoc(QDense<T> const &d, Block const &block_ind)
{
...
auto getBlock(BlockSparse &d, IndexSet const &is, Block const &block_ind)
    -> stdx::if_compiles_return<decltype(makeDataRange(d.data(), d.size())),
                                decltype(d.offsets)>
{
...
#ifdef DEBUG
    if (is.order() != r)
        Error("Mismatched size of IndexSet and block_ind in getBlock");
#endif
...
template <typename BlockSparseA, typename BlockSparseB>
std::tuple<BlockOffsets, size_t, std::vector<std::tuple<Block, Block, Block>>>
getContractedOffsets(BlockSparseA const &A, IndexSet const &Ais,
                     BlockSparseB const &B, IndexSet const &Bis,
                     IndexSet const &Cis)
{
    auto rA = order(Ais);
    auto rB = order(Bis);
...
#pragma omp for schedule(dynamic)
        for (int i = 0; i < (int)A.offsets.size(); ++i) {
            ...
            // Begin computing elements of Cblock(=destination of this
            // block-block contraction)
            for (auto iA : range(rA))
...
#ifdef ITENSOR_USE_OMP
                blockContractions_thread[thread_num].push_back(
                    std::make_tuple(aio.block, bio.block, Cblockind));
#else
                blockContractions.push_back(
                    std::make_tuple(aio.block, bio.block, Cblockind));
#endif
...
#ifdef ITENSOR_USE_OMP
                Cblocksizes_thread[thread_num].push_back(
                    make_blof(Cblockind, blockDim));
#else
    ...
    auto newCend =
        std::unique(Cblocksizes.begin(), Cblocksizes.end(),
                    [](auto a, auto b) { return a.block == b.block; });
...
void _loopContractedBlocks(
    QDense<TA> const &A, IndexSet const &Ais, QDense<TB> const &B,
    IndexSet const &Bis, QDense<TC> &C, IndexSet const &Cis,
    ...
    Callable &callback)
{
    for (auto const &[Ablockind, Bblockind, Cblockind] : blockContractions) {
        ...
        callback(ablock, Ablockind, bblock, Bblockind, cblock, Cblockind,
                 Cblockloc);
    }
}

template <typename TA, typename TB, typename TC, typename Callable>
void _loopContractedBlocksOMP(
    QDense<TA> const &A, IndexSet const &Ais, QDense<TB> const &B,
    IndexSet const &Bis, QDense<TC> &C, IndexSet const &Cis,
    std::vector<std::tuple<Block, Block, Block>> const &blockContractions,
    Callable &callback)
{
    ...
    std::sort(std::begin(blockContractionsSorted),
              std::end(blockContractionsSorted), sortBlockContractions);
    ...
    if (ncontractions != n)
        Error("Wrong number of contractions in QDense contraction");
#endif
...
#pragma omp parallel for schedule(dynamic)
    for (decltype(nnzblocksC) i = 0; i < nnzblocksC; i++) {
        ...
        for (auto j = offset[i]; j < offset[i] + nrepeat[i]; j++) {
            auto const &[Ablockind, Bblockind, Cblockind] =
                blockContractionsSorted[j];
            auto ablock = getBlock(A, Ais, Ablockind);
            auto bblock = getBlock(B, Bis, Bblockind);
            auto cblock = getBlock(C, Cis, Cblockind);
            auto Cblockloc = getBlockLoc(C, Cblockind);
...
{
#ifdef ITENSOR_USE_OMP
    _loopContractedBlocksOMP(A, Ais, B, Bis, C, Cis, blockContractions,
                             callback);
#else
...
// (faster) version
template <typename TA, typename TB, typename TC, typename Callable>
void loopContractedBlocks(TA const &A, IndexSet const &Ais, TB const &B,
                          IndexSet const &Bis, TC &C, IndexSet const &Cis,
                          Callable &callback)
{
    auto rA = Ais.order();
    auto rB = Bis.order();
    ...
    for (auto const &aio : A.offsets) {
        ...
        for (; couB.notDone(); ++couB) {
            ...
            callback(ablock, aio.block, bblock, Bblockind, cblock, Cblockind);
        }  // for couB
    }  // for A.offsets
}
