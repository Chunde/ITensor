...
namespace itensor
{
...
#ifdef DEBUG
        if (isTrivial(P))
            Error("Calling permuteIQ for trivial Permutation");
#endif
        auto r = Ais.order();
              ...
              bool own_data)
    {
        ...
        for (auto n : range(dr))
        {
            ...
            {
                jc = n;
                for (auto n : range(1, cr))
                    newind.nextIndex(Cis[n]);
            }
            else
            {
        ...
        for (auto io : d.offsets) // loop over non-zero blocks
        {
            ...
            for (auto o : range(C.store_))
            {
                ...
                if (size_type(br.block) != size_type(n))
                    continue;
