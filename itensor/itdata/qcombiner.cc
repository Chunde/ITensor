...
const char *typeNameOf(QCombiner const &d) { return "QCombiner"; }

void read(std::istream &s, QCombiner &dat)
{
    Range range;
...
void write(std::ostream &s, QCombiner const &dat)
{
    itensor::write(s, dat.range());
    itensor::write(s, dat.store());
}

std::ostream &operator<<(std::ostream &s, QCombiner const &dat)
{
...
Cplx doTask(GetElt const &g, QCombiner const &c)
{
    if (g.inds.size() != 0)
        Error("GetElt not defined for non-scalar QCombiner storage");
    return Cplx(1., 0.);
}

void permuteIQ(Permutation const &P, IndexSet const &Ais, QDenseReal const &dA,
               IndexSet &Bis, QDenseReal &dB)
{
#ifdef DEBUG
...
IndexSet replaceInd(IndexSet const &is, long loc, Index const &replacement)
{
...
template <typename T>
void combine(QDense<T> const &d, QCombiner const &C, IndexSet const &dis,
             IndexSet const &Cis, IndexSet &Nis, ManageStore &m)
{
#ifdef DEBUG
    ...
    auto combined = [&dperm, ncomb](size_type i) {
        return dperm[i] < long(ncomb);
    };
    ...
    for (auto i : range(dr))
        if (!combined(i)) newind.nextIndex(dis[i]);
    Nis = newind.build();
    ...
    for (auto io : d.offsets)   // loop over non-zero blocks
    {
        ...
        for (auto i : range(dr)) {
            if (combined(i))
                cblock[dperm[i]] = io.block[i];
            else
                nblock[nu++] = io.block[i];
        }
...
template <typename T>
void uncombine(QDense<T> const &d, QCombiner const &C, IndexSet const &dis,
               IndexSet const &Cis, IndexSet &Nis, ManageStore &m,
               bool own_data)
{
    using size_type = decltype(order(dis));
    ...
    for (auto io : d.offsets)  // loop over non-zero blocks
    {
        ...
        for (auto o : range(C.store_)) {
            ...
            for (auto m : range(1 + jc, dr))
                nblock[ncomb + m - 1] = io.block[m];
...
template <typename T>
void doTask(Contract &C, QDense<T> const &d, QCombiner const &cmb,
            ManageStore &m)
{
...
template void doTask(Contract &, QDense<Real> const &, QCombiner const &,
                     ManageStore &);
template void doTask(Contract &, QDense<Cplx> const &, QCombiner const &,
                     ManageStore &);

template <typename T>
void doTask(Contract &C, QCombiner const &cmb, QDense<T> const &d,
            ManageStore &m)
{
...
template void doTask(Contract &, QCombiner const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(Contract &, QCombiner const &, QDense<Cplx> const &,
                     ManageStore &);
