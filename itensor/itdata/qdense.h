...
namespace itensor
{
    ...
    template <typename T>
    bool constexpr isReal(QDense<T> const &t) { return std::is_same<T, Real>::value; }

    template <typename T>
    bool constexpr isCplx(QDense<T> const &t) { return std::is_same<T, Cplx>::value; }
    ...
    Data inline realData(QDenseCplx &d) { return Data(reinterpret_cast<Real *>(d.data()), 2 * d.size()); }

    Datac inline realData(QDenseCplx const &d) { return Datac(reinterpret_cast<const Real *>(d.data()), 2 * d.size()); }
    ...
    {
        if (!d)
            throw ITError("Empty storage in QDense apply");
    ...
    {
        if (!d)
            throw ITError("Empty storage in QDense visit");
    ...
    template <typename T>
    bool constexpr doTask(CheckComplex, QDense<T> const &d) { return isCplx(d); }
    ...
    auto inline constexpr doTask(StorageType const &S, QDenseReal const &d) -> StorageType::Type { return StorageType::QDenseReal; }

    auto inline constexpr doTask(StorageType const &S, QDenseCplx const &d) -> StorageType::Type { return StorageType::QDenseCplx; }
                          ...
                          Indexable const &ind) const
    {
        ...
        if (r == 0)
            return std::make_tuple(store.data(), Block(0), 0);
#ifdef DEBUG
        if (is.order() != r)
        {
        ...
        {
#ifdef DEBUG
            if (size_t(boff + eoff) >= store.size())
                Error("get_elt out of range");
#endif
