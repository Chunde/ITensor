...
namespace itensor
{
    ...
    template <typename T>
    bool constexpr isReal(QDiag<T> const &t) { return std::is_same<T, Real>::value; }

    template <typename T>
    bool constexpr isCplx(QDiag<T> const &t) { return std::is_same<T, Cplx>::value; }
    ...
    Data inline realData(QDiagCplx &d) { return Data(reinterpret_cast<Real *>(d.data()), 2 * d.size()); }

    Datac inline realData(QDiagCplx const &d) { return Datac(reinterpret_cast<const Real *>(d.data()), 2 * d.size()); }
    ...
    template <typename T>
    bool constexpr doTask(CheckComplex, QDiag<T> const &d) { return isCplx(d); }
    ...
    auto inline constexpr doTask(StorageType const &S, QDiagReal const &d) -> StorageType::Type { return StorageType::QDiagReal; }

    auto inline constexpr doTask(StorageType const &S, QDiagCplx const &d) -> StorageType::Type { return StorageType::QDiagCplx; }
                    ...
                    Indexable const &block_ind)
    {
        ...
        for (auto n : range(is))
        {
            for (auto j : range(block_ind[n]))
                starts[n] += is[n].blocksize0(j);
