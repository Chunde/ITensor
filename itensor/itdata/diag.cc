...
const char *typeNameOf(DiagReal const &d) { return "DiagReal"; }
const char *typeNameOf(DiagCplx const &d) { return "DiagCplx"; }

template <typename T>
Cplx doTask(GetElt const &g, Diag<T> const &d)
{
...
void contractDiagDense(Diag<T1> const &d, IndexSet const &dis,
                       Labels const &dind, Dense<T2> const &t,
                       IndexSet const &tis, Labels const &tind,
                       Labels const &Nind, IndexSet const &Nis, ManageStore &m)
{
    ...
    if (t_has_uncontracted) {
        ...
        if (d.allSame()) {
            ...
            contractDiagPartial(dref, dind, Tref, tind, Nref, Nind);
        } else {
    ...
    } else  // all inds of t contracted with d
    {
        ...
        if (d.allSame()) {
            auto dref = UnifVecWrapper<decltype(d.val)>(d.val, d.length);
            contractDiagFull(dref, dind, Tref, tind, Nref, Nind);
        } else {
...
template <typename T>
bool isReplaceDelta(Diag<T> const &d, IndexSet const &dis, Labels const &l)
{
...
template <typename T1, typename T2>
void doTask(Contract &C, Dense<T1> const &t, Diag<T2> const &d, ManageStore &m)
{
...
template void doTask(Contract &, Dense<Real> const &, Diag<Real> const &,
                     ManageStore &);
template void doTask(Contract &, Dense<Real> const &, Diag<Cplx> const &,
                     ManageStore &);
template void doTask(Contract &, Dense<Cplx> const &, Diag<Real> const &,
                     ManageStore &);
template void doTask(Contract &, Dense<Cplx> const &, Diag<Cplx> const &,
                     ManageStore &);

template <typename T1, typename T2>
void doTask(Contract &C, Diag<T1> const &d, Dense<T2> const &t, ManageStore &m)
{
    Labels Lind, Rind, Nind;
    computeLabels(C.Lis, C.Lis.order(), C.Ris, C.Ris.order(), Lind, Rind);
    // TODO: add a case where there is a scaled delta function,
...
template void doTask(Contract &, Diag<Real> const &, Dense<Real> const &,
                     ManageStore &);
template void doTask(Contract &, Diag<Real> const &, Dense<Cplx> const &,
                     ManageStore &);
template void doTask(Contract &, Diag<Cplx> const &, Dense<Real> const &,
                     ManageStore &);
template void doTask(Contract &, Diag<Cplx> const &, Dense<Cplx> const &,
                     ManageStore &);
...
template <typename T1, typename T2>
void add(PlusEQ const &P, Diag<T1> &D1, Diag<T2> const &D2)
{
#ifdef DEBUG
    if (D1.length != D2.length) Error("Mismatched lengths in plusEq");
#endif
    if (D1.allSame() || D2.allSame())
        Error("Diag plusEq allSame case not implemented");
...
template <typename T1, typename T2>
void doTask(PlusEQ const &P, Diag<T1> const &D1, Diag<T2> const &D2,
            ManageStore &m)
{
...
template void doTask(PlusEQ const &, Diag<Real> const &, Diag<Real> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, Diag<Real> const &, Diag<Cplx> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, Diag<Cplx> const &, Diag<Real> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, Diag<Cplx> const &, Diag<Cplx> const &,
                     ManageStore &);

template <typename N, typename T>
void doTask(Fill<N> const &f, Diag<T> const &d, ManageStore &m)
{
...
template <typename T1, typename T2>
void doTask(Mult<T1> const &M, Diag<T2> &D)
{
    doMult(M.x, D);
}
...
void doTask(Mult<Cplx> const &M, Diag<Real> const &D, ManageStore &m)
{
...
template <typename T>
Real doTask(NormNoScale, Diag<T> const &D)
{
...
void doTask(Conj, DiagReal const &d) {}

void doTask(Conj, DiagCplx &d)
{
    if (d.allSame()) {
        d.val = std::conj(d.val);
    } else {
        for (auto &el : d.store) el = std::conj(el);
    }
}

void doTask(TakeReal, DiagReal const &D) {}

void doTask(TakeReal, DiagCplx const &D, ManageStore &m)
{
    if (D.allSame()) {
...
void doTask(TakeImag, DiagReal &D)
{
    D.val = 0.;
    for (auto &el : D.store) el = 0.;
}

void doTask(TakeImag, DiagCplx const &D, ManageStore &m)
{
    if (D.allSame()) {
...
template <typename T>
void doTask(PrintIT &P, Diag<T> const &d)
{
    auto type = std::is_same<T, Real>::value ? "Real" : "Cplx";
    P.printInfo(
        d,
        tinyformat::format("Diag %s%s", type, d.allSame() ? ", all same" : ""),
...
template <class T>
Cplx doTask(SumEls S, Diag<T> const &d)
{
    if (d.allSame()) return Real(minDim(S.is)) * d.val;
    T sum = 0;
    for (const auto &elt : d.store) sum += elt;
    return sum;
}
...
template <typename V>
void doTask(ToDense &R, Diag<V> const &d, ManageStore &m)
{
    auto nd = m.makeNewData<Dense<V>>(dim(R.is), 0.);
    auto Nref = makeTenRef(nd->data(), nd->size(), &R.is);
    long tot_stride = 0;  // total strides
    for (auto i : range(length(R.is))) tot_stride += R.is.stride(i);
    if (d.allSame()) {
        for (auto i : range(d.length)) Nref[i * tot_stride] = d.val;
    } else {
        for (auto i : range(d.length)) Nref[i * tot_stride] = d.store[i];
    }
}
