...
const char *typeNameOf(Combiner const &d) { return "Combiner"; }

Cplx doTask(GetElt const &g, Combiner const &c)
{
    if (g.inds.size() != 0)
        Error("GetElt not defined for non-scalar Combiner storage");
    return Cplx(1., 0.);
}

Real doTask(NormNoScale, Combiner const &d) { return 0; }

void doTask(Conj, Combiner const &d) {}

template <typename T>
void permuteStore(Dense<T> const &d, IndexSet const &dis, Permutation const &P,
                  ManageStore &m)
{
...
void combine(Storage const &d, IndexSet const &dis, IndexSet const &Cis,
             IndexSet &Nis, ManageStore &m)
{
    ...
    } else  // combining
    {
        ...
        if (contig_sameord) {
            ...
            long i = 0;
            for (auto j : range(J1)) newind.setIndex(i++, dis[j]);
...
template <typename V>
void doTask(Contract &C, Dense<V> const &d, Combiner const &cmb, ManageStore &m)
{
    combine(d, C.Lis, C.Ris, C.Nis, m);
}
template void doTask(Contract &, DenseReal const &, Combiner const &,
                     ManageStore &);
template void doTask(Contract &, DenseCplx const &, Combiner const &,
                     ManageStore &);

template <typename V>
void doTask(Contract &C, Combiner const &cmb, Dense<V> const &d, ManageStore &m)
{
    combine(d, C.Ris, C.Lis, C.Nis, m);
    if (!m.newData()) m.assignPointerRtoL();
}
template void doTask(Contract &, Combiner const &, DenseReal const &,
                     ManageStore &);
template void doTask(Contract &, Combiner const &, DenseCplx const &,
                     ManageStore &);

bool doTask(CheckComplex, Combiner const &d) { return false; }

void doTask(PrintIT &P, Combiner const &d) { P.printInfo(d, "Combiner"); }

QN doTask(CalcDiv const &C, Combiner const &d) { return QN{}; }
