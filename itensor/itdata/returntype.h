...
NoneType testRetImpl(stdx::choice<3>, Task &t, Storage &s, ManageStore &m)
{
    return NoneType{};
}
template <typename Task, typename Storage>
auto testRetImpl(stdx::choice<2>, Task &t, Storage &s,
                 ManageStore &m) -> RetOrNone<decltype(doTask(t, s))>
{
    using Ret = RetOrNone<decltype(doTask(t, s))>;
    return Ret{};
}
template <typename Task, typename Storage>
auto testRetImpl(stdx::choice<1>, Task &t, Storage &s,
                 ManageStore &m) -> RetOrNone<decltype(doTask(t, s, m))>
{
...
struct TestRet {
    ...
    Storage &s;
    ManageStore &m;
    TestRet(Task &t_, Storage &s_, ManageStore &m_) : t(t_), s(s_), m(m_) {}
    auto operator()() -> decltype(testRetImpl(stdx::select_overload{}, t, s, m))
    {
...
template <typename Task, typename TList>
using DoTaskReturn =
    typename detail::GetRType<stdx::remove_reference_t<Task>, TList>::RType;
