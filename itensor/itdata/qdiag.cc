...
namespace itensor
{
    ...
    doTask(CalcDiv const &C, QDiag<T> const &D)
    {
                                   ...
                                   IntArray const &block)
        {
            ...
            if (q != d)
                Error("Diagonal elements of QDiag ITensor would have inconsistent divergence");
        };
    ...
    {
        if (D.allSame())
            return std::sqrt(std::norm(D.val)) * std::sqrt(D.length);
        auto d = realData(D);
    ...
    doTask(PrintIT &P, QDiag<T> const &d)
    {
        ...
        auto printBlock = [&P, &d, ord, scalefac](size_t nb,
                                                  size_t ne,
                                                  IntArray const &block)
        {
                   ...
                   ManageStore &m)
    {
...
#ifdef DEBUG
        if (Dis.order() == 0)
            Error("QDiag order 0 case not handled");
#endif
        ...
        if (T_has_uncontracted)
        {
            ...
            auto do_contract =
                [&D, &Dis, &Tis, &Cis, &DL, &TL, &CL](DataRange<const VT> tblock, Block const &Tblockind,
        ...
        else
        {
            ...
            auto do_contract =
                [&D, &Dis, &Tis, &DL, &TL, &CL](DataRange<const VT> tblock, Block const &Tblockind,
                                                DataRange<const VD> dblock, Block const &Dblockind,
                                                DataRange<VC> cblock, Block const &Cblockind)
            {
                Range Trange;
    ...
    {
        if ((order(dis) == 2) && d.allSame() && (d.val == 1.) && (dim(dis[0]) == dim(dis[1])))
        {
