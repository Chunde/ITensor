...
const char *typeNameOf(QDiagReal const &d) { return "QDiagReal"; }
const char *typeNameOf(QDiagCplx const &d) { return "QDiagCplx"; }

template <typename T, class F>
void loopDiagBlocks(QDiag<T> const &D, IndexSet const &is, F const &callback)
{
    auto r = order(is);
...
template <typename T>
QN doTask(CalcDiv const &C, QDiag<T> const &D)
{
...
#ifdef DEBUG
    auto checkBlock = [&d, &C](size_t nb, size_t ne, IntArray const &block) {
        auto q = QN();
        for (auto n : range(C.is)) {
        ...
        if (q != d)
            Error(
                "Diagonal elements of QDiag ITensor would have inconsistent "
                "divergence");
    };
...
size_t computeLength(IndexSet const &is)
{
...
template <typename T>
QDiag<T>::QDiag(IndexSet const &is) : length(computeLength(is))
{
...
template <typename T>
QDiag<T>::QDiag(IndexSet const &is, T val_)
    : val(val_), length(computeLength(is))
{
#ifdef DEBUG
    doTask(CalcDiv{is}, *this);
#endif
...
template <typename T>
Cplx doTask(GetElt &G, QDiag<T> const &D)
{
    auto r = G.is.order();
#ifdef DEBUG
    if (G.is.order() != decltype(r)(G.inds.size())) {
        printfln("is.order() = %d, ind.size() = %d", G.is.order(),
                 G.inds.size());
    ...
    for (auto i : G.inds)
        if (i != first_i) return 0;

    if (D.allSame()) return D.val;

    auto n = G.inds[0];
#ifdef DEBUG
    if (n > (decltype(n))D.size())
        Error("index out of range in getElt(QDiag..)");
#endif
    for (auto i : range(1, r)) {
...
template <typename T>
Cplx doTask(SumEls, QDiag<T> const &d)
{
    if (d.allSame()) {
...
template <typename T>
void doTask(Mult<Real> const &M, QDiag<T> &D)
{
    if (D.allSame()) {
        D.val *= M.x;
    } else {
...
void doTask(Mult<Cplx> const &M, QDiag<Cplx> &D)
{
    if (D.allSame()) {
        D.val *= M.x;
    } else {
...
void doTask(Mult<Cplx> const &M, QDiag<Real> const &d, ManageStore &m)
{
...
template <typename T>
Real doTask(NormNoScale, QDiag<T> const &D)
{
...
template <typename T>
void doTask(PrintIT &P, QDiag<T> const &d)
{
    P.s << tinyformat::format("QDiag%s%s\n", typeName<T>(),
                              d.allSame() ? " (all same)" : "");
    Real scalefac = 1.0;
    if (!P.x.isTooBigForReal())
        scalefac = P.x.real0();
    else
        P.s << "(omitting too large scale factor)\n";
    ...
    auto printBlock = [&P, &d, ord, scalefac](size_t nb, size_t ne,
                                              IntArray const &block) {
        // print indices for this block
        for (auto i : range(ord)) {
            if (i > 0) P.s << ", ";
            P.s << P.is[i].blocksize0(block[i]) << "<" << P.is[i].dir() << ">"
                << P.is[i].qn(1 + block[i]);
        }
        P.s << "\n";
...
class UnifVecWrapper
{
   ...
   public:
    ...
    T operator()(size_t j) const { return val_; }
};

template <typename VD, typename VT>
void blockDiagDense(QDiag<VD> const &D, IndexSet const &Dis, Labels const &DL,
                    QDense<VT> const &T, IndexSet const &Tis, Labels const &TL,
                    IndexSet const &Cis, Labels const &CL, ManageStore &m)
{
    ...
    if (T_has_uncontracted) {
        ...
        auto do_contract =
            [&D, &Dis, &Tis, &Cis, &DL, &TL, &CL](
                ...
                if (D.allSame()) {
                    ...
                    contractDiagPartial(dref, DL, Tref, TL, Cref, CL, starts);
                } else {
        ...
        loopContractedBlocks(T, Tis, D, Dis, C, Cis, do_contract);
    } else {
        ...
        auto do_contract =
            [&D, &Dis, &Tis, &DL, &TL, &CL](
                ...
                if (D.allSame()) {
                    auto dref = UnifVecWrapper<decltype(D.val)>(D.val, Dsize);
                    contractDiagFull(dref, DL, Tref, TL, Cref, CL, starts);
                } else {
                    auto Dref = makeVecRef(dblock.data(), Dsize);
                    contractDiagFull(Dref, DL, Tref, TL, Cref, CL, starts);
                }
...
template <typename T>
bool isReplaceDelta(QDiag<T> const &d, IndexSet const &dis, Labels const &l)
{
    if ((order(dis) == 2) && d.allSame() && (d.val == 1.) &&
        (dim(dis[0]) == dim(dis[1]))) {
...
void doTask(Contract &C, QDiag<T1> const &d, QDense<T2> const &t,
            ManageStore &m)
{
    ...
    if (isReplaceDelta(d, C.Lis, Lind)) {
        ...
        blockDiagDense(d, C.Lis, Lind, t, C.Ris, Rind, C.Nis, Nind, m);
    }
}
template void doTask(Contract &Con, QDiag<Real> const &A, QDense<Real> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDiag<Cplx> const &A, QDense<Real> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDiag<Real> const &A, QDense<Cplx> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDiag<Cplx> const &A, QDense<Cplx> const &B,
                     ManageStore &m);

template <typename T1, typename T2>
void doTask(Contract &C, QDense<T1> const &t, QDiag<T2> const &d,
            ManageStore &m)
{
    Labels Lind, Rind, Nind;
    computeLabels(C.Lis, C.Lis.order(), C.Ris, C.Ris.order(), Lind, Rind);
    ...
    if (isReplaceDelta(d, C.Ris, Rind)) {
        ...
        blockDiagDense(d, C.Ris, Rind, t, C.Lis, Lind, C.Nis, Nind, m);
    }
}
template void doTask(Contract &Con, QDense<Real> const &A, QDiag<Real> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDense<Cplx> const &A, QDiag<Real> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDense<Real> const &A, QDiag<Cplx> const &B,
                     ManageStore &m);
template void doTask(Contract &Con, QDense<Cplx> const &A, QDiag<Cplx> const &B,
                     ManageStore &m);

template <typename T>
bool doTask(IsDense, QDiag<T> const &d)
{
    return false;
}
...
template <typename V>
void doTask(RemoveQNs &R, QDiag<V> const &qd, ManageStore &m)
{
    // auto *pd = qd.data();
    if (qd.allSame())
        m.makeNewData<Diag<V>>(qd.length, qd.val);
    else
        m.makeNewData<Diag<V>>(qd.begin(), qd.end());
}
template void doTask(RemoveQNs &, QDiag<Real> const &, ManageStore &);
template void doTask(RemoveQNs &, QDiag<Cplx> const &, ManageStore &);

template <typename V>
void doTask(ToDense &R, QDiag<V> const &d, ManageStore &m)
{
    ...
    for (auto const &io : nd->offsets) {
        ...
        for (auto i : range(1, ninds)) {
            if (io.block[i] != io.block[0]) diag_block = false;
        }
        ...
        if (diag_block) {
            ...
            auto aref =
                makeTenRef(nd->data(), io.offset, nd->size(), &dense_range);

            long tot_stride = 0;  // total strides for this block
            for (auto i : range(ninds)) tot_stride += dense_range.stride(i);

            if (d.allSame()) {
                auto block_size = dense_range.extent(
                    0);  // Assume all dimensions of the block are
                         // the same (maybe this is not always true?)
                for (auto i : range(block_size)) aref[i * tot_stride] = d.val;
            } else {
