...
{
    static_assert(containsType<StorageTypes, stdx::decay_t<T>>{},
   ...
   private:
    PData clone() const final { return std::make_shared<ITWrap<T>>(d); }

    void plugInto(FuncBase &f) const final { f.applyTo(d); }

    void plugInto(FuncBase &f) final { f.applyTo(d); }
};

class ManageStore
{
    enum Action { None, AssignNewData, AssignPointerRtoL };
   ...
   public:
    ...
    ManageStore(PData *pparg1) : pparg1_(pparg1) {}

    ManageStore(PData *pparg1, PData *pparg2) : pparg1_(pparg1), pparg2_(pparg2)
    {
    }
    ...
    ~ManageStore() { updateArg1(); }

    ManageStore(const ManageStore &) = delete;

    ManageStore &operator=(const ManageStore &) = delete;

    bool hasPArg1() const { return bool(pparg1_); }

    bool hasPArg2() const { return bool(pparg2_); }

    void setparg1(PData *pparg1) { pparg1_ = pparg1; }

    void setparg2(PData *pparg2) { pparg2_ = pparg2; }
    ...
    UniqueRef modifyData();

    // This returns a pointer because if it returned a reference
    // it is too easy to copy the data type by writing
    // auto nd = modifyData(...);
    template <typename T>
    T *modifyData(const T &d);

    // This returns a pointer because if it returned a reference
    // it is too easy to copy the data type by writing
    // auto nd = makeNewData(...);
    template <typename StorageT, typename... Args>
    StorageT *makeNewData(Args &&...args);

    PData &newData() { return nd_; }

    void assignPointerRtoL();

    void pointTo(const PData &p);
...
StorageT *ManageStore::makeNewData(VArgs &&...vargs)
{
    // if(!pparg1_) Error("Can't call makeNewData with const-only access to
    // first arg");
    action_ = AssignNewData;
    auto newdat =
        std::make_shared<ITWrap<StorageT>>(std::forward<VArgs>(vargs)...);
    auto *ret = newdat.get();
    nd_ = std::move(newdat);
    return &(ret->d);
}

void inline ManageStore::assignPointerRtoL()
{
    // if(!pparg2_) Error("No second pointer provided for action
    // AssignPointerRtoL");
    action_ = AssignPointerRtoL;
}

void inline ManageStore::pointTo(const PData &p)
{
    action_ = AssignNewData;
    nd_ = p;
}

void inline ManageStore::updateArg1()
{
    if (!pparg1_) return;
...
ManageStore::UniqueRef inline ManageStore::modifyData()
{
...
template <typename T>
T *ManageStore::modifyData(const T &d)
{
    // if(!pparg1_) Error("Can't modify const data");
    if (!(pparg1_->unique())) {
...
inline ITData &CPData::operator*() { return *p; }

inline const ITData &CPData::operator*() const { return *p; }

inline ITData *CPData::operator->() { return p.get(); }

inline const ITData *CPData::operator->() const { return p.get(); }

template <typename T>
const char *typeNameOf(T const &t)
{
    return "[unknown]";
}
