...
void doTask(PrintStore, const ITLazy &L)
{
    println("Storage type of P is ITLazy");
}
void doTask(PrintStore, const ITReal &R)
{
    println("Storage type of P is ITReal");
}

bool hasResult(const ITLazy &Z) { return Z.hasResult(); }
...
PData evaluate(ITLazy &Z)
{
    ...
    InfArray<int, 12ul> contA(maxr, 0), contB(maxr, 0);
...
void doTask(Contract<Index> &C, const PData &R, const ITLazy &L, ManageStore &m)
{
    if (L.hasResult()) {
        // Eventually handle this in dotask.h?
