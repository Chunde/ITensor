...
namespace itensor
{
    ...
    public:
        ...
        explicit Dense(size_t size) : store(size) { std::fill(store.begin(), store.end(), 0.); }

        explicit Dense(UndefInitializer, size_t size) : store(size) {}

        // This allows a Dense to be constructed from a std::vector.
        // TODO: Does this copy?
        explicit Dense(std::vector<value_type> const &v)
            : store(v.begin(), v.end())
        {
        }
    ...
    template <typename T>
    bool constexpr isReal(Dense<T> const &t) { return std::is_same<T, Real>::value; }

    template <typename T>
    bool constexpr isCplx(Dense<T> const &t) { return std::is_same<T, Cplx>::value; }
    ...
    Data inline realData(DenseCplx &d) { return Data(reinterpret_cast<Real *>(d.data()), 2 * d.size()); }

    Datac inline realData(DenseCplx const &d) { return Datac(reinterpret_cast<const Real *>(d.data()), 2 * d.size()); }
    ...
    template <typename T>
    bool constexpr doTask(CheckComplex, Dense<T> const &d) { return isCplx(d); }
    ...
    auto constexpr inline doTask(StorageType const &S, DenseReal const &d) -> StorageType::Type { return StorageType::DenseReal; }

    auto constexpr inline doTask(StorageType const &S, DenseCplx const &d) -> StorageType::Type { return StorageType::DenseCplx; }
