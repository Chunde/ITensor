...
namespace itensor
{
    ...
    doTask(CalcDiv const &C,
           QDense<T> const &D)
    {
        if (order(C.is) == 0 || D.offsets.empty())
            return QN{};
           ...
           ManageStore &m)
    {
        ...
        auto do_contract =
            [&Con, &Lind, &Rind, &Cind, &betas](DataRange<const VA> ablock, Block const &Ablockind,
           ...
           ManageStore &m)
    {
        ...
        auto do_ncprod =
            [&P, &Aind, &Bind, &Cind](DataRange<const VA> ablock, Block const &Ablockind,
                                      DataRange<const VB> bblock, Block const &Bblockind,
                                      DataRange<VC> cblock, Block const &Cblockind)
        {
            Range Arange,
