...
const char *typeNameOf(QDenseReal const &d) { return "QDenseReal"; }
const char *typeNameOf(QDenseCplx const &d) { return "QDenseCplx"; }

BlOf make_blof(Block const &b, long o)
{
    BlOf B;
    B.block = b;
    B.offset = o;
...
bool operator!=(Block const &b1, Block const &b2) { return !(b1 == b2); }

bool operator<(Block const &l1, Block const &l2)
{
    return std::lexicographical_compare(l1.rbegin(), l1.rend(), l2.rbegin(),
                                        l2.rend());
}
...
struct compBlock {
    bool operator()(const BlOf &bo1, const BlOf &bo2) const
    {
        return bo1.block < bo2.block;
    }
    bool operator()(const BlOf &bo, Block const &blk) const
    {
        return bo.block < blk;
    }
    bool operator()(Block const &blk, const BlOf &bo) const
    {
        return blk < bo.block;
    }
};

QN calcDiv(IndexSet const &is, Block const &block_ind)
{
    QN div;
    for (auto i : range(order(is))) {
        div += is[i].dir() * is[i].qn(1 + block_ind[i]);
    }
    return div;
}

template <typename T>
QN doTask(CalcDiv const &C, QDense<T> const &D)
{
...
template <typename T>
QDense<T>::QDense(IndexSet const &is, QN const &div)
{
...
template <typename T>
QDense<T>::QDense(IndexSet const &is, Blocks const &blocks)
{
...
std::tuple<BlockOffsets, long> getBlockOffsets(IndexSet const &is,
                                               QN const &div)
{
    ...
    for (auto j : range(order(is))) RB.nextIndex(is[j].nblock());
...
template <typename T>
long QDense<T>::updateOffsets(IndexSet const &is, QN const &div)
{
...
template <typename T>
long QDense<T>::updateOffsets(IndexSet const &is, Blocks const &blocks)
{
    offsets.clear();
...
long offsetOf(BlockOffsets const &offsets, Block const &blockind)
{
...
int offsetOfLoc(BlockOffsets const &offsets, Block const &blockind)
{
    auto it =
        std::lower_bound(offsets.begin(), offsets.end(), blockind, compBlock());
    int loc = std::distance(offsets.begin(), it);
    return loc;
}

Cplx doTask(GetElt &G, QDenseReal const &d)
{
    auto *pelt = d.getElt(G.is, G.inds);
    if (pelt) return Cplx(*pelt, 0.);
    return Cplx(0., 0.);
}
Cplx doTask(GetElt &G, QDenseCplx const &d)
{
    auto *pelt = d.getElt(G.is, G.inds);
    if (pelt) return *pelt;
    return Cplx(0., 0.);
...
void setEltImpl(SetElt<E> &S, QDense<T> &d)
{
...
template <typename T>
void doTask(SetElt<Real> &S, QDense<T> &d)
{
...
void doTask(SetElt<Cplx> &S, QDenseCplx &d) { setEltImpl<Cplx, Cplx>(S, d); }

void doTask(SetElt<Cplx> &S, QDenseReal const &d, ManageStore &m)
{
...
template <typename T>
Cplx doTask(SumEls, QDense<T> const &d)
{
    Cplx s = 0.;
...
template <typename T>
void doTask(Mult<Real> const &M, QDense<T> &D)
{
    auto d = realData(D);
...
void doTask(Mult<Cplx> const &M, QDense<Cplx> &d)
{
    for (auto &el : d) el *= M.x;
}

void doTask(Mult<Cplx> const &M, QDense<Real> const &d, ManageStore &m)
{
    auto *nd = m.makeNewData<QDenseCplx>(d.offsets, d.begin(), d.end());
    doTask(M, *nd);
}

void doTask(MakeCplx const &, QDense<Cplx> &d)
{
    // nothing to do
}
void doTask(MakeCplx const &, QDense<Real> const &d, ManageStore &m)
{
    m.makeNewData<QDenseCplx>(d.offsets, d.begin(), d.end());
}

template <typename T>
int doTask(NNZBlocks, QDense<T> const &D)
{
...
template <typename T>
long doTask(NNZ, QDense<T> const &D)
{
    return D.store.size();
}
...
template <typename T>
void doTask(Fill<T> const &F, QDense<T> &d)
{
    stdx::fill(d, F.x);
}
...
void doTask(Fill<FT> const &F, QDense<DT> const &d, ManageStore &m)
{
...
void doTask(Conj, QDenseCplx &d)
{
    for (auto &el : d) applyConj(el);
}

void doTask(TakeReal, QDenseCplx const &d, ManageStore &m)
{
...
void doTask(TakeImag, QDenseReal &d)
{
    // Set all elements to zero
    doTask(Fill<Real>{0.}, d);
}

void doTask(TakeImag, QDenseCplx const &d, ManageStore &m)
{
    auto *nd = m.makeNewData<QDenseReal>(d.offsets, d.size());
    for (auto i : range(d)) {
...
template <typename T>
Real doTask(NormNoScale, QDense<T> const &D)
{
    auto d = realData(D);
...
template <typename T>
void doTask(PrintIT &P, QDense<T> const &d)
{
    ...
    for (auto const &io : d.offsets) {
        ...
        for (auto i : range(ord)) {
            for (auto j : range(io.block[i])) boff[i] += P.is[i].blocksize0(j);
        }
...
template <typename T1, typename T2>
void add(PlusEQ const &P, QDense<T1> &A, QDense<T2> const &B)
{
    auto r = order(P.is1());
    ...
    for (auto const &aio : A.offsets) {
        for (auto i : range(r)) Bblock[i] = aio.block[P.perm().dest(i)];
...
void doTask(PlusEQ const &P, QDense<TA> const &A, QDense<TB> const &B,
            ManageStore &m)
{
    ...
    for (auto ib : range(B.offsets.size())) {
        ...
        for (auto i : range(r)) Bblockp[i] = Bblock[invperm.dest(i)];
    }
...
template void doTask(PlusEQ const &, QDense<Real> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, QDense<Real> const &, QDense<Cplx> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, QDense<Cplx> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(PlusEQ const &, QDense<Cplx> const &, QDense<Cplx> const &,
                     ManageStore &);

template <typename VA, typename VB>
void doTask(Contract &Con, QDense<VA> const &A, QDense<VB> const &B,
            ManageStore &m)
{
    ...
    auto [Coffsets, Csize, blockContractions] =
        getContractedOffsets(A, Con.Lis, B, Con.Ris, Con.Nis);
    ...
    auto do_contract = [&Con, &Lind, &Rind, &Cind, &betas](
    ...
    loopContractedBlocks(A, Con.Lis, B, Con.Ris, C, Con.Nis, blockContractions,
                         do_contract);
...
template void doTask(Contract &Con, QDense<Real> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(Contract &Con, QDense<Cplx> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(Contract &Con, QDense<Real> const &, QDense<Cplx> const &,
                     ManageStore &);
template void doTask(Contract &Con, QDense<Cplx> const &, QDense<Cplx> const &,
                     ManageStore &);

template <typename VA, typename VB>
void doTask(NCProd &P, QDense<VA> const &A, QDense<VB> const &B, ManageStore &m)
{
    using VC = common_type<VA, VB>;
    auto &Ais = P.Lis;
    auto &Bis = P.Ris;
    ...
    auto do_ncprod = [&P, &Aind, &Bind, &Cind](
                         DataRange<const VA> ablock, Block const &Ablockind,
                         DataRange<const VB> bblock, Block const &Bblockind,
                         DataRange<VC> cblock, Block const &Cblockind) {
        Range Arange, Brange, Crange;
    ...
    loopContractedBlocks(A, Ais, B, Bis, C, Cis, do_ncprod);

#ifdef USESCALE
    P.scalefac = computeScalefac(C);
#endif
}
template void doTask(NCProd &, QDense<Real> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(NCProd &, QDense<Cplx> const &, QDense<Real> const &,
                     ManageStore &);
template void doTask(NCProd &, QDense<Real> const &, QDense<Cplx> const &,
                     ManageStore &);
template void doTask(NCProd &, QDense<Cplx> const &, QDense<Cplx> const &,
                     ManageStore &);

template <typename T>
void permuteQDense(Permutation const &P, QDense<T> const &dA,
                   IndexSet const &Ais, QDense<T> &dB, IndexSet &Bis)
{
    // Recalculate new indexset by permuting
    // original indexset (otherwise it segfaults)
    auto r = order(Ais);
    auto bind = IndexSetBuilder(r);
    for (auto i : range(r)) bind.setIndex(P.dest(i), Ais[i]);
    Bis = bind.build();
    ...
    for (auto const &aio : dA.offsets) {
        ...
        for (auto j : range(aio.block)) Bblock.at(P.dest(j)) = aio.block[j];
...
template <typename T>
void doTask(Order const &O, QDense<T> &dB)
{
    auto const dA = dB;
    auto Bis = O.is2();
...
template <typename V>
TenRef<Range, V> doTask(GetBlock<V> const &G, QDense<V> &d)
{
...
template <typename T>
bool doTask(IsDense, QDense<T> const &d)
{
    return true;
}
...
template <typename V>
void doTask(RemoveQNs &R, QDense<V> const &d, ManageStore &m)
{
    auto r = order(R.is);
...
std::ostream &operator<<(std::ostream &s, BlOf const &blof)
{
    s << "Block: " << blof.block << ", Offset: " << blof.offset << "\n";
    return s;
}

std::ostream &operator<<(std::ostream &s, BlockOffsets const &offsets)
{
    for (auto const &blof : offsets) s << blof;
    return s;
}

std::ostream &operator<<(std::ostream &s, Blocks const &blocks)
{
...
template <typename T>
std::ostream &operator<<(std::ostream &s, QDense<T> const &t)
{
    s << "QDense blocks and offsets:\n";
    s << t.offsets << "\n";
    s << "\nQDense storage:\n";
    for (auto i : range(t.store.size())) s << i << " " << t.store[i] << "\n";
    return s;
}
...
vector<long> offsets_to_array(BlockOffsets const &boff, int N)
{
...
BlockOffsets array_to_offsets(vector<long> const &a, long N)
{
    auto asize = a.size();
...
const char *juliaTypeNameOf(QDenseReal const &d)
{
    return "BlockSparse{Float64}";
}
const char *juliaTypeNameOf(QDenseCplx const &d)
{
    return "BlockSparse{ComplexF64}";
}

template <typename V>
void h5_write(h5::group parent, std::string const &name, QDense<V> const &D)
{
...
template <typename V>
void h5_read(h5::group parent, std::string const &name, QDense<V> &D)
{
    auto g = parent.open_group(name);
    auto type = h5_read_attribute<string>(g, "type");
    if (type != juliaTypeNameOf(D)) {
        Error(tinyformat::format(
            "Group does not contain %s or %s data in HDF5 file", typeNameOf(D),
            juliaTypeNameOf(D)));
    }
