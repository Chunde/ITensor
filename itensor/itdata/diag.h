...
namespace itensor
{
    ...
    template <typename T>
    bool constexpr isReal(Diag<T> const &D) { return isReal<T>(); }

    template <typename T>
    bool constexpr isCplx(Diag<T> const &D) { return isCplx<T>(); }
    ...
    Data inline realData(DiagCplx &d) { return Data(reinterpret_cast<Real *>(d.data()), 2 * d.size()); }

    Datac inline realData(DiagCplx const &d) { return Datac(reinterpret_cast<const Real *>(d.data()), 2 * d.size()); }
    ...
    auto inline doTask(StorageType const &S, DiagReal const &d) -> StorageType::Type { return StorageType::DiagReal; }

    auto inline doTask(StorageType const &S, DiagCplx const &d) -> StorageType::Type { return StorageType::DiagCplx; }
