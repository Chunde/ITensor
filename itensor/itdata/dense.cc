...
namespace itensor
{
...
#ifdef DEBUG
        if (D1.size() != D2.size())
            Error("Mismatched sizes in plusEq");
#endif
    ...
    h5_read(h5::group parent, std::string const &name, Dense<V> &D)
    {
        ...
        if (type != juliaTypeNameOf(D))
            Error(tinyformat::format("Group does not contain %s data in HDF5 file", typeNameOf(D)));
