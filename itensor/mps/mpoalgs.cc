...
namespace itensor
{
    ...
    nmultMPO(MPO const &Aorig,
             ...
             Args args)
    {
        if (!args.defined("Cutoff"))
            args.add("Cutoff", 1E-14);
        if (!args.defined("RespectDegenerate"))
            args.add("RespectDegenerate", true);
        ...
        {
            if (!sA(i))
                throw ITError("Error in nmultMPO(A,B): MPO tensor A(" + str(i) + ") does not have a unique site index. You may have meant to call nmultMPO(A,prime(B)).");
            if (!sB(i))
                throw ITError("Error in nmultMPO(A,B): MPO tensor B(" + str(i) + ") does not have a unique site index. You may have meant to call nmultMPO(A,prime(B)).");
        }
    ...
    {
        if (!x)
            Error("Error in applyMPO, MPS is uninitialized.");
        if (!K)
            Error("Error in applyMPO, MPO is uninitialized.");
             ...
             Args args)
    {
        ...
        if (!K)
            Error("Error in applyMPO, MPO is uninitialized.");
        if (!x0)
            Error("Error in applyMPO, guess MPS is uninitialized.");
                              ...
                              Args args)
    {
        ...
        if (dowrite)
            writedir_ = mkTempDir("E", args.getString("WriteDir", "./"));
        ...
        if (verbose)
            print("Building environment tensors...");
        ...
        if (verbose)
            printfln("  j=%02d truncerr=%.2E dim=%d", N - 1, spec.truncerr(), dim(commonIndex(U, D)));
        ...
        {
            if (dowrite)
                readFromFile(tinyformat::format("%s/E_%03d", writedir_, j - 1), E[j - 1]);
            if (not maxdim_set)
            {
            ...
            res.ref(j) = dag(U);
            if (verbose)
                printfln("  j=%02d truncerr=%.2E dim=%d", j, spec.truncerr(), dim(commonIndex(U, D)));
        }
    ...
    {
        if (&psi == &res)
            Error("Must pass distinct MPS arguments to applyExpH");
