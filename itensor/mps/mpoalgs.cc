...
void nmultMPO(MPO const &Aorig, MPO const &Borig, MPO &res, Args args)
{
    if (!args.defined("Cutoff")) args.add("Cutoff", 1E-14);
    if (!args.defined("RespectDegenerate")) args.add("RespectDegenerate", true);

    if (length(Aorig) != length(Borig))
        Error("nmultMPO(MPO): Mismatched MPO length");
    ...
    for (int i = 1; i <= N; ++i) {
        if (!sA(i))
            throw ITError("Error in nmultMPO(A,B): MPO tensor A(" + str(i) +
                          ") does not have a unique site index. You may have "
                          "meant to call nmultMPO(A,prime(B)).");
        if (!sB(i))
            throw ITError("Error in nmultMPO(A,B): MPO tensor B(" + str(i) +
                          ") does not have a unique site index. You may have "
                          "meant to call nmultMPO(A,prime(B)).");
    }
    ...
    for (int i = 1; i < N; ++i) {
        ...
        denmatDecomp(clust, res.ref(i), nfork, Fromleft,
                     {args, "Tags=", tags(lA(i))});

        auto mid = commonIndex(res(i), nfork);
        mid.dag();
        res.ref(i + 1) =
            ITensor(mid, sA(i + 1), sB(i + 1), rightLinkIndex(res, i + 1));
    }
...
MPO nmultMPO(MPO const &A, MPO const &B, Args args)
{
    MPO res;
...
MPS densityMatrixApplyMPOImpl(MPO const &K, MPS const &x,
                              Args args = Args::global());

void fitApplyMPOImpl(MPS const &psi, MPO const &K, MPS &res,
                     Args const &args = Args::global());

MPS applyMPO(MPO const &K, MPS const &x, Args args)
{
    ...
    if (method == "DensityMatrix") {
        ...
        Error(
            "applyMPO currently supports the following methods: "
            "'DensityMatrix', 'Fit'");
    }
...
MPS applyMPO(MPO const &K, MPS const &x, MPS const &x0, Args args)
{
    if (!x) Error("Error in applyMPO, MPS is uninitialized.");
    if (!K) Error("Error in applyMPO, MPO is uninitialized.");
...
MPS densityMatrixApplyMPOImpl(MPO const &K, MPS const &psi, Args args)
{
    ...
    auto dowrite = args.defined("WriteDim") &&
    ...
    for (auto n : range1(N)) {
        if (commonIndex(psi(n), K(n)) != siteIndex(psi, n))
            Error(
                "MPS and MPO have different site indices in applyMPO method "
                "'DensityMatrix'");
    }
    ...
    for (int j = 2; j < N; ++j) {
        ...
        if (dowrite) {
            writeToFile(tinyformat::format("%s/E_%03d", writedir_, j - 1),
                        E[j - 1]);
            E[j - 1] = ITensor();
        }
    }
    ...
    if (verbose)
        printfln("  j=%02d truncerr=%.2E dim=%d", N - 1, spec.truncerr(),
                 dim(commonIndex(U, D)));
    ...
    for (int j = N - 1; j > 1; --j) {
        if (dowrite)
            readFromFile(tinyformat::format("%s/E_%03d", writedir_, j - 1),
                         E[j - 1]);
        if (not maxdim_set) {
        ...
        res.ref(j) = dag(U);
        if (verbose)
            printfln("  j=%02d truncerr=%.2E dim=%d", j, spec.truncerr(),
                     dim(commonIndex(U, D)));
    }
...
void oneSiteFitApply(vector<ITensor> &E, Real fac, MPS const &x, MPO const &K,
                     MPS &Kx, Args const &args)
{
    auto N = length(x);
    auto verbose = args.getBool("Verbose", false);
    auto normalize = args.getBool("Normalize", false);
...
void twoSiteFitApply(vector<ITensor> &E, Real fac, MPS const &x, MPO const &K,
                     MPS &Kx, Args const &args)
{
    auto N = length(x);
    auto verbose = args.getBool("Verbose", false);
    ...
    for (int b = 1, ha = 1; ha <= 2; sweepnext(b, ha, N)) {
        ...
        wfK.dag();
        auto spec =
            Kx.svdBond(b, wfK, (ha == 1 ? Fromleft : Fromright), PH, args);
...
void fitApplyMPOImpl(Real fac, MPS const &x, MPO const &K, MPS &Kx,
                     Sweeps const &sweeps, Args args)
{
    auto N = length(x);
    ...
    for (auto sw : range1(sweeps.nsweep())) {
        ...
        if (NCenterSites == 2) {
            ...
            Error(tinyformat::format("NCenterSites = %d not supported",
                                     NCenterSites));
        }
    }
    Kx.dag();
...
void fitApplyMPOImpl(Real fac, MPS const &psi, MPO const &K, MPS &res,
                     Args args)
{
    if (args.defined("Maxm")) {
        if (args.defined("MaxDim")) {
...
void fitApplyMPOImpl(MPS const &psi, MPO const &K, MPS &res, Args const &args)
{
               ...
               Args const &args)
{
    ...
    vector<ITensor> lastB(N + 2), B(N + 2), BH(N + 2);
        ...
        for (int sw = 1; sw <= nsweep; ++sw) {
            for (int b = 1, ha = 1; ha <= 2; sweepnext(b, ha, N)) {
                ...
                if (up) {
                    ...
                    lwfH *= H(b);
                    rwfH = (BH.at(b + 2) ? BH.at(b + 2) * last(b + 1)
                                         : last(b + 1));
                    rwfH *= H(b + 1);
                } else  // dn
                {
                    lwf =
                        (B.at(b - 1) ? B.at(b - 1) * dag(prime(psi(b), "Link"))
                                     : dag(prime(psi(b), "Link")));
                    rwf = (B.at(b + 2)
                               ? B.at(b + 2) * dag(prime(psi(b + 1), "Link"))
                               : dag(prime(psi(b + 1), "Link")));

                    lwfH = (BH.at(b - 1) ? BH.at(b - 1) * dag(prime(last(b)))
                                         : dag(prime(last(b))));
                    lwfH *= H(b);
                    rwfH =
                        (BH.at(b + 2) ? BH.at(b + 2) * dag(prime(last(b + 1)))
                                      : dag(prime(last(b + 1))));
                    rwfH *= H(b + 1);
                }
                ...
                res.svdBond(b, wf, (ha == 1 ? Fromleft : Fromright),
                            args + Args("UseSVD", true));
...
//
//     if(!allow_arb_position && (!itensor::isOrtho(K) ||
//     itensor::orthoCenter(K) != 1))
...
//         res.ref(i+1) =
//         ITensor(mid,prime(res.sites()(i+1)),linkIndex(res,i+1));
//         }
