...
void rungeKutta4(const Deriv &D, Real tstep, std::vector<Tensor> &v,
                 const Args &args = Args::global())
{
    ...
    for (int j = 1; j <= N; ++j) {
        v.at(j) +=
            (tstep / 6.) * (k1.at(j) + 2 * k2.at(j) + 2 * k3.at(j) + k4.at(j));
    }
}

template <class Tensor, typename Deriv>
void midpointMethod(const Deriv &D, Real tstep, std::vector<Tensor> &v,
                    const Args &args = Args::global())
{
    int N = int(v.size()) - 1;
    while (!v.at(N) && N > 1) --N;
