...
bool isZero(Cplx const &z, Real thresh = 1E-13) { return std::abs(z) < thresh; }

bool less(Real x, Real y, Real eps = 1E-12)
{
    Real ax = std::fabs(x);
    Real ay = std::fabs(y);
...
bool equal(Cplx x, Cplx y, Real eps = 1E-12)
{
    Real ax = std::abs(x);
    Real ay = std::abs(y);
    Real scale = (ax < ay ? ay : ax);
...
bool less(Cplx const &z1, Cplx const &z2, Real eps = 1E-12)
{
...
bool isReal(const Cplx &z) { return z.imag() == 0; }

bool isApproxReal(Cplx const &z, Real epsilon = 1E-12)
{
    return std::fabs(z.imag()) < epsilon;
}

SiteTerm::SiteTerm() : i(-1) {}

SiteTerm::SiteTerm(string const &op_, int i_) : op(op_), i(i_) {}

bool isFermionic(SiteTerm const &st)
{
#ifdef DEBUG
    for (char c : st.op)
        if (c == '*') {
            Print(st.op);
            Error(
                "SiteTerm contains a '*' but isFermionic does not handle this "
                "case");
        }
#endif
...
SiteTermProd mult(SiteTermProd first, SiteTermProd const &second)
{
    first.insert(first.end(), second.begin(), second.end());
    return first;
}

bool isFermionic(SiteTermProd const &sprod)
{
    bool isf = false;
    for (auto &st : sprod) {
...
string fermionicTerm(const string &op)
{
    static array<pair<string, string>, 6> rewrites = {
        {make_pair("Cdagup", "Adagup"), make_pair("Cup", "Aup"),
         make_pair("Cdagdn", "Adagdn*Fup"), make_pair("Cdn", "Adn*Fup"),
         make_pair("C", "A"), make_pair("Cdag", "Adag")}};
    for (auto &p : rewrites) {
...
void rewriteFermionic(SiteTermProd &prod, bool isleftFermionic)
{
    ...
    if (isSiteFermionic) {
        for (auto &st : prod)
            if (isFermionic(st)) st.op = fermionicTerm(st.op);
    }

    // Add a FermiPhase operator at the end if the product of operators
    // to the left (including this site) is fermionic
    if ((isleftFermionic && !isSiteFermionic) ||
        (!isleftFermionic && isSiteFermionic)) {
        prod.emplace_back("F", i);
    }
}

ITensor computeProd(SiteSet const &sites, SiteTermProd const &p)
{
    auto i = p.front().i;
...
void HTerm::add(string const &op, int i, Real x)
{
    ...
    // If the operator is fermionic and being inserted in between existing
    // operators need to check if an extra minus is required
...
HTerm &HTerm::operator*=(Complex x)
{
    if (Nops() == 0) Error("No operators in HTerm");
    coef *= x;
    return *this;
...
bool HTerm::operator==(HTerm const &o) const
{
...
bool LessNoCoef::operator()(HTerm const &t1, HTerm const &t2) const
{
...
AutoMPO::Accumulator::Accumulator(AutoMPO *pa_, Real x_)
    : pa(pa_), state(New), coef(x_)
{
}

AutoMPO::Accumulator::Accumulator(AutoMPO *pa_, Complex x_)
    : pa(pa_), state(New), coef(x_)
{
}

AutoMPO::Accumulator::Accumulator(AutoMPO *pa_) : Accumulator(pa_, 1) {}

AutoMPO::Accumulator::Accumulator(AutoMPO *pa_, const char *op_)
    : pa(pa_), state(Op), coef(1), op(op_)
{
}

AutoMPO::Accumulator::Accumulator(AutoMPO *pa_, const string &op_)
    : pa(pa_), state(Op), coef(1), op(op_)
{
}

AutoMPO::Accumulator::~Accumulator()
{
...
AutoMPO::Accumulator &AutoMPO::Accumulator::operator,(Real x)
{
    coef *= x;
    return *this;
}

AutoMPO::Accumulator &AutoMPO::Accumulator::operator,(Complex x)
{
    coef *= x;
    return *this;
}

AutoMPO::Accumulator &AutoMPO::Accumulator::operator,(int i)
{
    if (state == Op) {
        term.add(op, i);
        state = New;
...
AutoMPO::Accumulator &AutoMPO::Accumulator::operator,(const char *op_)
{
    if (state == New) {
        op = op_;
        state = Op;
...
AutoMPO::Accumulator &AutoMPO::Accumulator::operator,(const string &op_)
{
    if (state == New) {
        op = op_;
        state = Op;
...
void AutoMPO::add(HTerm const &t)
{
...
Cdagdn_i Cdn_j  = (Adagdn_i F_i) F_{i+1} ... Fup_j Adn_j
                = - Adagdn_i F_{i+1} ... Fup_j Adn_j     //use Adagdn_i * F_i =
-Adagdn_i = Adagdn_i F_{i+1} ... Fup_j Fdn_j Adn_j //use Adn_j = -Fdn_j*Adn_j =
Adagdn_i F_{i+1} ... (F_j Adn_j)       //combine Fup_j*Fdn_j = F_j (definition)

Cdn_i Cdagdn_j = (Adn_i F_i) F_{i+1} ... Fup_j Adagdn_j
               = - Adn_i F_{i+1} ... Fup_j Adagdn_j      //use Adn_i*F_i =
-Adn_i = Adn_i F_{i+1} ... Fup_j Fdn_j Adagdn_j  //use Adagdn_j =
-Fdn_j*Adagdn_j = Adn_i F_{i+1} ... (F_j Adagdn_j)        //combined Fup_j*Fdn_j
= F_j (definition)
...
struct SiteQN {
    ...
    SiteQN(SiteTerm const &st_, QN const &q_) : st(st_), q(q_) {}
};

std::ostream &operator<<(std::ostream &s, SiteQN const &sq)
{
    s << "SiteQN: " << sq.st << ", " << sq.q;
    return s;
}

void plusAppend(string &s, string const &a)
{
    if (s.size() == 0 || s == "0")
        s = a;
    else {
...
string startTerm(const string &op)
{
    static array<pair<string, string>, 6> rewrites = {
        {make_pair("Cdagup", "Adagup*F"), make_pair("Cup", "Aup*F"),
         make_pair("Cdagdn", "Adagdn"), make_pair("Cdn", "Adn"),
         make_pair(
             "C",
             "A*F"),  // A*F is -A, so essentially a trick for putting in a -1
         make_pair("Cdag", "Adag")}};
    for (auto &p : rewrites) {
        if (p.first == op) return p.second;
    }
...
string endTerm(const string &op)
{
    static array<pair<string, string>, 6> rewrites = {
        {make_pair("Cup", "Aup"), make_pair("Cdagup", "Adagup"),
         make_pair("Cdn", "F*Adn"), make_pair("Cdagdn", "F*Adagdn"),
         make_pair("C", "A"), make_pair("Cdag", "Adag")}};
    for (auto &p : rewrites) {
        if (p.first == op) return p.second;
    }
...
ITensor convert_tensor(ITensor &&t) { return std::move(t); }

MPO toMPOImpl(AutoMPO const &am, Args const &args)
{
    ...
    for (auto &t : am.terms())
        if (t.Nops() > 2) {
            Error(
                "Only at most 2-operator terms allowed for exact AutoMPO "
                "conversion to MPO");
        }
    ...
    for (auto &ht : am.terms()) {
        for (auto n = ht.first().i; n <= ht.last().i; ++n) {
            auto &bn = basis.at(n);
            auto test_has_first = [&ht](SiteQN const &sq) {
                return sq.st == ht.first();
            };
    ...
    if (checkqns) {
        auto qn_comp = [&Zero](const SiteQN &sq1, const SiteQN &sq2) {
            ...
            if (sq1.q == Zero && sq2.q != Zero)
                return true;
            else if (sq2.q == Zero && sq1.q != Zero)
                return false;
            return sq1.q < sq2.q;
        };
    ...
    for (int n = 0; n <= N; ++n) {
        ...
        //     for(size_t l = 0; l < bn.size(); ++l) printfln("%d %s
        //     %s",l,bn.at(l).st,bn.at(l).q); println(); printfln("IQIndex for
        //     site %d:\n%s",n,links.at(n));
        //     }
    }
    ...
    for (auto n : range1(N)) {
        ...
        auto &W = H.ref(n);
        auto &row = links.at(n - 1), &col = links.at(n);
        ...
        for (auto r : range(dim(row)))
            for (auto c : range(dim(col))) {
                ...
                if (cst.i == n && rst == IL) {
                    // Call startTerm to handle fermionic cases with
                        ...
                        if(ht.first() == rst &&
                           ...
                           ht.last().i  != n)
                            ...
                            if(st.i == n)
                                ...
                                found += 1;
    #ifdef SHOW_AUTOMPO
                                ws[r][c] = tinyformat::format("%.2f
    %s",st.coef,st.op); #endif W += st.coef * op(sites,st.op,n) * rc;
                                }
                            }
                        }
                    //if(found == 0)
                    ...
                    for (const auto &ht : ht_by_n.at(n))
                        if (rst == ht.first() && ht.last().i == n) {
                            ...
                            if (isApproxReal(coef)) {
                                ws[r][c] = tinyformat::format("%.2f %s",
                                                              coef.real(), op);
                            } else {
                                ws[r][c] =
                                    tinyformat::format("%.2f %s", coef, op);
                            }
#endif
                        }
                }
                    ...
                    for (const auto &ht : ht_by_n.at(n))
                        if (ht.first().i == ht.last().i) {
                            ...
                            if (isApproxReal(ht.first().coef))
                                ws[r][c] = tinyformat::format(
                                    "%.2f %s", ht.coef.real(), ht.first().op);
                            else
                                ws[r][c] = tinyformat::format(
                                    "%.2f %s", ht.coef, ht.first().op);
#endif
                            W += ht.coef *
                                 convert_tensor(op(sites, ht.first().op, n)) *
                                 rc;
                        }
                }
            }
...
struct IQMPOMatElem {
    ...
    bool operator==(const IQMPOMatElem &other) const
    {
        return rowqn == other.rowqn && colqn == other.colqn &&
               row == other.row && col == other.col && val == other.val;
    }

    bool operator<(IQMPOMatElem const &o) const
    {
        if (row != o.row) {
            return row < o.row;
        } else if (col != o.col) {
...
struct MatIndex {
    ...
    bool operator==(const MatIndex &other) const
    {
...
struct MatElem {
    ...
    bool operator==(MatElem const &other) const
    {
        return ind == other.ind && val == other.val;
    }
};

template <typename T>
Mat<T> toMatrix(vector<MatElem<T>> const &vm)
{
    Mat<T> M;
    int nr = 0, nc = 0;
...
void decomposeTerm(int n, SiteTermProd const &ops, SiteTermProd &left,
                   SiteTermProd &onsite, SiteTermProd &right)
{
...
template <typename T>
int posInBlock(SiteTermProd const &ops, typename BasisBlock<T>::Basis &b)
{
    auto it = b.find(ops);
...
auto forceType(V x) -> stdx::enable_if_t<std::is_same<T, V>::value, T>
{
    return x;
}
template <typename T, class = stdx::enable_if_t<std::is_same<T, Real>::value>>
T forceType(Cplx z)
{
    return z.real();
}
...
//
template <typename T>
void partitionHTerms(SiteSet const &sites, AutoMPO::storage const &terms,
                     vector<QNBlock<T>> &qbs, vector<IQMatEls> &tempMPO,
                     bool checkqns = true)
{
    auto N = length(sites);
    ...
    for (HTerm const &ht : terms)
        for (int n = ht.first().i; n <= ht.last().i; ++n) {
            ...
            // and qbs.at(N-2) are the blocks at the link between sites N-1 and
            // N for site n the link on the left is qbs.at(n-2) and the link on
            // the right is part.at(n-1)
            if (left.empty()) {
                ...
                leftlink.mat.emplace_back(MatIndex(l, j),
                                          forceType<T>(ht.coef));
            }
            ...
            if (onsite.empty()) {
                if (leftF)
                    onsite.emplace_back("F", n);
                else
                    onsite.emplace_back("Id", n);
            } else {
...
bool operator<(QNProd const &p1, QNProd const &p2)
{
...
// SVD the coefficients matrix on each link and construct the compressed MPO
// matrix
template <typename T>
void compressMPO(SiteSet const &sites, vector<QNBlock<T>> const &qbs,
                 vector<IQMatEls> const &tempMPO, vector<MPOPiece<T>> &finalMPO,
                 vector<Index> &links, bool isExpH = false, Complex tau = 0,
                 Args args = Args::global())
{
    if (args.defined("Minm")) {
        if (args.defined("MinDim")) {
            Global::warnDeprecated(
                "Args Minm and MinDim are both defined. Minm is deprecated in "
                "favor of MinDim, MinDim will be used.");
        } else {
            Global::warnDeprecated(
                "Arg Minm is deprecated in favor of MinDim.");
    ...
    if (hasqn)
        links.at(0) = Index(ZeroQN, d0, tinyformat::format("Link,l=%d", 0));
    else
        links.at(0) = Index(d0, tinyformat::format("Link,l=%d", 0));
...
template <typename T>
MPO constructMPOTensors(SiteSet const &sites,
...
MPO svdMPO(AutoMPO const &am, Args const &args)
{
    bool isExpH = false;
    Cplx tau = 0.;
    ...
    if (is_real) {
        ...
        compressMPO(am.sites(), qbs, tempMPO, finalMPO, links, isExpH, tau,
                    args);
...
MPO toMPO(AutoMPO const &am, Args const &args)
{
...
MPO toExpH_ZW1(AutoMPO const &am, Complex tau, Args const &args)
{
    auto checkqns = args.getBool("CheckQN=", true);
    if (not hasQNs(am.sites()(1))) checkqns = false;
    ...
    for (auto &t : am.terms())
        if (t.Nops() > 2) {
            Error(
                "Only at most 2-operator terms allowed for AutoMPO conversion "
                "to MPO/IQMPO");
        }
    ...
    for (int n = 0; n <= N; ++n) basis.at(n).emplace_back(IL, QN());
    ...
    for (const auto &ht : am.terms())
        for (int n = ht.first().i; n < ht.last().i; ++n) {
            ...
            bool has_first =
                (std::find_if(bn.cbegin(), bn.cend(), test) != bn.end());
            if (!has_first) {
                if (checkqns) {
                ...
                if (rst == IL && cst == IL) {
                    for (const auto &ht : ht_by_n.at(n))
                        if (ht.first().i == ht.last().i) {
                            auto op =
                                ht.coef *
                                convert_tensor(sites.op(ht.first().op, n)) * rc;
                            op *= (-tau);
                            W += op;
                        }
                }
...
MPO toExpH(AutoMPO const &a, Complex tau, Args const &args)
{
    ...
    if (approx == "ZW1") {
        ...
        Error(
            tinyformat::format("Unknown approximation Approx=\"%s\"", approx));
    }
    return res;
}

std::ostream &operator<<(std::ostream &s, SiteTerm const &t)
{
    s << t.op << "(" << t.i << ")";
    return s;
}

std::ostream &operator<<(std::ostream &s, HTerm const &t)
{
    const char *pfix = "";
    if (abs(t.coef - 1.0) > 1E-12) {
        s << (isReal(t.coef) ? tinyformat::format("%f ", t.coef.real())
                             : tinyformat::format("%f ", t.coef));
    }
    for (auto &st : t.ops) {
...
std::ostream &operator<<(std::ostream &s, const AutoMPO &a)
{
    s << "AutoMPO:\n";
