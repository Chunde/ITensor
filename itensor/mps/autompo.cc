...
namespace itensor
{
    ...
    {
        if (not equal(coef, o.coef, 1E-12))
            return false;
        if (Nops() != o.Nops())
            return false;
    ...
    {
        if (t1.ops.size() != t2.ops.size())
            return t1.ops.size() < t2.ops.size();

        for (size_t j = 0ul; j < t1.ops.size(); ++j)
        {
            if (t1.ops[j] != t2.ops[j])
                return t1.ops[j] < t2.ops[j];
        }
        return false;
    }
    ...
    {
        if (state == Op)
            Error("Invalid input to AutoMPO (missing site number?)");
        term *= coef;
        pa->add(term);
    }
    ...
    */
              ...
              Args const &args)
    {
        ...
        if (checkqns)
        {
            ...
            for (auto &bn : basis)
                std::sort(bn.begin(), bn.end(), qn_comp);
        }
    ...
    struct IQMPOMatElem
    {
        ...
        IQMPOMatElem(const QN &rqn, const QN &cqn, int r, int c, const HTerm &t) : rowqn(rqn), colqn(cqn), row(r), col(c), val(t) {};
    ...
    decomposeTerm(int n,
                  ...
                  SiteTermProd &right)
    {
        auto isOnSiteOrOnTheRight = [&n](const SiteTerm &t)
        { return t.i >= n; };
        auto startOfOnSite = find_if(ops.begin(), ops.end(), isOnSiteOrOnTheRight);

        auto isOnTheRight = [&n](const SiteTerm &t)
        { return t.i > n; };
    ...
    T forceType(Cplx z) { return z.real(); }
                ...
                Args args = Args::global())
    {
        ...
        for (int n = 1; n <= N; ++n)
        {
            ...
            if (isExpH)
                Error("Need to put in factor of (-tau)");
