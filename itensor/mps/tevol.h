...
Real gateTEvol(Iterable const &gatelist, Real ttotal, Real tstep, MPS &psi,
               Args const &args = Args::global());

template <class Iterable>
Real gateTEvol(Iterable const &gatelist, Real ttotal, Real tstep, MPS &psi,
               Observer &obs, Args args = Args::global());
               ...
               Observer &obs, Args args)
{
    ...
    if (verbose) {
        printfln("Taking %d steps of timestep %.5f, total time %.5f", nt, tstep,
                 ttotal);
    }
