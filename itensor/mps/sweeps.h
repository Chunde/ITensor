...
class Sweeps
{
   public:
    ...
    Sweeps(int nsweeps, int mindim = 1, int maxdim = 500, Real cutoff = 1E-8,
           Real noise = 0.);
    ...
    int size() const { return nsweep_; }

    int mindim(int sw) const { return mindim_.at(sw); }
    void setmindim(int sw, int val) { mindim_.at(sw) = val; }

    // Use as sweeps.mindim() = 20,20,10; (all remaining set to 10)
    SweepSetter<int> mindim();

    int maxdim(int sw) const { return maxdim_.at(sw); }
    void setmaxdim(int sw, int val) { maxdim_.at(sw) = val; }

    // Use as sweeps.maxdim() = 50,50,100,100,500; (all remaining set to 500)
    SweepSetter<int> maxdim();

    // Deprecated, use maxdim() instead
    SweepSetter<int> maxm();

    Real cutoff(int sw) const { return cutoff_.at(sw); }
    void setcutoff(int sw, Real val) { cutoff_.at(sw) = val; }

    // Use as sweeps.cutoff() = 1E-8; (cutoff set to 1E-8 for all sweeps)
    // or as sweeps.cutoff() = 1E-4,1E-4,1E-5,1E-5,1E-10; (all remaining set to
    // 1E-10)
    SweepSetter<Real> cutoff();

    Real noise(int sw) const { return noise_.at(sw); }
    void setnoise(int sw, Real val) { noise_.at(sw) = val; }
    void setnoise(Real val) { noise_.assign(nsweep_ + 1, val); }

    // Use as sweeps.noise() = 1E-10; (noise set to 1E-10 for all sweeps)
    // or as sweeps.noise() = 1E-8,1E-9,1E-10,0.0; (all remaining set to 0)
    SweepSetter<Real> noise();

    int niter(int sw) const { return niter_.at(sw); }
    void setniter(int sw, int val) { niter_.at(sw) = val; }
    void setniter(int val) { niter_.assign(nsweep_ + 1, val); }

    // Use as sweeps.niter() = 5,4,3,2; (all remaining set to 2)
    SweepSetter<int> niter();
   ...
   private:
    ...
    void tableInit(InputGroup &table);

    std::vector<int> maxdim_, mindim_, niter_;
...
class SweepSetter
{
   public:
    SweepSetter(std::vector<T> &v)
        : nsweep_(int(v.size()) - 1), started_(false), v_(v), j_(1)
    {
        last_val_ = v_[j_];
    }
    ...
    SweepSetter &operator,(const Args &args)
    {
        checkStarted();
...
struct RampM {
    RampM(int start_m, int end_m, const Args &args = Args::global())
        : start_m_(start_m), end_m_(end_m), nwarm_(args.getInt("Warmup", -1))
    {
    }

    int operator()(int sw, int nsweep) const
    {
        const int actual_nwarm =
            (nwarm_ < 0 ? nsweep : std::min(nwarm_ + 1, nsweep));
        if (sw <= actual_nwarm)
            return (int)(start_m_ +
                         (sw - 1.) / (actual_nwarm - 1.) * (end_m_ - start_m_));
        else
            return end_m_;
    }

   private:
    const int start_m_, end_m_, nwarm_;
};

struct ExpM {
    ExpM(int start_m, int end_m, Args const &args = Args::global())
        : start_m_(start_m),
          end_m_(end_m),
   ...
   private:
    const int start_m_, end_m_;
    const Real exp_base_;
};

inline Sweeps::Sweeps() : nsweep_(0) {}

inline Sweeps::Sweeps(int nsw, int min_dim, int max_dim, Real cut, Real noise)
    : nsweep_(nsw)
{
    init({"MinDim", min_dim, "MaxDim", max_dim, "Cutoff", cut, "Noise", noise});
}

inline Sweeps::Sweeps(Args const &args)
{
    nsweep_ = args.getInt("Nsweep");
    init(args);
}

inline Sweeps::Sweeps(int nsw, InputGroup &sweep_table) : nsweep_(nsw)
{
    tableInit(sweep_table);
}

SweepSetter<int> inline Sweeps::mindim() { return SweepSetter<int>(mindim_); }

SweepSetter<int> inline Sweeps::maxdim() { return SweepSetter<int>(maxdim_); }

SweepSetter<int> inline Sweeps::maxm()
{
    Global::warnDeprecated("maxm() is deprecated, use maxdim() instead.");
    return SweepSetter<int>(maxdim_);
}

SweepSetter<Real> inline Sweeps::cutoff() { return SweepSetter<Real>(cutoff_); }

SweepSetter<Real> inline Sweeps::noise() { return SweepSetter<Real>(noise_); }

SweepSetter<int> inline Sweeps::niter() { return SweepSetter<int>(niter_); }

void inline Sweeps::nsweep(int val)
{
    if (val > nsweep_)
...
void inline Sweeps::init(Args args)
{
...
void inline Sweeps::tableInit(InputGroup &table)
{
    if (!table.GotoGroup()) {
    ...
    for (int i = 1; i <= nsweep_; i++) {
        table.file() >> maxdim_[i] >> mindim_[i] >> cutoff_[i] >> niter_[i] >>
            noise_[i];
        // printfln("Line %d: %d %d %.2E %d
        // %.2E",i,maxdim_[i],mindim_[i],cutoff_[i],niter_[i],noise_[i]);
        if (maxdim_[i] == 0) {
...
void inline Sweeps::write(std::ostream &s) const
{
...
void inline Sweeps::read(std::istream &s)
{
...
inline std::ostream &operator<<(std::ostream &s, const Sweeps &swps)
{
    s << "Sweeps:\n";
    for (int sw = 1; sw <= swps.nsweep(); ++sw) {
        s << tinyformat::format(
            "%d  MaxDim=%d, MinDim=%d, Cutoff=%.1E, Niter=%d, Noise=%.1E\n", sw,
            swps.maxdim(sw), swps.mindim(sw), swps.cutoff(sw), swps.niter(sw),
            swps.noise(sw));
    }
    return s;
}

void inline sweepnext(int &b, int &ha, int N, Args const &args = Args::global())
{
...
void inline sweepnext(int &b, int &ha, int N, int min_b)
{
    auto args = Args("NumCenter=", 2, "Minb=", min_b);
    sweepnext(b, ha, N, args);
}

void inline sweepnext1(int &b, int &ha, int N, int min_b = 1)
{
