...
void new_tensors(std::vector<ITensor> &A, SiteSet const &sites, int m = 1);

void new_tensors(std::vector<ITensor> &A, IndexSet const &sites, int m = 1);
...
MPS::MPS() : N_(0), atb_(1), writedir_("./"), do_write_(false) {}
...
MPS::MPS(SiteSet const &sites, int m)
    : N_(sites.length()),
...
MPS::MPS(IndexSet const &sites, int m)
    : N_(sites.length()),
      A_(sites.length() + 2),  // idmrg may use A_[0] and A[N+1]
      l_orth_lim_(0),
      r_orth_lim_(sites.length() + 1),
...
MPS::MPS(InitState const &initState)
...
MPS &MPS::operator=(MPS const &other)
{
    N_ = other.N_;
    A_ = other.A_;
    l_orth_lim_ = other.l_orth_lim_;
...
MPS::~MPS() { cleanupWrite(); }

// TODO: Add note in error to call randomMPS(InitState,dim) instead, once it is
// implemented
MPS &MPS::randomize(Args const &args)
{
    if (maxLinkDim(*this) > 1)
        Error(
            ".randomize() not currently supported on MPS with bond dimension "
            "greater than 1.");
...
MPS operator*(MPS x, Real r)
{
    x *= r;
    return x;
}

MPS operator*(Real r, MPS x)
{
    x *= r;
    return x;
}

MPS &operator*=(MPS &x, Cplx z)
{
    x.ref(x.leftLim() + 1) *= z;
    return x;
}
...
MPS operator*(Cplx z, MPS x)
{
    x *= z;
    return x;
}

int length(MPS const &W) { return W.length(); }

Matrix randomOrthog(int n, int m)
{
    auto r = std::max(n, m);
    auto M = randn(r, r);
    Matrix Q, R;
...
MPS randomCircuitMPS(SiteSet const &s, int m, Args const &args)
{
    auto N = length(s);
    auto M = MPS(N);
...
MPS randomMPS(SiteSet const &sites, int m, Args const &args)
{
    if (hasQNs(sites)) {
        Error(
            "randomMPS(SiteSet) with QN conservation is ambiguous, use "
            "randomMPS(InitState) instead.");
    }

    return randomCircuitMPS(sites, m, args);
}

MPS randomMPS(SiteSet const &sites, Args const &args)
{
    return randomMPS(sites, 1, args);
}

// TODO: implement for m>1 in terms of random gates
MPS randomMPS(InitState const &initstate, int m, Args const &args)
{
...
MPS randomMPS(InitState const &initstate, Args const &args)
{
    return randomMPS(initstate, 1, args);
}

ITensor const &MPS::operator()(int i) const
{
    if (i < 0) i = N_ + i + 1;
    setSite(i);
    return A_.at(i);
...
ITensor &MPS::uref(int i)
{
    if (do_write_) Error("replaceTags not supported if doWrite(true)");
    return A_.at(i);
}

// Deprecated
ITensor const &MPS::A(int i) const { return this->operator()(i); }

// Deprecated
ITensor &MPS::Aref(int i) { return this->ref(i); }

void MPS::doWrite(bool val, Args const &args)
{
...
void MPS::write(std::ostream &s) const
{
    if (do_write_) Error("MPS::write not yet supported if doWrite(true)");
...
void MPS::read(std::string const &dirname)
{
    l_orth_lim_ = 0;
    r_orth_lim_ = N_ + 1;
...
string MPS::AFName(int j, string const &dirname) const
{
    if (dirname == "") {
...
void MPS::setBond(int b) const
{
    ...
    // std::ofstream
    // inf((tinyformat::format("%s/info")%writedir_).str().c_str());
...
void new_tensors(std::vector<ITensor> &A, SiteSet const &sites, int m)
{
    auto N = length(sites);
    ...
    if (hasQNs(sites)) {
        if (m == 1)
            for (auto i : range1(N))
                a[i] = Index(QN(), m, tinyformat::format("Link,l=%d", i));
        else
            Error(
                "Cannot create QN conserving MPS with bond dimension greater "
                "than 1 from a SiteSet");
    } else {
        for (auto i : range1(N))
            a[i] = Index(m, tinyformat::format("Link,l=%d", i));
    }
...
void new_tensors(std::vector<ITensor> &A, IndexSet const &sites, int m)
{
    auto N = length(sites);
    auto a = std::vector<Index>(N + 1);
    if (hasQNs(sites)) {
...
void MPS::init_tensors(std::vector<ITensor> &A_, InitState const &initState)
{
    ...
    if (hasQNs(initState)) {
        ...
        for (auto i : range1(N_))
            a[i] = Index(qa[i], 1, tinyformat::format("Link,l=%d", i));
    } else {
        for (auto i : range1(N_))
            a[i] = Index(1, tinyformat::format("Link,l=%d", i));
    }
...
MPS &MPS::plusEq(MPS const &R, Args const &args)
{
...
Spectrum MPS::svdBond(int b, ITensor const &AA, Direction dir, Args args)
{
    return svdBond(b, AA, dir, LocalOp(), args);
}

struct SqrtInv {
    Real operator()(Real val) const
    {
        if (val == 0) return 0;
...
struct Sqrt {
    Real operator()(Real val) const { return std::sqrt(std::fabs(val)); }
};

Spectrum orthMPS(ITensor &A1, ITensor &A2, Direction dir, Args const &args)
{
...
MPS &MPS::position(int i, Args args)
{
    ...
    if (args.getBool("DoSVDBond", false)) {
        while (l_orth_lim_ < i - 1) {
            ...
            svdBond(l_orth_lim_ + 1, WF, Fromleft,
                    {args, "LeftTags=", original_link_tags});
        }
        while (r_orth_lim_ > i + 1) {
            ...
            svdBond(r_orth_lim_ - 2, WF, Fromright,
                    {args, "RightTags=", original_link_tags});
        }
    } else  // use orthMPS
    {
        while (l_orth_lim_ < i - 1) {
            ...
            orthMPS(ref(b), ref(b + 1), Fromleft,
                    {args, "LeftTags=", original_link_tags});
...
MPS &MPS::orthogonalize(Args args)
{
    if (args.defined("Maxm")) {
        if (args.defined("MaxDim")) {
            Global::warnDeprecated(
                "Args Maxm and MaxDim are both defined. Maxm is deprecated in "
                "favor of MaxDim, MaxDim will be used.");
        } else {
            Global::warnDeprecated(
                "Arg Maxm is deprecated in favor of MaxDim.");
    ...
    for (int j = 2; j < N; ++j) E[j] = E[j - 1] * psi(j) * psic(j);
    auto rho = E[N - 1] * psi(N) *
               itensor::prime(psic(N), rand_plev, siteInds(psic, N));
...
ITensor denseDelta(Index const &i, Index const &j)
{
    auto del = ITensor(i, j);
    for (auto ii : range1(minDim(del))) del.set(ii, ii, 1);
    return del;
}
}  // namespace detail

template <typename MPSType>
bool checkOrtho(MPSType const &A, int i, Direction dir, Real threshold)
{
    ...
    println("checkOrtho: Tensor at position ", i, " failed to be ",
            left ? "left" : "right", " ortho.");
...
template bool checkOrtho<MPS>(MPS const &A, int i, Direction dir,
                              Real threshold);
template bool checkOrtho<MPO>(MPO const &A, int i, Direction dir,
                              Real threshold);

template <typename MPSType>
bool checkOrtho(MPSType const &A, Real threshold)
{
    for (int i = 1; i <= A.leftLim(); ++i)
        if (!checkOrtho(A, i, Fromleft, threshold)) {
            std::cout << "checkOrtho: A_[i] not left orthogonal at site i=" << i
                      << std::endl;
            return false;
        }
...
void applyGate(ITensor const &gate, MPS &psi, Args const &args)
{
    ...
    if (fromleft)
        ...
        psi.svdBond(c, AA, Fromright, args);
}

void MPS::initWrite(Args const &args)
{
    if (!do_write_) {
...
void MPS::swap(MPS &other)
{
    if (N_ != other.N_) Error("Require same system size to swap MPS");
    A_.swap(other.A_);
...
InitState::InitState(SiteSet const &sites)
    : sites_(sites), state_(1 + length(sites))
{
...
InitState::InitState(SiteSet const &sites, String const &state)
    : sites_(sites), state_(1 + length(sites))
{
    setAll(state);
}

InitState &InitState::set(int i, const String &state)
{
    checkRange(i);
...
InitState &InitState::setAll(String const &state)
{
    for (int n = 1; n <= length(sites_); ++n) {
...
bool isOrtho(MPS const &psi) { return psi.leftLim() + 1 == psi.rightLim() - 1; }

int orthoCenter(MPS const &psi)
{
    if (!isOrtho(psi)) Error("orthogonality center not well defined.");
    return (psi.leftLim() + 1);
}

int rightLim(MPS const &x) { return x.rightLim(); }

int leftLim(MPS const &x) { return x.leftLim(); }

Real norm(MPS const &psi)
{
    if (not isOrtho(psi))
        Error(
            "\
...
template <typename MPSType>
Real averageLinkDim(MPSType const &x)
{
    Real avgdim = 0.;
    auto N = length(x);
...
Real averageM(MPS const &psi)
{
    Global::warnDeprecated(
        "averageM(MPS) is deprecated in favor of averageLinkDim(MPS)");
    return averageLinkDim(psi);
}

template <typename MPSType>
int maxLinkDim(MPSType const &x)
{
    int maxdim = 0;
...
int maxM(MPS const &psi)
{
    Global::warnDeprecated(
        "maxM(MPS) is deprecated in favor of maxLinkDim(MPS)");
    return maxLinkDim(psi);
}

Real normalize(MPS &psi)
{
    Global::warnDeprecated(
        "normalize(MPS) is deprecated in favor of .normalize()");
...
MPS setTags(MPS A, TagSet const &ts, IndexSet const &is)
{
    A.setTags(ts, is);
    return A;
}

MPS noTags(MPS A, IndexSet const &is)
{
    A.noTags(is);
    return A;
}

MPS addTags(MPS A, TagSet const &ts, IndexSet const &is)
{
    A.addTags(ts, is);
    return A;
}

MPS removeTags(MPS A, TagSet const &ts, IndexSet const &is)
{
    A.removeTags(ts, is);
    return A;
}

MPS replaceTags(MPS A, TagSet const &ts1, TagSet const &ts2, IndexSet const &is)
{
    A.replaceTags(ts1, ts2, is);
    return A;
}

MPS swapTags(MPS A, TagSet const &ts1, TagSet const &ts2, IndexSet const &is)
{
    A.swapTags(ts1, ts2, is);
    return A;
}

MPS prime(MPS A, int plev, IndexSet const &is)
{
    A.prime(plev, is);
    return A;
}

MPS prime(MPS A, IndexSet const &is)
{
    A.prime(is);
    return A;
}

MPS setPrime(MPS A, int plev, IndexSet const &is)
{
    A.setPrime(plev, is);
    return A;
}

MPS mapPrime(MPS A, int plevold, int plevnew, IndexSet const &is)
{
    A.mapPrime(plevold, plevnew, is);
    return A;
}

MPS swapPrime(MPS A, int plevold, int plevnew, IndexSet const &is)
{
    A.swapPrime(plevold, plevnew, is);
    return A;
}

MPS noPrime(MPS A, IndexSet const &is)
{
    A.noPrime(is);
    return A;
}

bool hasSiteInds(MPS const &x, IndexSet const &sites)
{
    auto N = length(x);
    if (N != length(sites))
        Error(
            "In hasSiteInds(MPS,IndexSet), lengths of MPS and IndexSet of site "
            "indices don't match");
    for (auto n : range1(N)) {
...
template <typename MPSType>
IndexSet siteInds(MPSType const &W, int b)
{
...
Index siteIndex(MPS const &psi, int j)
{
    return uniqueIndex(psi(j), {psi(j - 1), psi(j + 1)});
}

template <typename MPSType>
Index rightLinkIndex(MPSType const &psi, int i)
{
...
template <typename MPSType>
Index leftLinkIndex(MPSType const &psi, int i)
{
    if (i == 1) return Index();
    return commonIndex(psi(i), psi(i - 1));
}
...
template <typename MPSType>
Index linkIndex(MPSType const &psi, int i)
{
...
template <typename MPSType>
IndexSet linkInds(MPSType const &x, int i)
{
    if (i == 1)
        return IndexSet(rightLinkIndex(x, i));
    else if (i == length(x))
        return IndexSet(leftLinkIndex(x, i));
...
template <typename MPSType>
IndexSet linkInds(MPSType const &x)
{
    auto N = length(x);
...
MPS &MPS::replaceSiteInds(IndexSet const &sites)
{
    auto &x = *this;
    auto N = itensor::length(x);
    if (itensor::length(sites) != N)
        Error(
            "In replaceSiteInds(MPS,IndexSet), number of site indices not "
            "equal to number of MPS tensors");
...
MPS replaceSiteInds(MPS x, IndexSet const &sites)
{
    x.replaceSiteInds(sites);
    return x;
}

MPS &MPS::replaceLinkInds(IndexSet const &links)
{
    auto &x = *this;
    auto N = itensor::length(x);
    if (N == 1) return x;
    if (itensor::length(links) != (N - 1))
        Error(
            "In replaceLinkInds(MPS,IndexSet), number of link indices input is "
            "not equal to the number of links of the MPS");
    auto lx = itensor::linkInds(x);
    if (equals(lx, links)) return x;
    for (auto n : range1(N - 1)) {
        if (n == 1)
            A_[n].replaceInds({lx(n)}, {links(n)});
        else
            A_[n].replaceInds({lx(n - 1), lx(n)}, {links(n - 1), links(n)});
    }
    A_[N].replaceInds({lx(N - 1)}, {links(N - 1)});
    return x;
}

MPS replaceLinkInds(MPS x, IndexSet const &links)
{
    x.replaceLinkInds(links);
    return x;
}

void InitState::checkRange(int i) const
{
...
bool hasQNs(InitState const &initstate)
{
...
std::ostream &operator<<(std::ostream &s, MPS const &M)
{
    s << "\n";
...
std::ostream &operator<<(std::ostream &s, InitState const &state)
{
    s << "\n";
...
template <class MPSType>
MPSType removeQNs(MPSType const &psi)
{
    int N = length(psi);
    MPSType res;
    res = MPSType(N);
...
template <class MPSType>
MPSType sum(MPSType const &L, MPSType const &R, Args const &args)
{
    auto res = L;
    res.plusEq(R, args);
    return res;
...
template <class MPSType>
MPSType sum(std::vector<MPSType> const &terms, Args const &args)
{
    auto Nt = terms.size();
    if (Nt == 2) {
...
Cplx innerC(MPS const &psi, MPS const &phi)
{
    auto N = length(psi);
    ...
    for (auto i : range1(2, N)) L = L * phi(i) * psidag(i);
    return eltC(L);
}

void inner(MPS const &psi, MPS const &phi, Real &re, Real &im)
{
...
Real inner(MPS const &psi, MPS const &phi)  // Re[<psi|phi>]
{
    if (isComplex(psi) || isComplex(phi))
        Error(
            "Cannot use inner(...) with complex MPS/MPO, use innerC(...) "
            "instead");
    Real re, im;
    inner(psi, phi, re, im);
    return re;
...
{
    if (*r == *r.end()) r = range1(1, N);
}

//-------------------------------------------------------------------------------------------
//
//  Template implementation of correlationMatrix function for Real and Complex
//  types,
//    The implementation assumes that Hilbert space for each site on the lattice
//    is consistent with op1 and op2.  This could get rather complicated for a
//    heterogeneous lattice with for example alternating fermionic and bosonic
//    Hilbert spaces.  In such cases users will to create custom code for the
//    calculating correlations
//
template <class T>
std::vector<std::vector<T>> correlationMatrixT(
    const MPS &_psi, const SiteSet &sites, const string &_op1,
    const string &_op2, detail::RangeHelper<int> site_range, Args const &args)
{
    assert(checkConsistent(_psi, sites));

    // Decide if we need to calculate a non-hermitian corr. matrix which is
    // roughly double the work.
    bool isHermitian = false;  // Assume non-hermitian
    if (args.defined(
            "isHermitian"))  // Did the user explicitly request something?
        isHermitian = args.getBool("isHermitian");  // Honour users request
    else {
        ITensor O1 = sites.op(_op1, 1);
        ITensor O2 = sites.op(_op2, 1);
        // We need to decide if O1==O2^dagger allowing for some round off
        // errors.
    ...
    //
    // Copy _psi (because we need to move the ortho centre around), set the
    // ortho centre and calculate the norm constant.
    //
    MPS psi = _psi;
    ...
    if (fermionic1 != fermionic2)  // for example A_i*C_j
    {
        ...
        throw std::runtime_error(
            "correlationMatrix: Mixed fermionic and bosonic operators are not "
            "supported yet.");
    }
    ...
    for (auto i : site_range)
        C.push_back(std::vector<T>(Nb)),
            (void)i;  // (void)i eliminates an unused i warning
    ...
    for (auto i : site_range) {
        ...
        ITensor Li =
            L * psi(i);  // Update accumulated contraction from the left.

        // Get j == i diagonal correlations

        // We now need to prime all indices on psi(i) except the link to site
        // i+1.
        IndexSet linds =
            uniqueInds(psi(i), psi(i + 1));  // indices in psi(i) that are not
                                             // in psi(i+1), i.e. not the link
        ITensor psi_i_dag = dag(prime(psi(i), linds));
        ITensor c = Li * sites.op(onsiteOp, i) * psi_i_dag / norm2_psi;

        assert(order(c) == 0);  // If there is any screw up in the priming we
                                // get a higher order tensor out.
        C[ci][ci] = eltT<T>(c);
        ...
        for (auto j = i + 1; j <= end_site; j++) {
            ...
            c = (Li12 * sites.op(op2, j)) *
                dag(prime(prime(psi(j), "Site"), lind));
            C[ci][cj] = eltT<T>(c) / norm2_psi;

            if (isHermitian) C[cj][ci] = conj(C[ci][cj]);  // not always valid.
        ...
        if (!isHermitian)  // If isHermitian=false the we must calculate the
                           // below diag elements explicitly.
        {
            //  Get j < i correlations by swapping the operators
            if (fermionic1) op2 += "*F";
            ITensor Li21 = (Li * sites.op(op2, i)) * dag(prime(psi(i)));
            if (fermionic1)
                Li21 = -Li21;  // Required because we swapped fermionic ops,
                               // instead of sweeping right to left.
            for (auto j = i + 1; j <= end_site; j++) {
                auto cj = j - start_site;  // index into cm matrix.
                Index lind = commonIndex(psi(j), Li21);
                Li21 *= psi(j);

                c = (Li21 * sites.op(op1, j)) *
                    dag(prime(prime(psi(j), "Site"), lind));
        ...
        L = (L * psi(i)) *
            dag(prime(
                psi(i),
                "Link"));  // Update accumulated contraction from the left.
    }  // for i
...
//
template std::vector<std::vector<Real>> correlationMatrixT<Real>(
    const MPS &_psi, const SiteSet &sites, const string &op1, const string &op2,
    detail::RangeHelper<int> site_range, Args const &args);

template std::vector<std::vector<Complex>> correlationMatrixT<Complex>(
    const MPS &_psi, const SiteSet &sites, const string &op1, const string &op2,
    detail::RangeHelper<int> site_range, Args const &args);
...
template <class MPSType>
Cplx overlapC(MPSType const &psi, MPSType const &phi)
{
...
template <typename MPSType>
void overlap(MPSType const &psi, MPSType const &phi, Real &re, Real &im)
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
...
template <typename MPSType>
Real overlap(MPSType const &psi, MPSType const &phi)  // Re[<psi|phi>]
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
    ...
    if (std::fabs(im) > (1E-12 * std::fabs(re)))
        printfln(
            "Real overlap: WARNING, dropping non-zero imaginary part (=%.5E) "
...
void h5_write(h5::group parent, string const &name, MPS const &M)
{
...
void h5_read(h5::group parent, string const &name, MPS &M)
{
