...
namespace itensor
{
        ...
        randomize(Args const &args)
    {
        if (maxLinkDim(*this) > 1)
            Error(".randomize() not currently supported on MPS with bond dimension greater than 1.");
    ...
    {
        if (m > 1)
            Error("randomMPS(InitState,m>1) not currently supported.");
                ...
                int m)
    {
        ...
        if (hasQNs(sites))
        {
            if (m == 1)
                ...
                Error("Cannot create QN conserving MPS with bond dimension greater than 1 from a SiteSet");
        }
        else
        {
            for (auto i : range1(N))
                ...
                int m)
    {
        ...
        if (hasQNs(sites))
        {
            if (m == 1)
                ...
                Error("Cannot create QN conserving MPS with bond dimension greater than 1 from an IndexSet");
        }
        else
        {
            for (auto i : range1(N))
    ...
    {
        if (not *this)
            Error("position: MPS is default constructed");
        ...
        else // use orthMPS
        {
            while (l_orth_lim_ < i - 1)
            {
                ...
                ++l_orth_lim_;
                if (r_orth_lim_ < l_orth_lim_ + 2)
                    r_orth_lim_ = l_orth_lim_ + 2;
            }
            while (r_orth_lim_ > i + 1)
            {
                if (r_orth_lim_ > N_ + 1)
                ...
                --r_orth_lim_;
                if (l_orth_lim_ > r_orth_lim_ - 2)
                    l_orth_lim_ = r_orth_lim_ - 2;
            }
        }
        return *this;
    }
        ...
        orthogonalize(Args args)
    {
        ...
        if (doWrite())
            Error("Cannot call orthogonalize when doWrite()==true");
        ...
        if (maxdim_set)
            dargs.add("MaxDim", args.getInt("MaxDim"));
    ...
    {
        if (!isOrtho(psi))
            Error("orthogonality center not well defined.");
...
call .position(j) or .orthogonalize() to set ortho center");
    ...
    hasSiteInds(MPS const &x, IndexSet const &sites)
    {
        auto N = length(x);
        if (N != length(sites))
            Error("In hasSiteInds(MPS,IndexSet), lengths of MPS and IndexSet of site indices don't match");
        for (auto n : range1(N))
        {
            if (!hasIndex(x(n), sites(n)))
                return false;
        }
        return true;
    }
    ...
    linkInds(MPSType const &x)
    {
        ...
        for (auto n : range1(N - 1))
        {
            ...
            if (!s)
                Error("MPS or MPO has missing/null link index");
        ...
        replaceSiteInds(IndexSet const &sites)
    {
        ...
        if (itensor::length(sites) != N)
            Error("In replaceSiteInds(MPS,IndexSet), number of site indices not equal to number of MPS tensors");
        ...
        replaceLinkInds(IndexSet const &links)
    {
        ...
        if (itensor::length(links) != (N - 1))
            Error("In replaceLinkInds(MPS,IndexSet), number of link indices input is not equal to the number of links of the MPS");
    ...
    findCenter(MPS const &psi)
    {
        for (int j = 1; j <= length(psi); ++j)
        {
            auto &A = psi(j);
            if (A.order() == 0)
                Error("Zero order tensor in MPS");
        ...
        Args const &args)
    {
        ...
        else if (Nt > 2)
        {
            ...
            if (Nt % 2 == 1)
                newterms.at(nsize - 1) = terms.back();
    ...
    inner(MPS const &psi, MPS const &phi) // Re[<psi|phi>]
    {
        if (isComplex(psi) || isComplex(phi))
            Error("Cannot use inner(...) with complex MPS/MPO, use innerC(...) instead");
        Real re, im;
        inner(psi, phi, re, im);
        return re;
    }
                       ...
                       Args const &args)
    {
        ...
        for (auto i : site_range)
            C.push_back(std::vector<T>(Nb)), (void)i; // (void)i eliminates an unused i warning
    ...
    h5_read(h5::group parent, string const &name, MPS &M)
    {
        ...
        if (type != "MPS")
            Error("Group does not contain MPS data in HDF5 file");
