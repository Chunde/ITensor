...
class BondGate
{
   public:
    enum Type {
        tReal,  // real-time gate
        tImag,  // imaginary-time gate
        Swap,
        Custom
    };  // exchange states of sites i1 and i2

    BondGate(SiteSet const &sites, int i1, int i2);

    BondGate(SiteSet const &sites, int i1, int i2, Type type, Real tau,
             ITensor bondH);

    BondGate(SiteSet const &sites, int i1, int i2, ITensor gate);
...
ITensor inline operator*(BondGate const &G, ITensor T)
{
    T *= G.gate();
    return T;
}

ITensor inline operator*(ITensor T, BondGate const &G)
{
    T *= G.gate();
    return T;
}

inline BondGate::BondGate(SiteSet const &sites, int i1, int i2) : type_(Swap)
{
    if (i1 < i2) {
        i1_ = i1;
        i2_ = i2;
...
inline BondGate::BondGate(SiteSet const &sites, int i1, int i2, Type type,
                          Real tau, ITensor bondH)
    : type_(type)
{
    if (i1 < i2) {
        i1_ = i1;
...
inline BondGate::BondGate(SiteSet const &sites, int i1, int i2, ITensor gate)
    : type_(Custom)
{
    i1_ = i1;
    i2_ = i2;
...
void inline BondGate::makeSwapGate(SiteSet const &sites)
{
    auto s1 = sites(i1_);
    auto s2 = sites(i2_);
