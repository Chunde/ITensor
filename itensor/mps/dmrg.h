...
Real DMRGWorker(MPS &psi, LocalOpT &PH, Sweeps const &sweeps,
                Args const &args = Args::global());

template <class LocalOpT>
Real DMRGWorker(MPS &psi, LocalOpT &PH, Sweeps const &sweeps, DMRGObserver &obs,
...
// DMRG with an MPO
//
Real inline dmrg(MPS &psi, MPO const &H, Sweeps const &sweeps,
                 Args const &args = Args::global())
{
    LocalMPO PH(H, args);
...
//
std::tuple<Real, MPS> inline dmrg(MPO const &H, MPS const &psi0,
                                  Sweeps const &sweeps,
                                  Args const &args = Args::global())
{
    auto psi = psi0;
...
//
Real inline dmrg(MPS &psi, MPO const &H, Sweeps const &sweeps,
                 DMRGObserver &obs, Args const &args = Args::global())
{
    LocalMPO PH(H, args);
...
//
std::tuple<Real, MPS> inline dmrg(MPO const &H, MPS const &psi0,
                                  Sweeps const &sweeps, DMRGObserver &obs,
                                  Args const &args = Args::global())
{
    auto psi = psi0;
...
//
Real inline dmrg(MPS &psi, MPO const &H, ITensor const &LH, ITensor const &RH,
                 Sweeps const &sweeps, Args const &args = Args::global())
{
...
// and returns the optimized MPS
//
std::tuple<Real, MPS> inline dmrg(MPO const &H, ITensor const &LH,
                                  ITensor const &RH, MPS const &psi0,
                                  Sweeps const &sweeps,
                                  Args const &args = Args::global())
{
    auto psi = psi0;
...
//
Real inline dmrg(MPS &psi, std::vector<MPO> const &Hset, Sweeps const &sweeps,
                 Args const &args = Args::global())
{
...
std::tuple<Real, MPS> inline dmrg(std::vector<MPO> const &Hset, MPS const &psi0,
                                  Sweeps const &sweeps,
                                  Args const &args = Args::global())
{
    auto psi = psi0;
...
//          etc.
//
Real inline dmrg(MPS &psi, MPO const &H, std::vector<MPS> const &psis,
                 Sweeps const &sweeps, Args const &args = Args::global())
{
    if (hasQNs(psi)) {
        ...
        for (auto n : range(psis)) {
            ...
            if (qn_n != psi_qn) {
                ...
                Error(
                    "Excited-state DMRG intended for states with same totalQN");
            }
        }
    }
...
std::tuple<Real, MPS> inline dmrg(MPO const &H, std::vector<MPS> const &psis,
                                  MPS const &psi0, Sweeps const &sweeps,
                                  Args const &args = Args::global())
{
    auto psi = psi0;
...
template <class LocalOpT>
Real DMRGWorker(MPS &psi, LocalOpT &PH, Sweeps const &sweeps, Args const &args)
{
                ...
                Args args)
{
    if (args.defined("WriteM")) {
        if (args.defined("WriteDim")) {
            Global::warnDeprecated(
                "Args WirteM and WriteDim are both defined. WriteM is "
                "deprecated in favor of WriteDim, WriteDim will be used.");
        } else {
            Global::warnDeprecated(
                "Arg WriteM is deprecated in favor of WriteDim.");
    ...
    for (int sw = 1; sw <= sweeps.nsweep(); ++sw) {
        ...
        if (!PH.doWrite() && args.defined("WriteDim") &&
        ...
        for (int b = 1, ha = 1; ha <= 2; sweepnext(b, ha, N)) {
            ...
            if (!quiet) {
                printfln("    Truncated to Cutoff=%.1E, Min_dim=%d, Max_dim=%d",
                         sweeps.cutoff(sw), sweeps.mindim(sw),
                         sweeps.maxdim(sw));
                printfln("    Trunc. err=%.1E, States kept: %s",
                         spec.truncerr(), showDim(linkIndex(psi, b)));
            }
