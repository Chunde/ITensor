...
namespace itensor
{
        ...
        : Parent(sites)
    {
        ...
        if (_logrefNorm == DefaultLogRefScale)
            logrefNorm_ = sites.length();
    ...
    hasSiteInds(MPO const &A, IndexSet const &sites)
    {
        auto N = length(A);
        if (N != length(sites))
            Error("In hasSiteInds(MPO,IndexSet), lengths of MPO and IndexSet of site indices don't match");
        for (auto n : range1(N))
        {
            if (!hasIndex(A(n), sites(n)))
                return false;
        }
        return true;
    }
    ...
    uniqueSiteInds(MPO const &A, MPS const &x)
    {
        auto N = length(x);
        if (N != length(x))
            Error("In uniqueSiteInds(MPO,MPS), lengths of MPO and MPS do not match");
    ...
    uniqueSiteInds(MPO const &A, MPO const &B)
    {
        auto N = length(A);
        if (N != length(B))
            Error("In uniqueSiteInds(MPO,MPO), lengths of MPO and MPS do not match");
        auto inds = IndexSetBuilder(N);
        for (auto n : range1(N))
        {
    ...
    uniqueSiteInds(MPO const &A, IndexSet const &sites)
    {
        auto N = length(A);
        if (N != length(sites))
            Error("In uniqueSiteInds(MPO,IndexSet), lengths of MPO and IndexSet do not match");
        auto inds = IndexSetBuilder(N);
        for (auto n : range1(N))
        {
        ...
        replaceSiteInds(IndexSet const &sites_old, IndexSet const &sites_new)
    {
        ...
        if (itensor::length(sites_new) != N)
            Error("In replaceSiteInds(MPO,IndexSet,IndexSet), number of new sites must be equal length of MPO");
        if (itensor::hasSiteInds(A, sites_new))
            return A;
        for (auto n : range1(N))
    ...
    findCenter(MPO const &psi)
    {
        for (int j = 1; j <= length(psi); ++j)
        {
            const auto &A = psi(j);
            if (A.order() == 0)
                Error("Zero order tensor in MPO");
            bool allOut = true;
           ...
           MPO const &B)
    {
        auto N = length(A);
        if (N != length(B))
            Error("traceC(MPO,MPO): mismatched N");
    ...
    inner(MPO const &A,
          ...
          Real &im)
    {
        if (length(x) != length(y) || length(x) != length(A) || length(y) != length(B))
    ...
    inner(MPS const &x,
          ...
          Real &im)
    {
        if (length(x) != length(y) || length(x) != length(A) || length(x) != length(B))
            Error("Mismatched N in inner");
        auto N = length(x);
            ...
            Real &im)
    {
        ...
        auto N = length(H);
        if (length(phi) != N || length(psi) != N)
            Error("psiHphi: mismatched N");
            ...
            Real &im) //<psi|H|phi>
    {
        Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
        auto N = length(psi);
        if (N != length(phi) || length(H) < N)
            Error("mismatched N in psiHphi");
            ...
            Real &im)
    {
        ...
        if (length(psi) != length(phi) || length(psi) != length(H) || length(psi) != length(K))
    ...
    h5_read(h5::group parent, string const &name, MPO &M)
    {
        ...
        if (type != "MPO")
            Error("Group does not contain MPO data in HDF5 file");
