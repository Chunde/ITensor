...
MPO::MPO() : Parent(), logrefNorm_(DefaultLogRefScale) {}

MPO::MPO(int N) : Parent(N), logrefNorm_(DefaultLogRefScale) {}

MPO::MPO(const SiteSet &sites, Real _logrefNorm) : Parent(sites)
{
...
MPO &MPO::plusEq(MPO const &other_, Args const &args)
{
    if (doWrite()) Error("operator+= not supported if doWrite(true)");
...
MPO &operator*=(MPO &W, Real a)
{
    W.ref(W.leftLim() + 1) *= a;
    return W;
}

MPO &operator*=(MPO &W, Cplx a)
{
    W.ref(W.leftLim() + 1) *= a;
    return W;
}
...
MPO operator*(MPO W, Real r) { return W *= r; }

MPO operator*(Real r, MPO W) { return W *= r; }

MPO operator*(MPO W, Cplx z) { return W *= z; }

MPO operator*(Cplx z, MPO W) { return W *= z; }
...
int length(MPO const &W) { return W.length(); }

bool isOrtho(MPO const &W) { return W.leftLim() + 1 == W.rightLim() - 1; }

int orthoCenter(MPO const &W)
{
    if (!isOrtho(W)) Error("orthogonality center not well defined.");
    return (W.leftLim() + 1);
}

bool hasSiteInds(MPO const &A, IndexSet const &sites)
{
    auto N = length(A);
    if (N != length(sites))
        Error(
            "In hasSiteInds(MPO,IndexSet), lengths of MPO and IndexSet of site "
            "indices don't match");
    for (auto n : range1(N)) {
...
Index siteIndex(MPO const &W, int b, TagSet const &tsmatch)
{
...
Index uniqueSiteIndex(MPO const &W, IndexSet const &is, int b)
{
...
Index uniqueSiteIndex(MPO const &W, MPS const &A, int b)
{
    return uniqueIndex(W(b), {W(b - 1), W(b + 1), A(b)});
}

// Get the site Index that is unique to A
Index uniqueSiteIndex(MPO const &A, MPO const &B, int b)
{
    return uniqueIndex(A(b), {A(b - 1), A(b + 1), B(b)});
}

IndexSet uniqueSiteInds(MPO const &A, MPS const &x)
{
    auto N = length(x);
    if (N != length(x))
        Error(
            "In uniqueSiteInds(MPO,MPS), lengths of MPO and MPS do not match");
...
IndexSet siteInds(MPO const &A, MPO const &B, int b)
{
...
IndexSet uniqueSiteInds(MPO const &A, MPO const &B)
{
    auto N = length(A);
    if (N != length(B))
        Error(
            "In uniqueSiteInds(MPO,MPO), lengths of MPO and MPS do not match");
    auto inds = IndexSetBuilder(N);
    for (auto n : range1(N)) {
...
IndexSet uniqueSiteInds(MPO const &A, IndexSet const &sites)
{
    auto N = length(A);
    if (N != length(sites))
        Error(
            "In uniqueSiteInds(MPO,IndexSet), lengths of MPO and IndexSet do "
            "not match");
    auto inds = IndexSetBuilder(N);
    for (auto n : range1(N)) {
...
MPO &MPO::replaceSiteInds(IndexSet const &sites_old, IndexSet const &sites_new)
{
    auto &A = *this;
    auto N = itensor::length(A);
    if (itensor::length(sites_new) != N)
        Error(
            "In replaceSiteInds(MPO,IndexSet,IndexSet), number of new sites "
            "must be equal length of MPO");
    if (itensor::hasSiteInds(A, sites_new)) return A;
    for (auto n : range1(N)) A_[n].replaceInds({sites_old(n)}, {sites_new(n)});
    return A;
}

MPO replaceSiteInds(MPO A, IndexSet const &sites_old, IndexSet const &sites_new)
{
    A.replaceSiteInds(sites_old, sites_new);
    return A;
}

MPO &MPO::swapSiteInds()
{
    auto &A = *this;
    auto N = itensor::length(A);
    for (auto n : range1(N)) {
...
MPO swapSiteInds(MPO A)
{
    A.swapSiteInds();
    return A;
}

int findCenter(MPO const &psi)
{
...
void checkQNs(MPO const &H)
{
    ...
    for (int i = 1; i < center; ++i) {
        if (dir(linkIndex(H, i)) != In) {
            println("checkQNs: At site ", i,
                    " to the left of the OC, Right side Link not pointing In");
        ...
        if (i > 1) {
            if (dir(linkIndex(H, i - 1)) != Out) {
                println(
                    "checkQNs: At site ", i,
                    " to the left of the OC, Left side Link not pointing Out");
                Error("Incorrect Arrow in MPO");
            }
        }
    }
    ...
    for (int i = N; i > center; --i) {
        if (i < N)
            if (dir(linkIndex(H, i)) != Out) {
                println("checkQNs: At site ", i,
                        " to the right of the OC, Right side Link not pointing "
                        "Out");
                Error("Incorrect Arrow in MPO");
            }
        if (dir(linkIndex(H, i - 1)) != In) {
            println("checkQNs: At site ", i,
                    " to the right of the OC, Left side Link not pointing In");
            Error("Incorrect Arrow in MPO");
        }
    }
}

std::ostream &operator<<(std::ostream &s, MPO const &M)
{
    s << "\n";
...
void putMPOLinks(MPO &W, Args const &args)
{
...
MPO setTags(MPO A, TagSet const &ts, IndexSet const &is)
{
    A.setTags(ts, is);
    return A;
}

MPO noTags(MPO A, IndexSet const &is)
{
    A.noTags(is);
    return A;
}

MPO addTags(MPO A, TagSet const &ts, IndexSet const &is)
{
    A.addTags(ts, is);
    return A;
}

MPO removeTags(MPO A, TagSet const &ts, IndexSet const &is)
{
    A.removeTags(ts, is);
    return A;
}

MPO replaceTags(MPO A, TagSet const &ts1, TagSet const &ts2, IndexSet const &is)
{
    A.replaceTags(ts1, ts2, is);
    return A;
}

MPO swapTags(MPO A, TagSet const &ts1, TagSet const &ts2, IndexSet const &is)
{
    A.swapTags(ts1, ts2, is);
    return A;
}

MPO prime(MPO A, int plev, IndexSet const &is)
{
    A.prime(plev, is);
    return A;
}

MPO prime(MPO A, IndexSet const &is)
{
    A.prime(is);
    return A;
}

MPO setPrime(MPO A, int plev, IndexSet const &is)
{
    A.setPrime(plev, is);
    return A;
}

MPO noPrime(MPO A, IndexSet const &is)
{
    A.noPrime(is);
    return A;
}

bool isComplex(MPO const &W)
{
...
void trace(MPO const &A, Real &re, Real &im)
{
    auto z = traceC(A);
    re = real(z);
    im = imag(z);
...
Real trace(MPO const &A)
{
    Real re, im;
    trace(A, re, im);
    if (std::fabs(im) > (1E-12 * std::fabs(re)))
        printfln(
            "Real inner: WARNING, dropping non-zero imaginary part (=%.5E) of "
            "expectation value.",
            im);
    return re;
}

Cplx traceC(MPO const &A, MPO const &B)
{
    auto N = length(A);
    ...
    auto L = A(1) * Bp(1);
    if (N == 1) return eltC(L);
    for (auto i : range1(2, N)) L = L * A(i) * Bp(i);
    return eltC(L);
}

void trace(MPO const &A, MPO const &B, Real &re, Real &im)
{
    auto z = traceC(A, B);
    re = real(z);
    im = imag(z);
...
Real trace(MPO const &A, MPO const &B)  // Re[<psi|phi>]
{
    Real re, im;
    trace(A, B, re, im);
    if (std::fabs(im) > (1E-12 * std::fabs(re)))
...
//<x|A|y>
void inner(MPS const &x, MPO const &A, MPS const &y, Real &re, Real &im)
{
    auto N = length(A);
    ...
    for (auto n : range1(2, N)) L = L * y(n) * A(n) * xdag(n);
...
Real inner(MPS const &psi, MPO const &H,
           MPS const &phi)  // Re[<psi|H|phi>]
{
    if (isComplex(psi) || isComplex(H) || isComplex(phi))
        Error(
...
Cplx innerC(MPS const &psi, MPO const &H,
            MPS const &phi)  // Re[<psi|H|phi>]
{
    Real re, im;
    inner(psi, H, phi, re, im);
...
// Calculate <Ax|By>
void inner(MPO const &A, MPS const &x, MPO const &B, MPS const &y, Real &re,
           Real &im)
{
    if (length(x) != length(y) || length(x) != length(A) ||
        length(y) != length(B))
        Error("Mismatched N in inner");
    auto N = length(y);
...
// Calculate <Ax|By>
Real inner(MPO const &A, MPS const &x, MPO const &B, MPS const &y)
{
    if (isComplex(A) || isComplex(x) || isComplex(B) || isComplex(y))
        Error(
            "Cannot use inner(...) with complex MPS/MPO, use innerC(...) "
            "instead");
    Real re, im;
...
// Calculate <Ax|By>
Complex innerC(MPO const &A, MPS const &x, MPO const &B, MPS const &y)
{
    Real re, im;
    inner(A, x, B, y, re, im);
    return Cplx(re, im);
...
void inner(MPS const &x, MPO const &A, MPO const &B, MPS const &y, Real &re,
           Real &im)
{
    if (length(x) != length(y) || length(x) != length(A) ||
        length(x) != length(B))
...
Real inner(MPS const &psi, MPO const &H, MPO const &K,
           MPS const &phi)  //<psi|H K|phi>
{
    if (isComplex(psi) || isComplex(H) || isComplex(K) || isComplex(phi))
        Error(
            "Cannot use inner(...) with complex MPS/MPO, use innerC(...) "
            "instead");
    Real re, im;
...
Cplx innerC(MPS const &psi, MPO const &H, MPO const &K,
            MPS const &phi)  //<psi|H K|phi>
{
    Real re, im;
    inner(psi, H, K, phi, re, im);
...
Real errorMPOProd(MPS const &y, MPO const &A, MPS const &x)
{
...
void overlap(MPS const &psi, MPO const &H, MPS const &phi, Real &re, Real &im)
{
...
Real overlap(MPS const &psi, MPO const &H,
             MPS const &phi)  // Re[<psi|H|phi>]
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
    ...
    if (std::fabs(im) > 1E-5 * std::fabs(re) || std::fabs(im) > 1E-9) {
        printfln(
            "\nReal psiHphi: WARNING, dropping non-zero (=%.5E) imaginary part "
...
Cplx overlapC(MPS const &psi, MPO const &H,
              MPS const &phi)  // Re[<psi|H|phi>]
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
...
void overlap(MPS const &psi, MPO const &H, ITensor const &LB, ITensor const &RB,
             MPS const &phi, Real &re,
             Real &im)  //<psi|H|phi>
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    auto N = length(psi);
...
Real overlap(MPS const &psi, MPO const &H, ITensor const &LB, ITensor const &RB,
             MPS const &phi)  // Re[<psi|H|phi>]
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
...
void overlap(MPS const &psi, MPO const &H, MPO const &K, MPS const &phi,
             Real &re, Real &im)
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    // println("Running psiHKphi");
    if (length(psi) != length(phi) || length(psi) != length(H) ||
        length(psi) != length(K))
        Error("Mismatched N in overlap");
    auto N = length(psi);
    auto psidag = psi;
    psidag.dag().prime(2);
    auto Hp = H;
...
Real overlap(MPS const &psi, MPO const &H, MPO const &K,
             MPS const &phi)  //<psi|H K|phi>
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
...
Cplx overlapC(MPS const &psi, MPO const &H, MPO const &K,
              MPS const &phi)  //<psi|H K|phi>
{
    Global::warnDeprecated("overlap is deprecated in favor of inner/trace");
    Real re, im;
...
void h5_write(h5::group parent, string const &name, MPO const &M)
{
...
void h5_read(h5::group parent, string const &name, MPO &M)
{
