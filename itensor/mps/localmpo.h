...
class LocalMPO
{
   public:
    ...
    //
    LocalMPO(const MPO &H, const ITensor &LH, const ITensor &RH,
             const Args &args = Args::global());
    ...
    //
    LocalMPO(const MPS &Psi, const ITensor &LP, const ITensor &RP,
             const Args &args = Args::global());
    ...
    //
    LocalMPO(MPO const &H, ITensor const &LH, int LHlim, ITensor const &RH,
             int RHlim, Args const &args = Args::global());

    //
    // Sparse Matrix Methods
    //

    void product(const ITensor &phi, ITensor &phip) const;

    Real expect(const ITensor &phi) const { return lop_.expect(phi); }

    ITensor deltaRho(const ITensor &AA, const ITensor &comb,
                     Direction dir) const
    {
        return lop_.deltaRho(AA, comb, dir);
    }

    ITensor diag() const { return lop_.diag(); }
    ...
    //
    void position(int b, MPS const &psi);

    int position() const;

    void shift(int j, Direction dir, ITensor const &A);
    ...
    ITensor const &L() const { return PH_[LHlim_]; }
    // Replace left edge tensor at current bond
    void L(ITensor const &nL) { PH_[LHlim_] = nL; }
    // Replace left edge tensor bordering site j
    // (so that nL includes sites < j)
    void L(int j, ITensor const &nL);

    ITensor const &R() const { return PH_[RHlim_]; }
    // Replace right edge tensor at current bond
    void R(ITensor const &nR) { PH_[RHlim_] = nR; }
    // Replace right edge tensor bordering site j
    // (so that nR includes sites > j)
    void R(int j, ITensor const &nR);

    const MPO &H() const
    {
        if (Op_ == 0) Error("LocalMPO is null or contains an MPS");
        return *Op_;
    }

    int numCenter() const { return nc_; }
    void numCenter(int val)
    {
    ...
    size_t size() const { return lop_.size(); }

    explicit operator bool() const { return Op_ != 0 || Psi_ != 0; }

    bool doWrite() const { return do_write_; }
    void doWrite(bool val, Args const &args = Args::global())
    {
        if (Psi_ != 0)
            Error(
                "Write to disk not yet supported for LocalMPO initialized with "
                "an MPS");
    ...
    int leftLim() const { return LHlim_; }

    int rightLim() const { return RHlim_; }
   ...
   private:
    ...
    void makeL(const MPS &psi, int k);

    void makeR(const MPS &psi, int k);

    void setLHlim(int val);

    void setRHlim(int val);

    void initWrite(Args const &args);

    std::string PHFName(int j) const
    {
        return tinyformat::format("%s/PH_%03d", writedir_, j);
    }
};

inline LocalMPO::LocalMPO() : Op_(0), LHlim_(-1), RHlim_(-1), nc_(2), Psi_(0) {}

inline LocalMPO::LocalMPO(const MPO &H, const Args &args)
    : Op_(&H),
      PH_(H.length() + 2),
      LHlim_(0),
      RHlim_(H.length() + 1),
...
inline LocalMPO::LocalMPO(const MPS &Psi, const Args &args)
    : Op_(0),
      PH_(Psi.length() + 2),
      LHlim_(0),
      RHlim_(Psi.length() + 1),
...
inline LocalMPO::LocalMPO(const ITensor &LH, const ITensor &RH,
                          const Args &args)
    : Op_(0), PH_(2), LHlim_(0), RHlim_(1), nc_(0), Psi_(0)
{
    PH_[0] = LH;
    PH_[1] = RH;
...
inline LocalMPO::LocalMPO(const MPO &H, const ITensor &LH, const ITensor &RH,
                          const Args &args)
    : Op_(&H),
      PH_(H.length() + 2),
      LHlim_(0),
...
inline LocalMPO::LocalMPO(MPS const &Psi, ITensor const &LP, ITensor const &RP,
                          Args const &args)
    : Op_(0),
      PH_(Psi.length() + 2),
      LHlim_(0),
...
inline LocalMPO::LocalMPO(MPO const &H, ITensor const &LH, int LHlim,
                          ITensor const &RH, int RHlim, Args const &args)
    : Op_(&H),
      PH_(H.length() + 2),
      LHlim_(LHlim),
      RHlim_(RHlim),
      ...
      Psi_(0)
{
    PH_.at(LHlim) = LH;
    PH_.at(RHlim) = RH;
    if (H.length() == 1) lop_.update(Op_->A(1), L(), R());
    if (H.length() == 2) lop_.update(Op_->A(1), Op_->A(2), L(), R());
    if (args.defined("NumCenter")) numCenter(args.getInt("NumCenter"));
}

void inline LocalMPO::product(ITensor const &phi, ITensor &phip) const
{
    if (Op_ != 0) {
        ...
        ITensor othr;
        if (nc_ == 2) {
            othr = (!L() ? dag(prime(Psi_->A(b), "Link"))
                         : L() * dag(prime(Psi_->A(b), "Link")));
            othr *= (!R() ? dag(prime(Psi_->A(b + 1), "Link"))
                          : R() * dag(prime(Psi_->A(b + 1), "Link")));
        } else if (nc_ == 1) {
            othr = (!L() ? dag(prime(Psi_->A(b), "Link"))
                         : L() * dag(prime(Psi_->A(b), "Link")));
            if (R()) othr *= R();
            ...
            if (!L()) {
                if (!R())
                    Error("LocalMPO: Empty L() and R() in function product");
                else
                    othr = R();
            } else {
                othr = L();
...
void inline LocalMPO::L(int j, ITensor const &nL)
{
    if (LHlim_ > j - 1) setLHlim(j - 1);
    PH_[LHlim_] = nL;
}

void inline LocalMPO::R(int j, ITensor const &nR)
{
    if (RHlim_ < j + 1) setRHlim(j + 1);
    PH_[RHlim_] = nR;
}

inline void LocalMPO::position(int b, MPS const &psi)
{
...
int inline LocalMPO::position() const
{
...
inline void LocalMPO::shift(int j, Direction dir, ITensor const &A)
{
    if (!(*this)) Error("LocalMPO is null");
    ...
    {
        if ((j + 1) != LHlim_) {
            std::cout << "j+1 = " << (j + 1) << ", RHlim_ = " << RHlim_
                      << std::endl;
...
inline void LocalMPO::makeL(MPS const &psi, int k)
{
    if (!PH_.empty()) {
        if (Op_ == 0)  // Op is actually an MPS
        {
            while (LHlim_ < k) {
                auto ll = LHlim_;
                PH_.at(ll + 1) =
                    (!PH_.at(ll) ? psi(ll + 1) : PH_[ll] * psi(ll + 1));
...
inline void LocalMPO::makeR(MPS const &psi, int k)
{
    if (!PH_.empty()) {
        if (Op_ == 0)  // Op is actually an MPS
        {
            while (RHlim_ > k) {
                const int rl = RHlim_;
                PH_.at(rl - 1) =
                    (!PH_.at(rl) ? psi(rl - 1) : PH_[rl] * psi(rl - 1));
        ...
        } else  // normal MPO case
        {
            while (RHlim_ > k) {
                auto rl = RHlim_;
                // printfln(" Making environment with rl=%d (using
                // H[%d])",rl,rl-1); Print(PH_.at(rl)); Print(Op_->A(rl-1));
                // Print(psi(rl-1));
                if (PH_.at(rl)) {
...
void inline LocalMPO::setLHlim(int val)
{
    if (!do_write_) {
        LHlim_ = val;
        return;
...
void inline LocalMPO::setRHlim(int val)
{
    if (!do_write_) {
        RHlim_ = val;
        return;
...
void inline LocalMPO::initWrite(Args const &args)
{
