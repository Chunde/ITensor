...
{
   public:
    DMRGObserver(MPS const &psi, Args const &args = Args::global());
    ...
    void virtual lastSpectrum(Spectrum const &spec) { last_spec_ = spec; }

    MPS const &psi() const { return psi_; }

    Spectrum const &spectrum() const { return last_spec_; }

   private:
    /////////////

    MPS const &psi_;
    Real energy_errgoal;  // Stop DMRG once energy has converged to this
                          // precision
    ...
    Real entropy_errgoal;  // Stop DMRG once entropy has converged to this
                           // precision
...
inline DMRGObserver::DMRGObserver(MPS const &psi, Args const &args)
    : psi_(psi),
...
void inline DMRGObserver::measure(Args const &args)
{
    auto N = length(psi_);
    ...
    //         //    auto z =
    //         (dag(prime(wfb,sb))*psi_.sites().op(opname,b)*wfb).eltC();
    //         //    //auto z =
    //         (prime(wfb,psi_.sites()(b))*psi_.sites().op(opname,b)*wfb).eltC();
    //         //    if(std::fabs(z.imag()) < 1E-14)
    //         //        printfln("<%s>(%d) = %.10E",opname,b,z.real());
    //         //    else
    //         //        printfln("<%s>(%d) =
    //         (%.10E,%.10E)",opname,b,z.real(),z.imag());
    //         //    }
    //         }
    //     }
    ...
    if (!silent && printeigs) {
        if (b == N / 2 && ha == 2) {
            ...
            // Normalize eigs
            Real norm_eigs = 0;
            for (auto &p : center_eigs) norm_eigs += p;
    ...
    if (!silent) {
        if (b == 1 && ha == 2) {
            ...
            println("    Largest link dim during sweep ", swstr, " was ",
                    (max_eigs > 1 ? max_eigs : 1));
            max_eigs = -1;
            println("    Largest truncation error: ",
                    (max_te > 0 ? max_te : 0.));
            max_te = -1;
...
bool inline DMRGObserver::checkDone(Args const &args)
{
    ...
    if (energy_errgoal > 0 && entropy_errgoal < 0 && sw % 2 == 0) {
        ...
        if (dE < energy_errgoal) {
            printfln(
                "    Energy error goal met (dE = %.3E < %.3E); returning after "
                "%d sweeps.",
    ...
    if (energy_errgoal < 0 && entropy_errgoal > 0 && sw % 2 == 0) {
        ...
        if (dS < entropy_errgoal) {
            printfln(
                "    Entropy error goal met (dS = %.3E < %.3E); returning "
                "after %d sweeps.",
                dS, entropy_errgoal, sw);
            last_entropy_ = 1000;
            return true;
        }
    }
    // Both energy_errgoal and entropy_errgoal are specified, stop when one of
    // them is met
    if (energy_errgoal > 0 && entropy_errgoal > 0 && sw % 2 == 0) {
        Real dE = std::fabs(energy - last_energy_);
        Real dS = std::fabs(S - last_entropy_);
        ...
        if (dE < energy_errgoal && dS < entropy_errgoal) {
            printfln(
                "    Energy (Entropy) error goal met (dE = %.3E < %.3E and dS "
                "= %.3E < %.3E); returning after %d sweeps.",
    ...
    // outer calling using same Observer may continue running e.g. infinite dmrg
    // calling finite dmrg.
    if (fileExists("STOP_DMRG")) {
        println("File STOP_DMRG found: stopping this DMRG run after sweep ",
                sw);
