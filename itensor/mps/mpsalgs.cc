...
void plussers(Index const &l1, Index const &l2, Index &sumind, ITensor &first,
              ITensor &second)
{
...
#endif
        sumind = Index(std::move(siq), dir(sumind), tags(sumind));
...
MPSType &addAssumeOrth(MPSType &L, MPSType const &R, Args const &args)
{
    auto N = length(L);
...
void fitWF(MPS const &psi_basis, MPS &psi_to_fit)
{
...
bool checkQNs(MPS const &psi)
{
    ...
    for (int i = 1; i <= N; ++i) {
        ...
        if (div(psi(i)) != Zero) {
            cout << "At i = " << i << "\n";
            Print(psi(i));
            cout << "ITensor other than the ortho center had non-zero "
                    "divergence\n";
            return false;
        }
    }
