...
namespace itensor
{
                  ...
                  Args const &args)
    {
        ...
        {
            L.ref(i) = dag(first.at(i - 1)) * L(i) * first.at(i) + dag(second.at(i - 1)) * R(i) * second.at(i);
        }
    ...
    fitWF(MPS const &psi_basis, MPS &psi_to_fit)
    {
        ...
        auto nrm = norm(A);
        if (nrm == 0)
            Error("Zero inner of psi_to_fit and psi_basis");
        A /= nrm;
