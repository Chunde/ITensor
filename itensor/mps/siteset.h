...
namespace itensor
{
    ...
    public:
        ...
        ITensor virtual op(std::string const &opname,
    ...
    public:
        ...
        IndexVal virtual state(std::string const &state)
        {
    ...
    public:
        ...
        {
            if (not sites_.at(j))
                Error("Unassigned site in SiteStore");
    ...
    {
        if (not *this)
            Error("Cannot call .op(..) on default-initialized SiteSet");
        if (opname == "Id")
        {
            auto s = si(i);
            auto id_ = ITensor(dag(s), prime(s));
            for (auto j : range1(dim(s)))
                id_.set(j, j, 1.0);
            return id_;
        }
        else if (opname == "Proj")
    ...
    ITensor inline op(SiteSet const &sites,
                      std::string const &opname,
                      int i,
