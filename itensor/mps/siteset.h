...
class SiteSet
{
   ...
   public:
    ...
    IndexVal operator()(int i, String const &state) const;

    // Get the operator indicated by
    //"opname" located at site i
    ITensor op(String const &opname, int i,
               Args const &args = Args::global()) const;

    void read(std::istream &s) { readType<GenericSite>(s); }
    ...
    Index si(int i) const { return operator()(i); }

    // Primed Index at Site i
    Index siP(int i) const { return prime(operator()(i)); }

    // Alternate name for operator()(i,state)
    IndexVal st(int i, String const &state) const
    {
        return operator()(i, state);
    }

    IndexVal stP(int i, String const &state) const
    {
        return prime(operator()(i, state));
    }

    template <typename SiteType>
    void set(int i, SiteType &&s);

   protected:
    void init(SiteStore &&sites);

    template <typename SiteType>
    void readType(std::istream &s);
};
...
class SiteBase
{
   public:
    ...
    Index virtual index() const = 0;

    ITensor virtual op(std::string const &opname, Args const &args) const = 0;

    IndexVal virtual state(std::string const &state) = 0;
};
...
class SiteHolder : public SiteBase
{
   ...
   public:
    ...
    Index virtual index() const { return s.index(); }

    ITensor virtual op(std::string const &opname, Args const &args) const
    {
        return s.op(opname, args);
    }

    IndexVal virtual state(std::string const &state) { return s.state(state); }
};
...
struct SiteStore {
   ...
   public:
    ...
    int length() const { return sites_.empty() ? 0 : sites_.size() - 1ul; }

    // Deprecated in favor of .length()
    int N() const
    {
        Global::warnDeprecated(
            ".N() is deprecated in favor of length(SiteStore)");
        return this->length();
    }

    Index si(int j) const
    {
        if (not sites_.at(j)) Error("Unassigned site in SiteStore");
        return sites_[j]->index();
    }

    IndexVal state(int j, std::string const &state)
    {
        if (not sites_.at(j)) Error("Unassigned site in SiteStore");
        return sites_[j]->state(state);
    }

    ITensor op(int j, std::string const &opname, Args const &args) const
    {
        if (not sites_.at(j)) Error("Unassigned site in SiteStore");
...
inline SiteSet::SiteSet(int N, int d)
{
...
inline SiteSet::SiteSet(int N) { SiteSet::init(SiteStore(N)); }

inline SiteSet::SiteSet(IndexSet const &is)
{
    auto N = is.length();
    auto sites = SiteStore(N);
    for (auto j : range1(N)) sites.set(j, GenericSite(is(j)));
    SiteSet::init(std::move(sites));
}

int inline SiteSet::length() const { return sites_ ? sites_->length() : 0; }

int inline length(SiteSet const &sites) { return sites.length(); }

IndexSet inline inds(SiteSet const &sites) { return sites.inds(); }

// Deprecated in favor of .length()
int inline SiteSet::N() const
{
    Global::warnDeprecated(".N() is deprecated in favor of length(SiteSet)");
    return this->length();
}

Index inline SiteSet::operator()(int i) const
{
    if (not *this)
        Error("Cannot retrieve site from default-initialized SiteSet");
    return sites_->si(i);
}

IndexSet inline SiteSet::inds() const
{
    if (not *this)
        Error("Cannot retrieve sites from default-initialized SiteSet");
    auto N = length();
...
IndexVal inline SiteSet::operator()(int i, String const &state) const
{
    if (not *this)
        Error("Cannot retrieve state from default-initialized SiteSet");
    return sites_->state(i, state);
}

template <typename SiteType>
void SiteSet::set(int i, SiteType &&s)
{
    sites_->set(i, std::move(s));
}

bool inline hasQNs(SiteSet const &sites)
{
...
ITensor inline SiteSet::op(String const &opname, int i, Args const &args) const
{
...
ITensor inline op(SiteSet const &sites, std::string const &opname, int i,
                  Args const &args = Args::global())
{
    return sites.op(opname, i, args);
}

void inline SiteSet::init(SiteStore &&store)
{
    sites_ = std::make_shared<SiteStore>(std::move(store));
}

template <typename SiteType>
void SiteSet::readType(std::istream &s)
{
...
void inline SiteSet::write(std::ostream &s) const
{
...
inline std::ostream &operator<<(std::ostream &s, SiteSet const &sites)
{
    s << "SiteSet:\n";
...
class BasicSiteSet : public SiteSet
{
   public:
    ...
    BasicSiteSet(int N, Args const &args = Args::global())
    {
        auto sites = SiteStore(N);
        for (int j = 1; j <= N; ++j) {
    ...
    BasicSiteSet(IndexSet const &is)
    {
        int N = is.length();
        auto sites = SiteStore(N);
        for (auto j : range1(N)) sites.set(j, SiteType(is(j)));
        SiteSet::init(std::move(sites));
    }

    void read(std::istream &s) { SiteSet::readType<SiteType>(s); }
};
...
class MixedSiteSet : public SiteSet
{
   public:
    ...
    MixedSiteSet(int N, Args const &args = Args::global())
    {
        auto sites = SiteStore(N);
        for (int j = 1; j <= N; ++j) {
            if (j % 2 == 1)
                sites.set(j, ASiteType({args, "SiteNumber=", j}));
            else
                sites.set(j, BSiteType({args, "SiteNumber=", j}));
        }
        SiteSet::init(std::move(sites));
    }
    ...
    MixedSiteSet(IndexSet const &is)
    {
        ...
        for (auto j : range1(N)) {
            if (j % 2 == 1)
                sites.set(j, ASiteType(is(j)));
            else
                sites.set(j, BSiteType(is(j)));
        }
        SiteSet::init(std::move(sites));
    }
    ...
    void read(std::istream &s)
    {
        ...
        if (N > 0) {
            ...
            for (int j = 1; j <= N; ++j) {
                ...
                I.read(s);
                if (j % 2 == 1)
                    store.set(j, ASiteType(I));
                else
                    store.set(j, BSiteType(I));
            }
            init(std::move(store));
        }
    }
...
SiteSetT merge(SiteSetT const &sites1, SiteSetT const &sites2,
               Args const &args = Args::global())
{
...
template <typename SiteType>
Index index(SiteType const &s)
{
    return s.index();
}

template <typename SiteType>
IndexVal state(SiteType const &s, std::string const &st)
{
    return s.state(st);
}

template <typename SiteType>
ITensor op(SiteType const &s, std::string const &opname,
           Args const &args = Args::global())
{
    return s.op(opname, args);
}
