...
template <typename MPSType>
void do_upgrade(string mpsfile,
                string sitefile,
                string new_mpsfile)
{
    ...
    if (!s.good())
        throw ITError("Couldn't open file \"" + mpsfile + "\" for reading");
...
int main(int argc, char *argv[])
{
    ...
    while (not inputok())
    {
        std::cin >> type;
        if (not inputok())
            printfln("Please input either 1 for MPS, or 2 for IQMPS. (Got %d.)", type);
    }

    if (type == 1)
        do_upgrade<MPS>(mpsfile, sitefile, new_mpsfile);
    else if (type == 2)
        do_upgrade<IQMPS>(mpsfile, sitefile, new_mpsfile);
