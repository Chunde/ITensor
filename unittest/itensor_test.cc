...
operator<<(std::ostream &s, Type t)
{
    if (t == Type::NoType)
        s << "NoType";
    else if (t == Type::DenseReal)
        s << "DenseReal";
    else if (t == Type::DenseCplx)
        s << "DenseCplx";
    else if (t == Type::DiagReal)
        s << "DiagReal";
    else if (t == Type::DiagRealAllSame)
        s << "DiagRealAllSame";
    else if (t == Type::DiagCplx)
        s << "DiagCplx";
    else if (t == Type::DiagCplxAllSame)
        s << "DiagCplxAllSame";
    else if (t == Type::Combiner)
        s << "Combiner";
    else if (t == Type::QDenseReal)
        s << "QDenseReal";
    else
                ...
                for (int k3 = 1; k3 <= 3; ++k3)
                    for (int j4 = 1; j4 <= 2; ++j4)
                    {
                        CHECK_CLOSE(elt(r, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)),
                                    f1 * elt(v, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)) + elt(w, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)) / f2);
                    }
        ...
        for (int j1 = 1; j1 <= 2; ++j1)
            for (int j2 = 1; j2 <= 2; ++j2)
                for (int k3 = 1; k3 <= 3; ++k3)
                    ...
                    {
                        CHECK_CLOSE(elt(r, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)),
                                    f1 * elt(yy, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)) + f2 * elt(zz, l1(j1), l2(j2), b3(k3), l4(j4), a2(1), a4(1)));
                    }
        ...
        SECTION("QNs")
        {
            ...
            SECTION("General Diag")
            {
                ...
                for (auto n : range(Nblocks))
                {
                    ...
                    if (d[n].size() != pD.size())
                        Error("Block size and vector size must match");
