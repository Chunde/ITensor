...
namespace itensor
{
  ...
  bool inline checkTags(MPO const &K,
                        ...
                        std::string const &linkTags = "Link,0")
  {
    ...
    for (auto n : range1(N))
    {
      if (n < N)
      {
        ...
        if (!hasTags(l, linkTags + ",l=" + str(n)))
          return false;
      }
      auto s = siteInds(K, n);
      if (!findIndex(s, siteTagsBra + ",n=" + str(n)))
        return false;
      if (!findIndex(s, siteTagsKet + ",n=" + str(n)))
        return false;
    }
    return true;
  }

  bool inline checkTags(MPS const &psi,
                        ...
                        std::string const &linkTags = "Link,0")
  {
    ...
    for (auto n : range1(N))
    {
      ...
      if (!hasTags(s, siteTags + ",n=" + str(n)))
        return false;
    }
    return true;
  }

  Real inline diff(MPS const &psi, MPS const &phi)
  {
  ...
  MPO inline randomUnitaryMPO(SiteSet const &sites, Args const &args = Args::global())
  {
